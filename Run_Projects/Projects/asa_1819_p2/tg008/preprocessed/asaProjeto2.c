...
int main()
{
  ...
  estacoes = new_sym_var(sizeof(int) * 8);
            ...
            if (j != 0)
            {
              if (transposed->mt[i][j] == flux->mt[j][i])
              {
                ...
                else
                {
                  ...
                  {
                    if ((i >= ((2 + fornecedores) + estacoes)) && (abs(i - j) == estacoes))
                    {
...
int bfs(int start, int end)
{
  ...
  while (headBfs != tailBfs)
  {
    ...
    for (j = 0; j <= (totalNodes - 1); j++)
    {
      if ((colorsBFS[j] == 0) && ((capacity->mt[i][j] - flux->mt[i][j]) > 0))
      {
        enqueue(j);
        predecessorBFS[j] = i;
      }
