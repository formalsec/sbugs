...
int main()
{
  ...
  n_chp = new_sym_var(sizeof(int) * 8);
  n_transp = new_sym_var(sizeof(int) * 8);
  ...
  for (i = 0; i < ret.n_arcs; i++)
  {
    for (j = i; j > 0; j--)
    {
      ...
      {
        if ((cut[j]->from == cut[j - 1]->from) && (cut[j]->to < cut[j - 1]->to))
        {
          aux_t = cut[j - 1];
          cut[j - 1] = cut[j];
          cut[j] = aux_t;
...
void parse_input(vert *verts, int n_sup, int n_vert)
{
  ...
  for (i = 2; i <= (n_sup + 1); i++)
  {
    verts[i].limit = new_sym_var(sizeof(int) * 8);
...
void parse_matrix(vert *verts, transp *arcs, int n_transp, int n_vert, int n_sup)
{
  ...
  {
    transp path;
    path.from = new_sym_var(sizeof(int) * 8);
    path.to = new_sym_var(sizeof(int) * 8);
    path.limit = new_sym_var(sizeof(int) * 8);
    path.flow = 0;
    path.status = 100;
    path.cut = 103;
    arcs[i] = path;
...
inp push(vert *verts, int pos, inp n_e)
{
  ...
  for (i = 0; i < verts[pos].n_out; i++)
  {
    ...
    if ((((verts[pos].out[i]->status == 100) && (verts[to].height < verts[pos].height)) && (((verts[to].limit - verts[to].flow) > 0) || (verts[to].type == 't'))) && ((verts[pos].limit - verts[pos].flow) > 0))
    {
...
ret find_arcs(vert *verts, int *positions, int n_parse, transp **cut, int n_cut, int *cut_v, int nv, int n_vert)
{
  ...
  for (i = 0; i < n_parse; i++)
  {
    ...
    else
    {
      ...
      {
        int pos = positions[i];
        if (verts[verts[pos].entry[j]->from].status == 104)
        {
