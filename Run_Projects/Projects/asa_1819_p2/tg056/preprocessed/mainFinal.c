...
int main()
{
  ...
  for (i = 1; i < ((2 + (num_estacoes * 2)) + num_fornecedores); i++)
  {
    ...
    while (a != 0)
    {
      ...
      {
        if (a->origem->id > ((1 + num_fornecedores) + num_estacoes))
        {
          a->origem->id -= num_estacoes;
        }
        else
        {
...
void lerInput()
{
  ...
  int d;
  int c;
  num_fornecedores = new_sym_var(sizeof(int) * 8);
  num_estacoes = new_sym_var(sizeof(int) * 8);
  num_lig = new_sym_var(sizeof(int) * 8);
...
void push(int u, int v)
{
  ...
  if ((aresta != 0) && (arestaRev != 0))
  {
    send = (vertices[u]->excesso < (aresta->capacidade - aresta->fluxo)) ? (vertices[u]->excesso) : (aresta->capacidade - aresta->fluxo);
    aresta->fluxo += send;
...
void relabel(int u)
{
  ...
  for (v = 0; v < NODES; v++)
  {
    ...
    if (aresta != 0)
    {
      ...
      {
        min_height = (min_height < aresta->destino->altura) ? (min_height) : (aresta->destino->altura);
...
int pushRelabel(int source, int sink)
{
  ...
  vertices[source]->excesso = 10000000;
  for (i = 0; i < NODES; i++)
    push(source, i);
