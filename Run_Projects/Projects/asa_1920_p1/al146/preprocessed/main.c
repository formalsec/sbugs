...
void readInput(void)
{
  ...
  for (i = 0; i < v; i++)
  {
    ...
    {
      aux[0] = new_sym_var(sizeof(int) * 8);
  ...
  for (i = 0; i < e; i++)
  {
    ...
    {
      aux[0] = new_sym_var(sizeof(int) * 8);
      aux[1] = new_sym_var(sizeof(int) * 8);
...
void printGraph(void)
{
  ...
  {
    printf("\nAdjacent to %d\n", i + 1);
    for (aux = graph->vertices[i]; aux != 0; aux = aux->next)
      printf("%d-", aux->dest + 1);
...
void strongConnect(int vertex)
{
  ...
  pushStack(vertex);
  onStack[vertex] = 1;
  for (node = graph->vertices[vertex]; node != 0; node = node->next)
