...
void push_stack(Stack* stack, int number) {
    ...
    if (stack->top_index == EMPTY) {
        ...
        stack->stack_list = (int*)realloc(stack->stack_list,
...
AlgorithmData* initialize_data(int num_students) {
    ...
    new_data->states = (State*)calloc(
        num_students, sizeof(int)); /* Initialized as 0 (NOT_VISITED) */

    new_data->disc
        = (int*)malloc(sizeof(int) * num_students); /* Time of discovery */
    new_data->lows
        = (int*)malloc(sizeof(int) * num_students); /* Low-link values */
