...
int main(){
	...
	vertices=malloc(sizeof(vertices_s)*(num_vertices));

	for (cont=0; cont<num_vertices; cont++){

		scanf("%d",&val);
		vertices[cont].valor = val;
		vertices[cont].adj=malloc(sizeof(int)*2);
	...
	for (cont=0; cont< num_ligacoes; cont++){
		...
		if(vertices[indice].deg>vertices[indice].size){
			...
			vertices[indice].adj=realloc(vertices[indice].adj, sizeof(int)*vertices[indice].size);
		}
