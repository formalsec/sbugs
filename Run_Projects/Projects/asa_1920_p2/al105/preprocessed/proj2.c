...
void delEdge(struct Graph *graph, int src, int dst)
{
  ...
  while (temp)
  {
    ...
    if ((temp->next != 0) && (temp->next->vertex == dst))
    {
      aux = temp->next;
...
void preencher_supermercados(int S, int M, int N, struct Graph *graph)
{
  ...
  for (i = 0; i < S; i++)
  {
    ...
    if (flag == 1)
    {
      graph->adjLists[vertice]->capacity++;
    }
    else
    {
...
int bfs(struct Graph *graph, int startVertex, int M, int N, int pred[])
{
  ...
  pred[startVertex] = -1;
  if (graph->adjLists[startVertex] == 0)
  {
    return 0;
  }
  else
  ...
  while (!isEmpty(q))
  {
    ...
    while (temp)
    {
      ...
      if ((graph->adjLists[adjVertex] != 0) && (graph->adjLists[adjVertex]->capacity > 1))
      {
      ...
      if ((graph->adjLists[adjVertex] != 0) && (graph->adjLists[adjVertex]->visited == 0))
      {
