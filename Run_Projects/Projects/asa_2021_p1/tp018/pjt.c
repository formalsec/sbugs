...
int main(){
    ...
    int *topSort = (int*)malloc((nNodes+1)*sizeof(int)); /*Lista para guardar id dos nodos segundo a sua ordenacao topologica*/
    Node *nodeList = (Node *)malloc((nNodes+1) * sizeof(Node)); /*struct array dos nodos*/
    for(i=0; i<=nNodes; i++){
        nodeList[i].adj = malloc(sizeof(int)* DIM);
        nodeList[i].adjParent = malloc(sizeof(int)* DIM);
