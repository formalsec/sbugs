...
void pushFIFO(FIFO fifo, Node node)
{
  fifo->list[fifo->tail++] = node;
}

Node popFIFO(FIFO fifo)
{
  return fifo->list[fifo->head++];
}

int emptyFIFO(FIFO fifo)
{
  return (fifo->tail - fifo->head) == 0;
}
...
void setNodeEdgeNumber(Node node, int number)
{
  node->edgeNumber = number;
}

int getNodeEdgeNumber(Node node)
{
  return node->edgeNumber;
}
...
int isNodeEquals(Node node, Node other)
{
  return getNodeNumber(node) == getNodeNumber(other);
}

void setGraphNodeNumber(Graph graph, int number)
{
  graph->nodeNumber = number;
}
...
void addGraphNode(Graph graph, int index, Node node)
{
  graph->nodeList[index] = node;
}

Node getGraphNode(Graph graph, int index)
{
  return graph->nodeList[index];
}

Graph createGraph()
{
  return (Graph) malloc(sizeof(graphObject));
}
...
void inicializeGraphSource(Graph graph)
{
  setGraphSource(graph, createNode(-1));
}

void inicializeGraphSink(Graph graph)
{
  setGraphSink(graph, createNode(-2));
}

void readTwoInts(int *first, int *second)
{
  for (int first_index = 0; first_index < 10; first_index++)
  {
    first[first_index] = new_sym_var(sizeof(int) * 8);
  }

  for (int second_index = 0; second_index < 10; second_index++)
  {
    second[second_index] = new_sym_var(sizeof(int) * 8);
  }
...
void readThreeInts(int *first, int *second, int *third)
{
  ...
  for (int third_index = 0; third_index < 10; third_index++)
  {
    third[third_index] = new_sym_var(sizeof(int) * 8);
  }
...
void readFirstLine(int *processNumber, int *relationsNumber)
{
  readTwoInts(processNumber, relationsNumber);
}
...
void resetBFSSetup(Graph graph)
{
  ...
  {
    node = getGraphNode(graph, i);
    setNodeDiscovered(node, 0);
    setNodeParent(node, 0);
    setNodeParentEdge(node, 0);
  }
