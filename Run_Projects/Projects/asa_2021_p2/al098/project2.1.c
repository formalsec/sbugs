...
FIFO createFIFO(int maxSize) {
    ...
    FIFO fifo = (FIFO)malloc(sizeof(fifoObject));
    fifo->head = 0;
    fifo->tail = 0;
    fifo->maxSize=maxSize;
    fifo->list = (Node*)malloc(sizeof(Node)*maxSize);
    return fifo;
}
...
Stack createStack(int maxSize) {
    ...
    Stack stack = (Stack)malloc(sizeof(stackObject));
    stack->top = -1;
    stack->list = (Edge*)malloc(sizeof(Edge*)*maxSize);
    return stack;
}
...
Edge createEdge(Node node, int maxCapacity) { 
    ...
    Edge edge = (Edge)malloc(sizeof(edgeObject));
...
void addNodeEdge(Node node, Edge edge) { 
    ...
    Edge* aux = (Edge*)malloc(sizeof(Edge) * (edgeNumber + 1));
...
Node createNode(int number) { 
    ...
    Node node = (Node)malloc(sizeof(nodeObject));
...
/* Inicialize and Delete */
Graph createGraph() { return (Graph)malloc(sizeof(graphObject)); }
...
void inicializeGraphNodeList(Graph graph) { 
    ...
    Node *list = (Node*)malloc(sizeof(Node) * (size+1));
