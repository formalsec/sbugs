  ...
  while (queue)
  {
    ...
    while (visiting)
    {
      ...
      if ((capacity > 0) && (graph[v].visited != been_visited))
      {
        ...
        if (0)
        {
          printf("Added node %d with max capacity %d\n", v, min(queue->max_capacity, capacity));
        }
        else
        {
...
int edmonds_karp(node *graph)
{
  ...
  while (1)
  {
    queue_element *origin = push_queue(0, SOURCE, -50);
...
int main()
{
  ...
  {
    n_processes = new_sym_var(sizeof(int) * 8);
  ...
  for (i = 1; i < SINK; i++)
  {
    ...
    {
      cost1 = new_sym_var(sizeof(int) * 8);
      cost2 = new_sym_var(sizeof(int) * 8);
