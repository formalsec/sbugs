...
queue_element * push_queue(queue_element * head, int n, int max_capacity) {
	queue_element * new_next = (queue_element *) malloc(sizeof(queue_element));
	new_next->n = n;
...
int main() {
    ...
    graph = malloc(sizeof(node) * V);

    /* Initializing source */
    graph[SOURCE].capacities = malloc(sizeof(edge) * V);
    graph[SOURCE].visited = 0;

    /* Initializing sink */
    graph[SINK].capacities = malloc(sizeof(edge) * V);
    ...
    for (i = 1; i < SINK; i++) {
        ...
        graph[i].capacities = malloc(sizeof(edge) * V);
        graph[i].visited = 0;
