...
int main() {
  ...
  if (scanf("%d %d", &n, &k) == 2) {                             /*memory for:*/
    graph.Cij = malloc(sizeof(int*)*(n+2));          /*capacity matrix lines*/
    graph.Fij = malloc(sizeof(int*)*(n+2));              /*flow matrix lines*/
    graph.color = malloc(sizeof(int)*(n+2));                    /*node color*/
    graph.pred = malloc(sizeof(int)*(n+2));
    q = malloc(sizeof(int)*(n+2));
    for (i = 0; i < (n+2); i++) {
      graph.Cij[i] = calloc(sizeof(int), (n+2));    /*capacity matrix columns*/
      graph.Fij[i] = calloc(sizeof(int), (n+2));        /*flow matrix columns*/
    }
