...
int main(){
	...
	product p;
	product *system = (product*)malloc(MAX_ID*sizeof(product)); /*products array*/
	order o;
	order *orders = (order*)malloc(ORDERS*sizeof(order)); /*orders array*/
	...
	while(1){
		...
		switch(input[0]){
			case 'a':; /*if the first letter is 'a'*/
				...
				p.price = (int*)malloc(sizeof(int)); /*allocating memory*/
				/*parsing*/
				input[0] = ' ';
