...
int main(){
    ...
    list_games = malloc(MAX_GAMES*sizeof(Game));                                /* initializes table to store games by order of entry */
    for (i = 0; i < MAX_GAMES; i++)
        list_games[i] = NULL;

    heads_teams = malloc(TABLE*sizeof(Link_team));                              /* initializes the hashtable for teams */
    for (i = 0; i < TABLE; i++)
        heads_teams[i] = NULL;

    heads_games = malloc(TABLE*sizeof(Link_game));                              /* initializes the hashtable for games */
    for (i = 0; i < TABLE; i++)
        heads_games[i] = NULL;
...
Instruction get_game_info(char input[MAX_INPUT], int i){
    ...
    temp = malloc((j+1)*sizeof(char));                                          /* ALOCA O ESPACO NECESSARIO PARA O NOME */
    ...
    temp = realloc(temp, (j+1)*sizeof(char));                                   /* REALOCA O ESPACO NECESSARIO PARA O NOME */
    k = j;                                                                      /* CONTADOR DECRESCENTE PARA COPIAR DE SEGUIDA */
...
Link_game store_game(Link_game head, Game new_game, Game *list_games){
        Link_game temp = malloc(sizeof(struct node_game));                      /* MEMORIA NECESSARIA PARA O NOVO JOGO */
        int i;
