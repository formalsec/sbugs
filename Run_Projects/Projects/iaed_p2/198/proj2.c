...
listT* mk_listT(){ /*make listT*/
	listT *l = malloc(sizeof(listT)); /*allocate space for listT*/
	l->head = l->last = NULL; /*initialize listT as null*/
	return l;
}

listG* mk_listG(){ /*make listT*/
	listG *l = malloc(sizeof(listG)); /*allocate space for listT*/
	l->head = l->last = NULL; /*initialize listT as null*/
	return l;
}
...
void add_lastT(listT *l, const char*n){
	nodeT *nn = malloc(sizeof(nodeT)); /*allocate for node*/
	nn-> previous = l->last; /*define nn as the last node*/
	nn-> next = NULL; /*define next as NULL*/
	nn-> name = malloc(sizeof(char)*(strlen(n)+1)); /*allocate, +1 for /0 used by strcpy*/
...
void add_lastG(listG *l, const char*n, const char*T1, const char*T2, int* s1, int* s2){
	nodeG *nn = malloc(sizeof(nodeG)); /*allocate for node*/
	nn-> previous = l->last; /*define nn as the last node*/
	nn-> next = NULL; /*define next as NULL*/
	nn-> name = malloc(sizeof(char)*(strlen(n)+1)); /*allocate, +1 for /0 used by strcpy*/
...
int* scan_int(){
	int *s = malloc(sizeof(int)); /*auxiliar for score*/
