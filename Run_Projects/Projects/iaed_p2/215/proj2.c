...
void adiciona_jogo(char argv[]){
    ...
    jogos[hash].head = malloc(sizeof(struct Jogo));  /*aloca espaco para o novo membro*/
...
void adiciona_equipa(char argv[]){
	...
	if(procura_equipa(argv) != NULL){         /*caso ja exista uma equipa com o mesmo nome*/
		...
		if(!x){      /*caso inicial*/                          
			equipas[hash].head = malloc(sizeof(struct Equipa));    /*aloca espaco para a nova equipa*/
			...
			x = malloc(sizeof(struct Equipa));     /*aloca espaco para a nova equipa*/
