...
{
	lugar_no_hash_j *lista_j = malloc(Max_tam*sizeof(lugar_no_hash_j));
	lista_j -> jogo = jg;
...
lugar_no_hash_j **iniciar_hash_jogo(int m)
{
	...
	hashtable_j = malloc(Max_tam*sizeof(lugar_no_hash_j));
	for (i = 0; i < M; i++)
		hashtable_j[i] = NULL;
	return hashtable_j;
}
...
link **iniciar_listinha(int m)
{
	...
	link **lista;
	M = m;
	lista = malloc(Max_char*sizeof(link));
	for (i = 0; i < M; i++)
		lista[i] = NULL;
	return lista;
}
...
lugar_no_hash_e **iniciar_hash_equipe(int m) 
{
	...
	M = m;
	hashtable_e = malloc(Max_tam*sizeof(lugar_no_hash_e));
	for (i = 0; i < M; i++) 
		hashtable_e[i] = NULL;
	return hashtable_e;
}
...
{
	lugar_no_hash_e *lista_e = malloc(Max_tam*sizeof(lugar_no_hash_e));
	lista_e -> equipe = eq;
