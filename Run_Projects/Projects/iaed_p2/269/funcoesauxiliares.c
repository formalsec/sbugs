...
{
	jogos *hashtable_jogos = malloc(MAX_TAMANHO_TABELA * sizeof(struct jogo)); 
...
{
	equipas *hashtable_equipas = malloc(MAX_TAMANHO_TABELA * sizeof(struct equipa)); 
	/* Aloca espaco a memoria */
	int p;
	for (p = 0; p < MAX_TAMANHO_TABELA; p++)
	{
...
/*Funcao auxiliar com funcao de dar free aos mallocs ou seja libertar a memoria que foi 
...
/*Funcao auxiliar com a funcao de dar free aos mallocs usados para criar a hashtable com
