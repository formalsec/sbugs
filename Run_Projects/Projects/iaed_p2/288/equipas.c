...
void alphasort(char **nomes, int count)  							/* Ordena alfabticamente uma array de strings (descricoes de produtos)*/
{
	...
	for(i=0;i<=count;i++)											/* Ciclos for que ordenam a array atraves de strcmp*/
	{
      	for(j=i+1;j<=count;j++)
      	{
        	...
        	{														
            	temp=malloc(sizeof(char)*(strlen(nomes[i])+1)); 	/* alocacao da var temp com o tamanho de nomes[i]+1 para permitir a copia seguinte*/
            	strcpy(temp,nomes[i]);
            	nomes[i]=realloc(nomes[i], (strlen(nomes[j])+1)); 	/* realocacao da var nomes[i] com o tamanho de nomes[j]+1 para permitir a copia seguinte*/
            	strcpy(nomes[i],nomes[j]);
            	nomes[j]=realloc(nomes[j], (strlen(temp)+1)); 	  	/* realocacao da var nomes[j] com o tamanho de temp+1 para permitir a copia seguinte*/
           		strcpy(nomes[j],temp);
