...
void processa(Avl avl, Hashtable ht, Lista lst){
    char cmd;
    int NL = 0; /*Numero da linha de cada comando*/
    while(((cmd = getchar()) != 'x')){
        if(cmd >= 'A') /*Incrementa NL sempre que for recebido um comando novo valido*/
            NL++;
        switch(cmd){
...
/*Inicializa toda a memoria necessaria para o programa*/
...
/*Limpa toda a memoria alocada dinamicamente antes de sair do programa*/
...
/*Obtem o nome a partir do input*/
void obtem_nome(char* nome){
    scanf(" %[^:\n]", nome);
}

/*Obtem o score a partir do input*/
void obtem_score(int score[]){
    scanf(":%d:%d", &score[0], &score[1]);
}

/*Obtem o nome das equipas a partir do input*/
...
void adiciona_jogo(Avl avl, Hashtable ht, Lista lst, int NL){
    ...
    /*Se o jogo ainda nao existir, procede com a verificacao das equipas e posterior criacao do jogo*/
    if(NULLitem(link_ht)){
        for(i = 0; i < MAX_E; i++){
            link_avl[i] = avl_verifica(avl, nome_equipas[i], 0, NL);
            if(NULLitem(link_avl[i])) /*Sai da funcao se alguma das equipas nao existir*/
                return;
            equipas[i] = link_avl[i]->equipa; /*Insere a equipa no no da AVL criado para a equipa*/
        }
...
void adiciona_equipa(Avl avl, int NL){
    ...
    /*Se a equipa ainda nao existir, entao cria espaco para a mesma e insere-a na AVL*/
    if(NULLitem(link)){
...
void lista_melhores(Avl avl, int NL){
    ...
    if(!NULLitem(avl->head)){ /*Caso a AVL nao esteja vazia, ou seja, caso existirem equipas*/
        printf("%d Melhores %d\n", NL, max_ganhos);
