...
void readInput(struct Vertice **routers, int v) {
  struct Vertice *router = malloc(sizeof(struct Vertice)*v);
  int i, l1, l2;
  for (i = 1; i <= v; i++) {
...
int Tarjan_Aux(struct Vertice **routers, int **d, int cutVertice, node_t *head, int id, int n) {
  int aux = **d;
  struct Vertice *router = malloc(sizeof(struct Vertice)*n);
  router = *routers;
  if (id > head->id) {
    head->id = id;
  }
...
int main() {
  int v;
  scanf("%d", &v);
  struct Vertice *routers = malloc(sizeof(struct Vertice)*v);
  readInput(&routers, v);
  Tarjan(&routers, v);
  DFS(&routers, v);
  return 0;
