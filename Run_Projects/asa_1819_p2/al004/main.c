...
item *init_item(unsigned from, unsigned to, unsigned c)
{
    ...
    new->_key = (item_key *)malloc(sizeof(item_key));
    new->_key->_from = from;
    new->_key->_to = to;
    new->_c = c;
    new->_f = 0;
...
{
    item_key *res = (item_key *)malloc(sizeof(item_key *));
    res->_from = from;
    res->_to = to;
...
void merge(item *edges[], unsigned i, unsigned m, unsigned f)
{
    ...
    item **aux = (item **)malloc(sizeof(item *) * size);
    unsigned aux_n = 0;
...
graph *init_graph(unsigned V)
{
    ...
    res->_vertexs = (vertex **)malloc(sizeof(vertex *) * V);
    res->v_number = V;
    res->e_number = 0;
...
{
    unsigned i;
    unsigned *labels = (unsigned *)malloc(sizeof(unsigned) * g->v_number);
    vertex *u = NULL;
    item *it;
    list_node *head = NULL;
    list_node *tail = NULL;
...
int main()
{
    ...
    */
    ...
    edges = (item **)malloc(sizeof(item *) * edge_count);
