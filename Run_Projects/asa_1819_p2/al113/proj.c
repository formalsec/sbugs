...
int pushRelabel(Graph g) {
	int i, flow;
	Link k;
	int* e = (int*) malloc(sizeof(int)*(g->numVertices)); /* vertices' excess */
	int* h = (int*) malloc(sizeof(int)*(g->numVertices)); /* vertices' heights */
...
void performAlgorithm(Graph g) {
	int i, j, maxFlow;
	Link k;
	int* reach = (int*) malloc(sizeof(int)*(g->numVertices)); /* vertices' reach (1 - can reach sink, 0 - can't reach sink) */
	int* critical = (int*) malloc(sizeof(int)*(g->numVertices)); /* stores the destination of the min cut edges */
