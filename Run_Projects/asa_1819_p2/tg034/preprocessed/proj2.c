...
int main()
{
  ...
  P_Connection edge_list = 0;
  P_Vertex supplier_list = 0;
  unsigned long int number_of_productors;
  unsigned long int number_of_suppliers;
  unsigned long int number_of_connections;
  unsigned long int number_of_vertexes;
  number_of_productors = new_sym_var(sizeof(unsigned long int) * 8);
  number_of_suppliers = new_sym_var(sizeof(unsigned long int) * 8);
  number_of_connections = new_sym_var(sizeof(unsigned long int) * 8);
...
void GetProductors(Vertex list_of_vertexes, unsigned long int number_of_productors)
{
  ...
  {
    prodcap = new_sym_var(sizeof(unsigned long int) * 8);
...
void Getsuppliers(Vertex list_of_vertexes, unsigned long int f, unsigned long int number_of_suppliers)
{
  ...
  {
    supcap = new_sym_var(sizeof(unsigned long int) * 8);
...
void readConections(Vertex list_of_vertexes, unsigned long int number_of_connections, unsigned long int number_of_productors, unsigned long int number_of_suppliers)
{
  ...
  {
    vertex1_id = new_sym_var(sizeof(unsigned long int) * 8);
    vertex2_id = new_sym_var(sizeof(unsigned long int) * 8);
    capacity = new_sym_var(sizeof(unsigned long int) * 8);
...
void addConnection(Vertex vertexes, unsigned long int id1, unsigned long int id2, unsigned long int cpct)
{
  ...
  if (vertexes[id1].list_of_connections == 0)
  {
  ...
  while (!done)
  {
    ...
    for (i = 2; i < number_of_vertexes; i++)
    {
      while (list_of_vertexes[i].flow != 0)
      {
        ...
        for (c = list_of_vertexes[i].list_of_connections; c != 0; c = c->next_node)
        {
          x = c->capacity - c->flow;
        ...
        {
          for (p = list_of_vertexes[i].list_of_previous; p != 0; p = p->next)
          {
...
void findReachable(Vertex list_of_vertexes, unsigned long int id)
{
  P_Connection p;
  Node c;
  list_of_vertexes[id].reachable = 1;
  for (p = list_of_vertexes[id].list_of_previous; p != 0; p = p->next)
  {
  ...
  for (c = list_of_vertexes[id].list_of_connections; c != 0; c = c->next_node)
  {
...
void printSup(P_Vertex supplier_list)
{
  ...
  for (p = supplier_list; p != 0; p = p->next)
  {
    if (p->next != 0)
    {
      printf("%lu ", p->v->id);
