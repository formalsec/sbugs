...
int main()
{
  ...
  number_of_routers = new_sym_var(sizeof(unsigned long int) * 8);
...
Router readInput(unsigned long int number_of_routers)
{
  ...
  {
    routers[i].id = i + 1;
    routers[i].list_of_connections = 0;
    routers[i].visited = !1;
  ...
  {
    router1_id = new_sym_var(sizeof(unsigned long int) * 8);
    router2_id = new_sym_var(sizeof(unsigned long int) * 8);
...
unsigned long int DFS(Router routers, unsigned long int position, unsigned long int *time, unsigned long int *articulation_points)
{
  ...
  int children = 0;
  routers[position].visited = 1;
  routers[position].discovery_time = (*time)++;
  for (c = routers[position].list_of_connections; c != 0; c = c->next_node)
  {
