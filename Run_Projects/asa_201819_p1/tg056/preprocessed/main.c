  ...
  if (p->p_art != 1)
  {
    ...
    while (1)
    {
      ...
      if (filho->tempo_d == 0)
      {
        ...
        filhos++;
        visit(aux->id);
        p->low = (p->low < filho->low) ? (p->low) : (filho->low);
        if ((p->pai == (-1)) && (filhos > 1))
        {
          p->p_art = 1;
        }
        ...
        if ((p->pai != (-1)) && (filho->low >= p->tempo_d))
        {
          p->p_art = 1;
        }
        else
      ...
      else
      {
        if (p->pai != aux->id)
        {
          p->low = (p->low < filho->tempo_d) ? (p->low) : (filho->tempo_d);
        }
        else
        {
