...
int Funcao_A(char argum[])
{
	...
	{
		n_teams = 0;
		Equipa = malloc(sizeof(equipas)*1);                              /* aloca a memoria necessaria na estrutura */
		...
		for(i = 0; i <= n_teams; i++)
		{
			if(strcmp(Equipa[i].equipa, team_1) == 0)
			{
				for(j = 0; j <= n_teams; j++)
				{
					...
					{
						n_jogos = 0;
						Jogo = malloc(sizeof(jogos)*1);                    /* aloca memoria dinamica necessaria para a struct de jogos */
...
int Funcao_r(char argum[])
{
	...
	if(flag_existe >= 0)                                                   /* procede a eliminacao do jogo do sistema */
	{
		...
		Jogo = realloc(Jogo, sizeof(jogos)*n_jogos);                      /* realoca o espaco necessario, de novo, para a memoria dinamica */
		n_jogos = n_jogos - 1;
