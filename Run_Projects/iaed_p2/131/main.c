    ...
    while (x < m)                                         /*percorrer heads da hashtable*/
    {
        ...
        for (t = heads[x]; t != NULL; t = t->next)                /*percorrer lista da hastable correspondente a uma hashfunction*/
        {
            ...
            if (t->equipa->vitorias > v)
            {
                v = t->equipa->vitorias;
                free(array);
                array = malloc(sizeof(link));                      /*re mallocar caso max de vitorias altere*/
                array[0] = t;
                contador = 1;
            }
            ...
            {
                contador++;
                array = realloc((void *)array, sizeof(link) * contador);      /*fazer realloc caso haja mais uma equipa com mesmas vitorias*/
                array[contador - 1] = t;
            }
        }
...
void funcaox(Hash *HashTableEquipas, Hash_Jogo *HashTableJogo, list *ponteiro_lista_unica)  /*fazer free de todos os mallocs no projeto*/
{
