      ...
      if (strcmp(temp->name,nome)==0){
        if (temp==jogos[i]){
          if (temp->golos_casa > temp->golos_fora){                                 /* a string vencedor serve para mais tarde dentro da funcao descobrir */
            vencedor=(char*)malloc(sizeof(char)*(strlen(temp->equipa1->nome)+1));   /* em que equipa vai ser decrementada a vitoria caso o jogo que e para ser eliminado tinha vencedor*/
            existe=1;
...
void sort_teams(int inp,node_equipa *equipas){
  ...
  node_equipa x;
    ...
    if (existe==1){
    node_equipa *array=malloc(sizeof(struct stru_node_equipa)*capacity); /*  e criado um vetor de estruturas  de equipas dinamico */
    for(j=0;j<n;j++){
      for(x=equipas[j];x!=NULL;x=x->next){
        if((used>=capacity)&&(x->vitorias==maximo))                      /* a variavel use diz nos se o vetor esta sobrelotado*/
        {
          capacity*=2;
          array=(node_equipa*)realloc(array,sizeof(struct stru_node_equipa)*capacity);   /* esta aqui a realocacao dinamica do vetor de estruturas fazendo jus a sua caracteristica  */
