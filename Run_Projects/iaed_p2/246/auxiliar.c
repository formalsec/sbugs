...
init functions will malloc space for my hashtables(both game and team) depending on the 'TableSize' 
defined on structs.h . These functions will also set every index of the hashtables to NULL.

##################################################################################################*/
link_game* init_j(){
    int i;
    link_game *heads = malloc(TableSize*sizeof(struct game));
...
link_team* init_t(){
    int i;
    link_team *heads = malloc(TableSize*sizeof(struct team));
...
Both NEWgame and NEWteam will malloc space to the information needed, put the information on the 
...
insertBegin will malloc space for the necessary information and insert that node at the beginning
of the linked list.
