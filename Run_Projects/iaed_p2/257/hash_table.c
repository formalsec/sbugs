...
Game_node** init_table_game(){
	int i;
	Game_node **games_table = malloc(HASH_SIZE * sizeof(Game_node*));		/* creates pointer to the hash table */
...
Team_node** init_table_team(){
	int i;
	Team_node **team_table = malloc(HASH_SIZE * sizeof(Team_node*));		/* creates pointer to the hash table */
	for (i = 0; i < HASH_SIZE; i++)
		team_table[i] = NULL;
	return team_table;
}
