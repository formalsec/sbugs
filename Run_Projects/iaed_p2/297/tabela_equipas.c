...
equipa* make_equipa(size_t vit, const char* nome){
    equipa* new_e = malloc(sizeof(equipa));     /* aloco espaco equipa */
    new_e->flag = malloc((strlen(nome) + 1) * sizeof(char));    /* aloca espaco flag */
...
char** encontra_melhores(size_t n_strings, size_t max, equipas* tb_e){
    size_t i, j = 0;
    char** melhores = calloc(n_strings, sizeof(char*)); /* gera espaco adequado */
