...
int main() {
	...
	games **games_ht = calloc(GAMES_SZ, sizeof(games *)); /* hash table for games*/
	game_names *names = NULL, **names_pt = &names; /* linked list of games */
	teams **teams_ht = calloc(TEAMS_SZ, sizeof(teams *)), /* hash table for teams */
			/* stores the teams contiguously */
		  **teams_contiguous = calloc(TEAMS_SZ, sizeof(teams *));
	char command;
