...
Team* newTeam(char* name){
    Team* ptr_team = (Team*)malloc(sizeof(Team));    /* Allocate space for Team structure */

    ptr_team -> name = (char*)malloc(sizeof(char)*(strlen(name)+1));  /* Allocate and point to space for name of Team */
