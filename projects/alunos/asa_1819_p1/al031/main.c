...
graph graphInit(int V) {
	...
	g->visit = 1;
	g -> d = (int*) can_fail_malloc(V * sizeof(int));
	g -> d_cut = (int*) can_fail_malloc(V * sizeof(int));
	g -> low = (int*) can_fail_malloc(V * sizeof(int));
	g -> low_cut = (int*) can_fail_malloc(V * sizeof(int));
	g -> parent = (int*) can_fail_malloc(V * sizeof(int));
	g -> adj = (node*) can_fail_malloc(V * sizeof(node));
	for (v = 0; v < V; v++) {
		initVertex(g, v);
	}
	return g;
