...
Network NETWORKinit(int V) {
   int v;
   Network G = can_fail_malloc(sizeof*G);
   G->V = V; 
   G->A = 0;
...
void BrokenRouters(Network G, int *cc){
   ...
   int size = G->V;
   int *des = (int*)can_fail_malloc(sizeof(int)*size);
   int *visitado = (int*)can_fail_malloc(sizeof(int)*size);
   int *pai = (int*)can_fail_malloc(sizeof(int)*size);
   int *low = (int*)can_fail_malloc(sizeof(int)*size);
   int *brknRouters = (int*)can_fail_malloc(sizeof(int)*size);
   for(i=0;i<size;i++){
        visitado[i]=0;
        pai[i]=-1;
        brknRouters[i]=0;
...
int main(int argc, char const *argv[]){
   ...
   int *cc = (int*)can_fail_malloc(sizeof(int)*N); /*array para as componentes ligadas*/
   int numCC = NETWORKcompsligadas(G,cc); /*numCC = numero de componentes ligadas*/
   printf("%d\n", numCC);
   int *subnetworks = (int*)can_fail_malloc(sizeof(int)*numCC); /*array de identificadores*/
