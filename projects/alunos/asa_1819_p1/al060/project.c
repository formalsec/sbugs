...
int main(){
	...
	Network *n = can_fail_malloc(sizeof(Network));
...
void createNetwork(int n_routers, int n_ligacoes, Network *n){
	...
	n->routers = can_fail_malloc(n_routers * sizeof(Router));
	n->num_routers = n_routers;
	n->num_ligacoes = n_ligacoes;
	Router *tmp_r = can_fail_malloc(sizeof(Router));
	
	for (i=0; i < n_routers;i++){
		
		tmp_r->id = i + 1;
		tmp_r->l_size = 0;
		tmp_r->ligacoes = can_fail_malloc(n_routers * sizeof(int));
		tmp_r->parent = GREY;
        tmp_r->weak = WHITE;
		n->routers[i] = *tmp_r;
...
void stronglyConnectedRouters(Network *n){
	...
	int i, scc = 1, max, j = 0;
	int *scc_id = can_fail_malloc(scc * sizeof(int));
	...
	for (i = 0 ; i < n->num_routers ; i++){
		if (n->routers[i].state != BLACK){
			max = 0;
			scc_id = can_fail_realloc(scc_id, scc * sizeof(int));
			dfs(i, n, &max, &j);
			scc_id[scc - 1] = max;
			scc++;
		}
