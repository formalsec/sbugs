...
int subnetsnumber(int routers,int** matriz,Item* x){
	int subnetnumber=1;
	int i;
	int *nofwhiterouters=(int*) can_fail_malloc (sizeof(int));
	*nofwhiterouters=routers;
	...
	for(i=0;i<routers;i++){
		if(wrouters[i]==white){
			Item* correntitem=(Item*)can_fail_malloc(sizeof(Item));
...
int subnetmissingall(int routers,int** matriz,int NdeOFFrouters,int* listTOoff){
	...
	*nofwhiterouters=routers-NdeOFFrouters;
	int *routerspercorridos=(int*) can_fail_malloc (sizeof(int));
	for(i=0;i<routers;i++){
...
int main(){
	...
	Item* x=(Item*) can_fail_malloc (sizeof(Item));
	*x=criaitem(0);
	/*lê e da as primeiras informacoes do stdin;*/
	scanf("%d\n",&routers);
	scanf("%d\n",&connectors);
	/*cria a matriz (limpa) a ser usada*/
	matriz=(int**) can_fail_malloc (routers*sizeof(int*));
	for(i=0;i<routers;i++){
		matriz[i]=(int*) can_fail_malloc (sizeof(int));
		matriz[i][0]=1;
	}
	/*prenche a matriz*/
	for(i=0;i<connectors;i++){
		scanf("%d %d\n",&con1,&con2);
		matriz[con1-1][0]++;
		matriz[con1-1]=(int*) can_fail_realloc (matriz[con1-1],sizeof(int)*matriz[con1-1][0]);
		matriz[con1-1][matriz[con1-1][0]-1]=con2;
		matriz[con2-1][0]++;
		matriz[con2-1]=(int*) can_fail_realloc (matriz[con2-1],sizeof(int)*matriz[con2-1][0]);
