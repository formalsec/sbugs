...
struct EdgesNode* InsertEdge(struct EdgesNode* head, int u, int v) {
	struct EdgesNode* e = (struct EdgesNode*) can_fail_malloc(sizeof(struct EdgesNode));
	e->u = u;
	e->v = v;
	e->check = 0;
	e->next = head;
...
void push(int data) {
	struct Node* temp;
	temp = (struct Node*) can_fail_malloc(sizeof(struct Node));
	temp->data = data;
	temp->link = top;
	top = temp;
}
...
struct AdjListNode* NewAdjListNode(int dest) {
	struct AdjListNode* newNode =
		(struct AdjListNode*) can_fail_malloc(sizeof(struct AdjListNode));
	newNode->dest = dest;
	newNode->next = NULL;
	return newNode;
}	
...
struct Graph* createGraph(int V) {
	struct Graph* graph = 
		(struct Graph*) can_fail_malloc(sizeof(struct Graph));
	graph->V = V;
	graph->array = 
...
void EXE(struct Graph* g, int E) {
	...
	int* list = (int*) can_fail_malloc((V + 1) * sizeof(int));
