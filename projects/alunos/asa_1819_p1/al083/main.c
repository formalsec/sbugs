...
struct router* criarRouter(int id){
	struct router* novoRouter = can_fail_malloc(sizeof(struct router));
	novoRouter->id = id;
...
int* criarVetor(int vertices){
	int* novoVetor = can_fail_malloc(vertices*sizeof(int));
	int i;
...
struct Rede* criarRede(int vertices){
	struct Rede* novaRede = can_fail_malloc(sizeof(struct Rede));
	novaRede->numRouters = vertices;
	novaRede->lista_adj = can_fail_malloc(vertices * sizeof(struct router*));
