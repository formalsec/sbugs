...
graph create_graph( int N ) {
    ...
    G->N = N;  
    G->adj = can_fail_malloc(N * sizeof(link));   
    for (v = 0; v < N; v++) {  
        G->adj[v] = NULL;
    }   
    return G;
...
int main() {
    ...
    scanf("%d", &N);
    int * visited = (int*)can_fail_malloc(N * sizeof(int));
    memset(visited, 0, N * sizeof(visited[0]));
    int * low = (int*)can_fail_malloc(N * sizeof(int));
    memset(low, -1, N * sizeof(low[0]));
    int * disc = (int*)can_fail_malloc(N * sizeof(int));
    memset(disc, 0, N * sizeof(disc[0]));
    int * parent = (int*)can_fail_malloc(N * sizeof(int));
    memset(parent, -1, N * sizeof(parent[0]));
    int * articulation = (int*)can_fail_malloc(N * sizeof(int));
