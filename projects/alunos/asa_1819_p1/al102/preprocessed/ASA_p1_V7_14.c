...
void readInput(Rede r)
{
  ...
  int u;
  int v;
  M_ligacoes = new_sym_var(sizeof(int) * 8);
...
void buscaProfundidadeRede(Rede r, int router)
{
  ...
  for (l = r->adjacencias[router - 1]; l != 0; l = l->prox)
  {
    if (r->adjacencias[l->id - 1]->visitado == 0)
    {
      ...
      r->adjacencias[router - 1]->menorTempoDesc = minimo(r->adjacencias[router - 1]->menorTempoDesc, r->adjacencias[l->id - 1]->menorTempoDesc);
      ...
      {
        if ((r->adjacencias[router - 1]->routerPai != 0) && (r->adjacencias[l->id - 1]->menorTempoDesc >= r->adjacencias[router - 1]->tempoDesc))
        {
          r->adjacencias[router - 1]->C_Router = 1;
        }
        else
    ...
    {
      if (l->id != r->adjacencias[router - 1]->routerPai)
      {
        r->adjacencias[router - 1]->menorTempoDesc = minimo(r->adjacencias[router - 1]->menorTempoDesc, r->adjacencias[l->id - 1]->tempoDesc);
      }
      else
      {
...
void printOutput(Rede r)
{
  ...
  for (router = 0; router < N_routers; router++)
  {
    for (l = r->adjacencias[router]; l != 0; l = l->prox)
    {
      l->visitado = 0;
    }
