...
int findBiggestSubgraph(List* graph, int numVertices) {
	...
	int* visited = (int*) can_fail_malloc(sizeof(int)*(numVertices));		/* 0 = not visited, 1 = visited */
...
void performAlgorithm(List* graph, int numVertices) {
	...
	/* DECLARATION */	
	int* d = (int*) can_fail_malloc(sizeof(int)*(numVertices));		/* d - discovery time */
	int* low = (int*) can_fail_malloc(sizeof(int)*(numVertices));
	int numSubGraphs = 0;
	...
	/* SORTING SUBGRAFS ID */
   	int* subAux = (int*) can_fail_malloc(sizeof(int)*numSubGraphs);		/* subAux array is used to sort the ids */
...
int main(int argc, char* argv[]) {
	...
	List* network = (List*) can_fail_malloc(sizeof(List)*(numRouters));
