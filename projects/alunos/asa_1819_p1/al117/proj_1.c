...
struct Graph* createGraph(int num_nodes){
	...
	graph -> adjLists = can_fail_malloc((1+num_nodes)*sizeof(struct node*));
   	graph -> visited = can_fail_malloc((1+num_nodes) * sizeof(int));
   	graph -> parent = can_fail_malloc((1+num_nodes) * sizeof(int));
   	graph -> articulationPoints = can_fail_malloc((1+num_nodes) * sizeof(int));
   	graph -> subtree_ids = can_fail_malloc((1+num_nodes) * sizeof(int));
   	graph -> low = can_fail_malloc((1+num_nodes) * sizeof(int));
   	graph -> disc = can_fail_malloc((1+num_nodes) * sizeof(int));
