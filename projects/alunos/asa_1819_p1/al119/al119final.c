...
struct vertice* cria_vertice(int v)
{
    struct vertice* temp_vertice = can_fail_malloc(sizeof(struct vertice));
...
{
    struct grafo* rede = can_fail_malloc(sizeof(struct grafo));
    rede->numRouters = vertices;

    rede->matriz = can_fail_malloc(vertices * sizeof(struct vertice*));

    rede->visitado = can_fail_malloc(vertices * sizeof(int));
