...
void newLigacao(rede redeC, int r1, int r2){
	ligacao lig_r1 = (ligacao) can_fail_malloc(sizeof(struct Ligacao));
	ligacao lig_r2 = (ligacao) can_fail_malloc(sizeof(struct Ligacao));
...
router newRouter(int id){
	router new = (router) can_fail_malloc(sizeof(struct Router));
	new->id = id;
	new->d = INT_MAX;
	new->low = INT_MAX;
	new->crit = 0;
...
rede newRede(int n_routers){
	rede new = (rede) can_fail_malloc(sizeof(struct Rede));
	new->routers = (router*) can_fail_malloc(sizeof(router) * n_routers);
	new->crit_routers = 0;
...
int main(int argc,char *argv[]){
	...
	sub_redes_rep = (int*) can_fail_malloc(sizeof(int) * RedeCaracol->sub_redes);
