...
void createDataAux(Data data, int vertex_1, int vertex_2) {
	Connection current;
	if ((current = (Connection) can_fail_malloc(sizeof(struct connection))) == NULL) {
		perror("Error in can_fail_malloc in createDataAux (current)");
		exit(EXIT_FAILURE);
	}
...
Data createData() {
    Data data;
	if ((data = (Data) can_fail_malloc(sizeof(struct data))) == NULL) {
    	perror("Error in can_fail_malloc in createData (data)");
      	exit(EXIT_FAILURE);
    }
	...
	data->nr_vertexs_bigger_graph = 0;
   	if ((data->vertexs = (Vertexs) can_fail_malloc(data->nr_vertexs * sizeof(struct vertexs))) == NULL) {
   		perror("Error in can_fail_malloc in createData (data->vertexs)");
   		exit(EXIT_FAILURE);
   	}
	int i;
   	for (i = 0; i < data->nr_vertexs; i++) {
