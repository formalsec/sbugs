...
int main(){
	...
	r=(router*)can_fail_malloc(sizeof(router)*N);
	s=(int*)can_fail_malloc(sizeof(int)*N);
	a=(int*)can_fail_malloc(sizeof(int)*N);
	b=(int*)can_fail_malloc(sizeof(int)*N);
	for(z=0;z<N;z++)
		s[z]=a[z]=b[z]=0;
	for(y=1;y<N+1;y++){
		int* p=(int*)can_fail_malloc(sizeof(int)*M);/*Vetor com as ligações diretas do router para depois inserir em r*/
		w=0;
		for(z=0;z<M;z++){
