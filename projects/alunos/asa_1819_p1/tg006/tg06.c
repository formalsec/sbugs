...
int main() {
	...
	grid general = (grid) can_fail_malloc(sizeof(struct general));
	...
	general->time = 0;
	general->nArtic = 0;
	general->extras = (dfs*) can_fail_malloc((sizeof(dfs)*general->nRouters));
	general->adj = (link *) can_fail_malloc((sizeof(link)*general->nRouters));

	for(; i<general->nRouters;i++) {
		general->extras[i] = (dfs) can_fail_malloc(sizeof(struct dfs_ex));
...
link insertBegin(link head, int i) {
	link newEl = can_fail_malloc(sizeof(struct node));
