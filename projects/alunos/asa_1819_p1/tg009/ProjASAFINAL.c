...
struct AdjListVertice* newAdjListVertice(int vertice) {
    ...
    struct AdjListVertice* new = (struct AdjListVertice*)can_fail_malloc(sizeof(struct AdjListVertice));
...
struct Grafo* criaGrafo(int v, int t) {
    ...
    struct Grafo* Grafo = (struct Grafo*)can_fail_malloc(sizeof(struct Grafo));
    Grafo -> array = (struct AdjListHead*)can_fail_malloc(sizeof(struct AdjListHead)*v);
    Grafo -> visited = (int*) can_fail_malloc(sizeof(int) * v);
    Grafo -> ap = (int*) can_fail_malloc(sizeof(int) * v);
    Grafo -> numVertices = v;
    Grafo -> numLigacoes = t;
    Grafo -> numAP = 0;
    Grafo -> numMaxSub = 0;
...
{
    int i;
    int* des = (int*) can_fail_malloc(sizeof(int) * grafo->numVertices);
    int* visitado = (int*) can_fail_malloc(sizeof(int) * grafo->numVertices);
    int* pai = (int*) can_fail_malloc(sizeof(int) * grafo->numVertices);
    int* low = (int*) can_fail_malloc(sizeof(int) * grafo->numVertices);
...
int main(int argc, const char * argv[]) {
    ...
    for (i = 0; i < nVertices; i++) {
        if (newGrafo -> visited[i] == 0) {
            contSub++;
            newGrafo -> vertMaxArray = can_fail_realloc(newGrafo->vertMaxArray, contSub*(sizeof(int)));
