...
Audit initAudit(Graph G)
{
	Audit new = (Audit) can_fail_malloc(sizeof(struct audit));

	new->numSubNets = 0;
	new->netID = 0;
	new->subNetIDs = (int*) can_fail_malloc(sizeof(int) * G->V);
	new->numCutV = 0;
	new->cutV = (char*) can_fail_malloc(sizeof(char) * G->V);
	new->maxNetSize = 0;
	return new;
}
...
Graph initG(int V)
{
	...
	new->V = V;
	new->E = 0;
	new->adjacencies = (Link*) can_fail_malloc(sizeof(Link) * V);
...
void doTarjanSearchG(Graph G, Audit output)
{
	char* color = (char*) can_fail_malloc(sizeof(char) * G->V); /* Vertex visit states */
	int* d = (int*) can_fail_malloc(sizeof(int) * G->V);        /* Discovery times */
	char* root = (char*) can_fail_malloc(sizeof(char) * G->V);  /* Indicates if a vertex is a root (0 if it is) */
	int* low = (int*) can_fail_malloc(sizeof(int) * G->V);      /* Lowest d[u] found by a vertex during DFS */
