...
int main ()
{
	...
	nodes = (connection*)can_fail_malloc(sizeof(connection) * numNodes);
	for (i = 0; i < numNodes; i++)
	{
		nodes[i].n = 0;
		nodes[i].next = (int*) can_fail_malloc (sizeof(int) * 0);
	}
	...
	while (index < numConnec)
	{
		...
		nodes[origin].next = (int*) can_fail_realloc (nodes[origin].next, sizeof(int) * originConnections);
		nodes[origin].next[originConnections - 1] = dest;
		int destConnections = ++(nodes[dest].n);
		nodes[dest].next = (int*) can_fail_realloc (nodes[dest].next, sizeof(int) * destConnections);
		nodes[dest].next[destConnections - 1] = origin;
		index++;
	}
	
	bool* visited = (bool*) can_fail_malloc (sizeof(bool) * (numNodes)); 
	int* disc = (int*) can_fail_malloc (sizeof(int) * (numNodes)); 
	int* low = (int*) can_fail_malloc (sizeof(int) * (numNodes)); 
	int* parent = (int*) can_fail_malloc (sizeof(int) * (numNodes)); 
	bool* artpoint = (bool*) can_fail_malloc(sizeof(bool) * (numNodes));
	int* subnetworks = (int*) can_fail_malloc(sizeof(int) * 0);
	...
	for (i = 0; i < numNodes; i++)
	{
		if (visited[i] == false) 
		{
			...
			subs++;
			subnetworks = can_fail_realloc(subnetworks, sizeof(int) * subs);
