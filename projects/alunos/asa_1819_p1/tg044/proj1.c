...
Grafo *initGraph(int V) {
  ...
  if(V > 0 && (grafo = (Grafo*)can_fail_malloc(sizeof(struct grafo))) != NULL) {
    grafo->V = V;
    grafo->E = 0;
    grafo->numeroDeArvores = 0;
    grafo->listaAdjacencias = (verticeDoGrafo**)can_fail_malloc(V * sizeof(struct node*));
  }  
  return grafo;
}
...
void adicionaPonteAoGrafo(Grafo *grafo, verticeId u, verticeId v) {
  if((u > 0 && u <= grafo->V) && (v > 0 && v <= grafo->V)) {
    verticeDoGrafo *w = (verticeDoGrafo*)can_fail_malloc(sizeof(struct node));
    verticeDoGrafo *x = (verticeDoGrafo*)can_fail_malloc(sizeof(struct node));
...
int main() {
  ...
  Grafo *grafo;
  lista = (int**)can_fail_malloc(5 * sizeof(int*));
  ...
  if(lista != NULL) {
    lista[TIME] = (int*)can_fail_malloc(sizeof(int));
    lista[DISCOVERY] = (int*)can_fail_malloc(grafo->V * sizeof(int));
    lista[LOW] = (int*)can_fail_malloc(grafo->V * sizeof(int));
    lista[ANTECESSOR] = (int*)can_fail_malloc(grafo->V * sizeof(int));
    lista[ARTICULATION] = (int*)can_fail_malloc(grafo->V * sizeof(int));
  }
