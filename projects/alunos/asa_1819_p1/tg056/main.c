...
void lerGrafo(){
  int i, r1, r2, numeroLigacoes;
	...
	Graph = can_fail_malloc(sizeof(node) * numeroRouters);
  maxSubrede = can_fail_malloc(sizeof(int) * numeroRouters);
...
void addRouter(int id){
	node *r;
	r = (node*)can_fail_malloc(sizeof(node));
  r->tempo_d = 0;
  r->pai = -1;
  r->p_art = 0;
	r->next = NULL;
...
void criaLigacao(int r1,int r2){
  ...
  else Graph[r1-1]->next = criaRouter(r2);
  ...
  else Graph[r2-1]->next = criaRouter(r1);
}
fakeNode *criaRouter(int id){
	fakeNode *r = (fakeNode*)can_fail_malloc(sizeof(fakeNode));
  r->id = id;
  r->next = NULL;
  return r;
}
