...
int pushRelabel(Graph G) {
	...
	int* h = (int*) can_fail_malloc(G->V * sizeof(int));
	int* e = (int*) can_fail_malloc(G->V * sizeof(int));
	...
	N = (link*) can_fail_malloc(G->V * sizeof(link));
	for (i = 0; i < G->V; i++)
		N[i] = NULL;
	for (i = 0; i < G->V; i++) {
	...
	markedVertices = (char*) can_fail_malloc(G->V * sizeof(char));
	for (i = 0; i < G->V; i++)
		markedVertices[i] = 0;
...
void initializePreFlow(Graph G, int* h, int* e) {
	...
	for (x = G->adj[0]; x != NULL; x = x->next) {
		int* vertex = (int*) can_fail_malloc(sizeof(int));
		Edge edge = (Edge)(x->data);
...
void getMinCut(Graph G) {
	...
	edgeArray = (Edge*) can_fail_malloc(nedges * sizeof(Edge));
	edgeArrayAux = (Edge*) can_fail_malloc(nedges * sizeof(Edge));
...
char* DFS(Graph GRT) {
	int i;
	char* color = (char*) can_fail_malloc(GRT->V * sizeof(char));
	for (i = 0; i < GRT->V; i++) {
		markedVertices[i] = 0;
		color[i] = WHITE;
	}
...
void graph_insertEdge(Graph G, int u, int v, int cap) {
	Edge data = (Edge) can_fail_malloc(sizeof(struct edge));
	data->u = u;
	data->v = v;
	data->flow = 0;
	data->capacity = cap;
