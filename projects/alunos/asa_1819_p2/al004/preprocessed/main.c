...
void merge(item *edges[], unsigned i, unsigned m, unsigned f)
{
  ...
  while ((n1 <= m) && (n2 <= f))
  {
    ...
    {
      if ((get_from_item(edges[n1]) == get_from_item(edges[n2])) && (get_to_item(edges[n1]) < get_to_item(edges[n2])))
      {
        aux[aux_n] = edges[n1];
        n1++;
      }
...
unsigned check_bounds(graph *g, int i1, int i2)
{
  if (i2 == (-1))
  {
    return (i1 < ((int) g->v_number)) && (g->_vertexs[i1] != 0);
  }
  else
  {
  ...
  return (((i1 < ((int) g->v_number)) && (i2 < ((int) g->v_number))) && (g->_vertexs[i1] != 0)) && (g->_vertexs[i1] != 0);
}
...
void reset_graph(graph *g)
{
  ...
  for (i = 0; i < g->v_number; i++)
  {
    if (g->_vertexs[i] != 0)
    {
      g->_vertexs[i]->_e = 0;
      g->_vertexs[i]->_h = 0;
      for (aux = g->_vertexs[i]->_edges; aux != 0; aux = aux->_n)
      {
...
void DFS_visit(graph *g, vertex *v)
{
  ...
  if (v->_edges != 0)
  {
    for (edge = v->_edges; edge != 0; edge = edge->_n)
    {
    ...
    for (edge = v->_edges_in; edge != 0; edge = edge->_n)
    {
...
void initialize_preflow(graph *g, unsigned s)
{
  ...
  for (aux = g->_vertexs[s]->_edges; aux != 0; aux = aux->_n)
  {
    aux->_i->_f = aux->_i->_c;
...
{
  list_node *aux;
  unsigned min_h = 32767;
  for (aux = v->_edges; aux != 0; aux = aux->_n)
  {
  ...
  for (aux = v->_edges_in; aux != 0; aux = aux->_n)
  {
...
void discharge(graph *g, vertex *u, unsigned *labels, list_node **head, list_node **tail, int s, int t)
{
  ...
  while (u->_e > 0)
  {
    if ((u->_current != 0) && (in_list == 0))
    {
      v = g->_vertexs[get_to_item(u->_current->_i)];
    }
    else
    {
      if ((u->_current != 0) && (in_list == 1))
      {
    ...
    {
      if ((u->_h == (v->_h + 1)) && ((get_residual_capacity_item(u->_current->_i) > 0) && (in_list == 0)))
      {
        push(g, u->_current->_i, 0, head, tail, s, t);
      }
      else
      {
        if (((u->_h == (v->_h + 1)) && (get_residual_capacity_inverse_item(u->_current->_i) > 0)) && (in_list == 1))
        {
...
void BFS(graph *g, unsigned s)
{
  ...
  while (fifo != 0)
  {
    ...
    it = pop_item(&fifo);
    for (edge = g->_vertexs[get_from_item(it)]->_edges; edge != 0; edge = edge->_n)
    {
    ...
    for (edge = g->_vertexs[get_from_item(it)]->_edges_in; edge != 0; edge = edge->_n)
    {
...
int main()
{
  ...
  i = 0;
  x = 3;
  f = new_sym_var(sizeof(unsigned) * 8);
  e = new_sym_var(sizeof(unsigned) * 8);
  t = new_sym_var(sizeof(unsigned) * 8);
  g = init_graph((f + (2 * e)) + 2);
  for (i = 0; i < f; i++)
  {
    x = 1;
    c = new_sym_var(sizeof(unsigned) * 8);
  ...
  {
    x = 3;
    o = new_sym_var(sizeof(unsigned) * 8);
    d = new_sym_var(sizeof(unsigned) * 8);
    c = new_sym_var(sizeof(unsigned) * 8);
  ...
  for (i = 0; i < g->v_number; i++)
  {
    for (edge = g->_vertexs[i]->_edges; edge != 0; edge = edge->_n)
    {
      if (((((g->_vertexs[i]->_visited == 1) && (g->_vertexs[get_to_item(edge->_i)]->_visited == 0)) && (get_residual_capacity_item(edge->_i) == 0)) || (((g->_vertexs[i]->_visited == 0) && (g->_vertexs[get_to_item(edge->_i)]->_visited == 1)) && (get_residual_capacity_item(edge->_i) == 0))) && (get_capacity_item(edge->_i) != 0))
      {
        ...
        else
        {
          ...
          {
            if (((g->_vertexs[i]->_id > (f + e)) && (g->_vertexs[i]->_id <= (f + (2 * e)))) && (get_capacity_item(edge->_i) != 0))
            {
              edge_count++;
            }
            else
  ...
  for (i = 0; i < edge_count; i++)
  {
    ...
    {
      if (((get_from_item(edges[i]) > (f + e)) && (get_from_item(edges[i]) <= (f + (2 * e)))) && (get_capacity_item(edges[i]) != 0))
      {
