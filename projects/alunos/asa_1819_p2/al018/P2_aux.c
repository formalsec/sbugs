...
lista * cria_lista (){
	lista * novo = (lista*)can_fail_malloc(sizeof(lista));
	novo->next = NULL;
	novo->last = novo;
	novo->peso = -1;
	return novo;
...
void inserir_inicio (lista * original, int inteiro, int peso){
	...
	lista * novo;
	novo = (lista*)can_fail_malloc(sizeof(lista));
	novo->valor = inteiro;
	novo->next = aux;
	original->next = novo;
...
ligas * cria_ligas(){
	ligas * novo = (ligas*)can_fail_malloc(sizeof(ligas));
	novo->next = NULL;
	return novo;
}
...
void Ford_Fulkerson(lista ** ligacoes) {
	...
	int *isVisited =(int*)can_fail_calloc(numero_vertices,sizeof(int));
...
int main () {
	...
	lista **ligacoes = can_fail_malloc(numero_vertices * sizeof(lista*));
	for (i=0; i< numero_vertices; i++){
