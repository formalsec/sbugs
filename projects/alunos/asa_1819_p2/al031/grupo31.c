...
{
	priorityNode newNode = (priorityNode) can_fail_malloc(sizeof(struct PriorityNode));
...
{
	node newNode = (node) can_fail_malloc(sizeof(struct Node));
...
link initLink(int u, int v, int capacity) {
	link l = (link) can_fail_malloc(sizeof(struct Link));
	l->u = u;
	l->v = v;
	l->fuv = capacity;
	l->fvu = 0;
...
graph graphInit(int vert, int minStation, int minAux, int offset) {
	...
	g->height = (int*) can_fail_malloc(vert * sizeof(int));
	g->e = (int*) can_fail_malloc(vert * sizeof(int));
	g->adj = (node*) can_fail_malloc(vert * sizeof(node));
	g->minDealer = 2;
...
void printCut(graph g)
{
	...
	graphDestroy(g);
	aux = (int*) can_fail_malloc(j * sizeof(int));
	aux2 = (int*) can_fail_malloc(j * sizeof(int));
