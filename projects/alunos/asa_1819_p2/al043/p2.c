...
neighbour_ptr newNeighbour(int num) {
    neighbour_ptr aux = can_fail_malloc(sizeof(struct neighbour));
    aux -> id = num;
    return aux;
}
...
int main() {
    ...
    scanf("%d %d %d", &f, &e, &t);
    int aux = f+e+2;
    node_ptr g = can_fail_malloc(aux * sizeof(struct node));
    int *pc = can_fail_malloc(aux * sizeof(int));
    int *height = can_fail_malloc(aux * sizeof(int));
    int *excess = can_fail_malloc(aux * sizeof(int));
    ...
    for (i=0; i<t; i++) {
        scanf("%d %d %d", &u, &v, &w);
        aux_edge = can_fail_malloc(sizeof(struct edge));
        aux_edge -> u = v;
        aux_edge -> v = u;
        aux_edge -> f = 0;
        aux_edge -> w = w;
