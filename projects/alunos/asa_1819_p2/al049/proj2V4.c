...
Vector vectorInit(long initSize) {
	...
	vectorPtr->size = 0;
	vectorPtr->elements = (void**) can_fail_malloc(sizeof(void*)*vectorPtr->capacity);
...
int vectorPush(Vector vectorPtr, void* element) {
	if (vectorPtr->size == vectorPtr->capacity) {
		void** tmp = (void**) can_fail_realloc(vectorPtr->elements, sizeof(void*)*2*vectorPtr->capacity);
		if (tmp == NULL)
			return 0;
...
List listInit() {
	List lst = (List) can_fail_malloc(sizeof(struct list));
	if (lst == NULL)
		return NULL;
	lst->size = 0;
	lst->head = NULL;
...
Edge edgeInit(int n, int flow) {
	Edge e = (Edge) can_fail_malloc(sizeof(struct edge));
	if (e == NULL)
		return NULL;
...
Graph graphInit(int suppl, int stations) {
	...
	g->vertices = (Vertex*) can_fail_malloc(sizeof(Vertex)*size);
	if (g->vertices == NULL) {
		free(g);
		return NULL;
	}
