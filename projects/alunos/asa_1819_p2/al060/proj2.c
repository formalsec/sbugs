...
void insert(Vertex *vertex, int dest, int weight){
	...
	adj = (Node*) can_fail_malloc(sizeof(Node));
	adj->dest = dest;
	adj->weight = weight;
	adj->flow = 0;
	adj->next = NULL;
...
void insertQ(Queue *queue, int id, Graph *g){
		...
		v = (QueueN*) can_fail_malloc(sizeof(QueueN));
		v->id = id;
		v->next = NULL;
		v->prev = NULL;
...
int getStr(){
	int i = 0, v;
	int c;
	char *tmp_str = can_fail_malloc(sizeof(char) * 1000);
...
int relabelToFront(Graph *g, int n_prov, int n_stations){
	...
	excess = (int*) can_fail_malloc(sizeof(int) * ((n_prov + 2 * n_stations) + 2));
	height = (int*) can_fail_malloc(sizeof(int) * ((n_prov + 2 * n_stations) + 2));
...
void findMinCut(Graph *g, int n_prov, int n_stations, int max_cap){
	...
	visited = (int*) can_fail_malloc(sizeof(int) * ((n_prov + 2 * n_stations) + 2));
	memset(visited, 0, (sizeof(int) * ((n_prov + 2 * n_stations) + 2)));
	stations = (int*) can_fail_malloc(sizeof(int));
	edges = (Edge*) can_fail_malloc(sizeof(Edge));
	dfs(g, 1, visited);
		...
		for(node = g->adjency_list[i].head; node != NULL; node = node->next){
			if(visited[i]  ==  !(visited[node->dest]) && node->flow && cut_cap + node->flow <= max_cap){
				if(node->dest > n_stations + n_prov + 1){
					j = j + 1;
					stations = can_fail_realloc(stations, j * sizeof(int));
					stations[j - 1] = node->dest;
					cut_cap = cut_cap + node->flow;
				}
				else if(i > n_stations + n_prov + 1){
					k = k + 1;
					edges = can_fail_realloc(edges, k * sizeof(Edge));
...
int main(){
	...
	g->adjency_list = (Vertex*) can_fail_malloc(sizeof(Vertex) * ((n_prov + 2 * n_stations) + 2));
