...
void cria_listlig(int local,Listlig* head){
    ...
    Listlig* v=(Listlig*) can_fail_malloc (sizeof(Listlig));
    v->local=local;
...
void cria_listligarch(int des,int ori,Listligarch* head){
    ...
    Listligarch* v=(Listligarch*) can_fail_malloc (sizeof(Listligarch));
    v->des=des;
    v->ori=ori;
...
struct Lig* cria_lig(int weight, int des, int flow,Lig* firstOfvector,int nlig){
    ...
    Lig* v=(Lig*) can_fail_malloc (sizeof(Lig));
    v->weight=weight;
    v->des=des;
    v->flow=flow;
    v->next=NULL;
...
void relabel_to_front(Lig* matriz,int* vnlig,int Nparagens,int* vectorflow, int* hights){
    ...
    int oldhight;
    Listlig* v;
    Listlig *current =(Listlig*) can_fail_malloc (sizeof(Listlig));
    Listlig *head= current;
    current->local=2;
 
    /*cria a Linked list*/
    for(i=1; i<Nparagens-2 ;i++){
        v=(Listlig*) can_fail_malloc (sizeof(Listlig));
        v->local=i+2;
        current->next= v;
        current= current->next;
    }
...
int main()
{
    ...
    Lig* u;
    Lig* v;
    Listlig* estacoes=(Listlig*) can_fail_malloc (sizeof(Listlig));
    Listligarch* archos=(Listligarch*) can_fail_malloc (sizeof(Listligarch));
    vnlig=(int*) can_fail_malloc (Nparagens*sizeof(int));
