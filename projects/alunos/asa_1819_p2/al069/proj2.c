...
{
	Queue *x = (Queue*)can_fail_malloc(sizeof(struct queue));
...
int main(){
	...
	if(scanf("%d %d %d", &nfornecedores, &nabastecimentos, &nligacoes))
	{
		if( (nfornecedores > 0) && (nabastecimentos >= 0) && (nligacoes >= 0))
		{
			...
			grafo = (Vertice *)can_fail_malloc(sizeof(Vertice)*(nfornecedores+2*nabastecimentos+2));
			infila = (int *)can_fail_malloc(sizeof(int)*(nfornecedores+2*nabastecimentos+2));
		...
		printf("%d\n",fluxomaximo);
		estacoesaumenta = (int *)can_fail_malloc(sizeof(int)*(nabastecimentos));
		arestas = (Fila*)can_fail_malloc(sizeof(Fila)*(nabastecimentos+nfornecedores+2));
