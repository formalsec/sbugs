...
struct node2* new_node2(int v){
  struct node2* new_node=(struct node2*) can_fail_malloc(sizeof(struct node2));
  new_node->number=v;
  new_node->next=NULL;
  return new_node;
}
...
struct edge* create_edge(int u, int v){
	struct edge* new_edge=(struct edge*) can_fail_malloc(sizeof(struct edge));
	new_edge->o=u;
	new_edge->d=v;
	return new_edge;
}
...
void save_edge(int u, int v){
	if(u!=0){
		...
		if(size_arestas_aumentadas<=contador){
			...
			arestas_aumentadas= (struct edge**) can_fail_realloc((void*)arestas_aumentadas,(size_arestas_aumentadas*(sizeof(struct edge*))));
		}
...
void merge(struct edge* vetor[], int comeco, int meio, int fim) {
    ...
    struct edge* *vetAux;
    vetAux = (struct edge**)can_fail_malloc(tam * sizeof(struct edge));
...
int main(){
	FIFO.head=NULL;
	FIFO.tail=NULL;
	arestas_aumentadas=(struct edge**) can_fail_malloc(sizeof(struct edge*));
	read_input();
	/*printGraph();*/
