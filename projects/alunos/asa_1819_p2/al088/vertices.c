...
edgesLink new_edgeLink(long int source, long int destiny){
  edgesLink t=can_fail_malloc(sizeof(struct edges));
  t->source=source;
  t->destiny=destiny;
  t->next=NULL;
  return t;
...
neighboursLink new_edge(long int indice, long int capacity,long int source){
  neighboursLink t= can_fail_malloc(sizeof(struct neighbours));
  t->destiny=indice;
  t->capacity=capacity;
  t->flow=0;
  t->source=source;
...
int main(){
  ...
  info* verticesInfo=(info*)can_fail_malloc(sizeof(info) * (2+numProviders+2*numStations));
