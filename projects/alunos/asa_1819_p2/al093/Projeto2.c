...
void createGraph() {
  ...
  net = (struct Net*) can_fail_malloc(sizeof(struct Net));
  ...
  net->array = (struct AdjList*)can_fail_malloc((net->nodesNumber + 1)*sizeof(struct AdjList));
...
void relabelToFront() {
  ...
  list = (int*)can_fail_malloc((net->nodesNumber-1)*sizeof(int));
  int i;
...
void addLL(int src, int dest, struct nodeSet** set, int mode) {
  struct nodeSet *newNode = (struct nodeSet*)can_fail_malloc(sizeof(struct nodeSet));
...
void initializeOutput() {
  output = (struct output*)can_fail_malloc(sizeof(struct output));
