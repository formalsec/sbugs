...
void Relabel(Lista l, int vertice)
{
  ...
  for (t = l[vertice].transp; t != 0; t = t->prox)
  {
...
void Push(Lista l, int vertice1, int vertice2)
{
  ...
  Transportes aux;
  for (aux = transp1; aux != 0; aux = aux->prox)
  {
    if (aux->d == vertice2)
    {
  ...
  for (aux = transp2; aux != 0; aux = aux->prox)
  {
    if (aux->d == vertice1)
    {
      aux->fluxo -= aumento;
...
void Discharge(Lista l, int vertice)
{
  ...
  while (l[vertice].fluxoExcesso > 0)
  {
    ...
    {
      if ((((transp->c - transp->fluxo) > 0) && (l[vertice].altura == (l[transp->d].altura + 1))) && (((l[transp->d].cap - l[transp->d].fluxoExcesso) > 0) || (l[transp->d].cap == (-1))))
      {
...
void buscaProfundidade(Lista l, int visitados[], int vertice, int f, int estacoesAumentar[], int ligacoesAumentar[][2])
{
  Transportes transp;
  visitados[vertice] = 1;
  for (transp = l[vertice].transp; transp != 0; transp = transp->prox)
  {
    if ((transp->fluxo <= 0) && (visitados[transp->d] == 0))
    {
      if (transp->d >= (f + 2))
      {
        ...
        {
          if (l[transp->d].cap == (-transp->fluxo))
          {
