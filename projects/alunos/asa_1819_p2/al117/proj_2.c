...
struct Graph* createGraph(int num_supliers, int num_stations, int num_edges){
	...
	graph -> nodesList = can_fail_malloc((graph->num_nodes) * sizeof(struct Node));
	graph -> edgeList = can_fail_malloc((graph->num_nodes) * sizeof(struct Edge));
	graph -> bfs_parent = can_fail_malloc((graph->num_nodes) * sizeof(struct Node));
	graph -> bfs_level = can_fail_malloc((graph->num_nodes) * sizeof(struct Node));
	graph -> dfs_visited = can_fail_malloc((graph->num_nodes) * sizeof(int));
	graph -> mincut_visited = can_fail_malloc((graph->num_nodes) * sizeof(int));
	graph -> edges_increment = can_fail_malloc((graph->num_edges) * sizeof(struct Edge));
	graph -> nodes_increment = can_fail_malloc((graph->num_nodes) * sizeof(int));
	for(i=0; i<graph->num_nodes; i++){
...
struct Edge* createEdge(int o, int d, int capacity){
	struct Edge* edge= can_fail_malloc(sizeof(struct Edge));
	edge -> o = o;
	edge -> d = d;
	edge -> flow = 0;
...
int bfs(struct Graph *graph, int s, int t){
	struct Edge *edge_aux = can_fail_malloc(sizeof(struct Edge));
...
int sendFlow(int s, int t, int flow, struct Graph *graph){
	struct Edge *edge_aux = can_fail_malloc(sizeof(struct Edge));
	struct Edge *edge_aux_2 = can_fail_malloc(sizeof(struct Edge));
	if(s == t){
		return flow;
	}
