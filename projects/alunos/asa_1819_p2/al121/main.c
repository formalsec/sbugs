...
void addEdge(int u, int v, int cap){ /*Adds both uv and vu*/
    Edge *edge=can_fail_malloc(sizeof(Edge)); 
    Edge *inverse=can_fail_malloc(sizeof(Edge)); 
...
int main(){
    ...
    graph=can_fail_malloc(nr_vertex*sizeof(Vertex)); 
    for(i=0;i<nr_vertex;i++){
        graph[i].adj=NULL;
        graph[i].id=i;
        graph[i].h=0;
    ...
    int *minCut=can_fail_malloc(nr_vertex*sizeof(int)); /*to Sort edges*/
