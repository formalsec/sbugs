...
int createNetwork(){
  network = (Vertex*) can_fail_malloc(nVert*sizeof(Vertex));
  int i;
  for(i=0;i<nVert;i++){
    network[i].id=i+1;
    network[i].prod=0;
...
int queue(int n, Queue* q){
  Queue* a = (Queue*) can_fail_malloc (sizeof(Queue));
  a->n=n;
  a->next=NULL;
  Queue* current=q;
  while(current->next!=NULL){
...
Queue* getPath(){
  ...
  Vertex v = network[0];
  while(v.parent!=-1){
    Queue* p = (Queue*) can_fail_malloc(sizeof(Queue));
    p->n = v.id-1;
    p->next = path;
    path=p;
    v=network[v.parent];
...
int minimalCut(){
  network2 = (Vertex*) can_fail_malloc(nVert*sizeof(Vertex));
  int i;
  for(i=0;i<nVert;i++){
    network2[i].id=i+1;
    network2[i].prod=0;
      ...
      if(network2[i].parent == -1 && !network2[i].bigger){
        ...
        while(adj!=NULL){
          if(network2[adj->dest].parent >= 0 && network2[adj->dest].bigger==0){
            MinEdge* m = can_fail_malloc (sizeof(MinEdge));
...
void BFS2(){
  ...
  q->n = 0;
  q->next = NULL;
  visited2 = (Queue*) can_fail_malloc (sizeof(Queue));
  visited2->n=0;
  visited2->next=NULL;
  network2[0].parent = 0;
  while(q!=NULL){
    ...
    while(adj!=NULL){
      if(network2[adj->dest].parent==-1 && adj->cap>0){
        ...
        if(adj->dest>nFor && network[adj->dest].cap-network[adj->dest].flow==0){
          ...
          if(bigger==NULL){
            bigger = (Queue*) can_fail_malloc (sizeof(Queue));
            bigger->n = adj->dest;
            bigger->next = NULL;
          }
          else{
