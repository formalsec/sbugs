...
void addToQueue(queue *queue, unsigned long value){
	v_link *toAdd = (v_link*) can_fail_malloc(sizeof(v_link));
	toAdd->id = value;
	toAdd->next = NULL;
	if(queue->start == NULL){
		queue->start = toAdd;
...
void associateEdge(vertice *vertices, edge *edge){
	link *link1 = (link*) can_fail_malloc(sizeof(link));
	link *link2 = (link*) can_fail_malloc(sizeof(link));
	link1->edge = edge;
	link2->edge = edge;
	link1->next = NULL;
	link2->next = NULL;
...
void queuedPushRelabelStrat(vertice *vertices, unsigned long numVertices){
	unsigned long i, u;
	queue *overflown = (queue*) can_fail_malloc(sizeof(queue));
...
int *verticeSplitter(vertice *vertices, unsigned long verticeNum){
	unsigned long i;
	int *isOnFarSide = (int*) can_fail_malloc(sizeof(int)*verticeNum);
...
void mergeSource(edge *edges, unsigned long start, unsigned long mid, unsigned long end){
	...
	unsigned long i, j, k;
	edge *left = (edge*) can_fail_malloc(sizeof(edge)*leftNum);
	edge *right = (edge*) can_fail_malloc(sizeof(edge)*rightNum);
	for (i = 0; i < leftNum; i++)
...
int main(){
	...
	*/
	...
	vertice *vertices=(vertice*) can_fail_malloc(sizeof(vertice)*verticeNum);
	edge *edges=(edge*) can_fail_malloc(sizeof(edge)*edgeNum);
	...
	*/
	...
	edge *edges_copy=(edge*) can_fail_malloc(sizeof(edge)*edgeNum);
	for(i=0; i<edgeNum; i++){
