...
Queue_t queue_alloc() {
	Queue_t q = (Queue_t) can_fail_malloc(sizeof(struct Queue));
...
Link_t new_node(int data, Link_t next) {
	Link_t x = (Link_t) can_fail_malloc(sizeof(struct Link));
...
Edge_t newEdge(int from_node, int to_node, long capacity) {
	Edge_t e = (Edge_t) can_fail_malloc(sizeof(struct Edge));
...
Vector_t vector_alloc(int init_capacity) {
	Vector_t v = (Vector_t) can_fail_malloc(sizeof(struct Vector));
	int capacity = MAX(init_capacity, 1);

	v->size = 0;
	v->capacity = capacity;
	v->elements = (Edge_t*) can_fail_malloc(capacity * sizeof(Edge_t));
...
void vector_pushBack(Vector_t vector, Edge_t data) {
	if (vector->size == vector->capacity) {
		...
		Edge_t *new_elements = (Edge_t*) can_fail_malloc(new_capacity * sizeof(Edge_t));
...
Graph_t graph_alloc(int num_nodes) {
	int i;
	Graph_t g = (Graph_t) can_fail_malloc(sizeof(struct Graph));

	g->num_nodes = num_nodes;
	g->max_flow = 0;
	g->adj_list = (Vector_t*) can_fail_malloc(num_nodes * sizeof(Vector_t));
...
void dinic(Graph_t graph) {
	...
	int *next = (int*) can_fail_malloc(n * sizeof(int));
	int *level = (int*) can_fail_malloc(n * sizeof(int));
	long flow;
...
void min_cut(Graph_t graph, int out_min) {
	int i, size, n = graph->num_nodes, node;
	bool *can_reach = (bool*) can_fail_malloc(n * sizeof(bool));
	bool *visited = (bool*) can_fail_malloc(n * sizeof(bool));
	Queue_t queue = queue_alloc();
	Edge_t edge, e, r;
