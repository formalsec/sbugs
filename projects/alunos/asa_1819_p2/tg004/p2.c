...
node* readInput(node *graph, list_edge *edgeList, int* edges, int* link, int* suppliers, int* processors, int* firstPostId, int* originalNodes, int* totalNodes) {
	...
	*totalNodes = *originalNodes + *processors + 1; /* Add 1 for source and one for each processing post */
	...
	graph = (node*) can_fail_malloc(sizeof(node)*(*totalNodes));
...
lst_iitem_edge* createEdge(int source, int destination, int capacity) {
	...
	newEdge = (lst_iitem_edge*) can_fail_malloc(sizeof(lst_iitem_edge));
	newEdge->source = source;
	newEdge->destination = destination;
	newEdge->capacity = capacity;
	newEdge->flow = 0;
	newEdge->reverse = (lst_iitem_edge*) can_fail_malloc(sizeof(lst_iitem_edge));
...
void edmondsKarp(node* nodes, int totalNodes) {
	...
	parents=(lst_iitem_edge**)can_fail_malloc(sizeof(lst_iitem_edge*)*totalNodes);
	maxFlow = 0;
	curr = -1;
...
list_edge* lst_new() {
	list_edge *list;
	list = (list_edge*) can_fail_malloc(sizeof(list_edge));
	list->first = NULL;
	return list;
}
