...
int main() {
    ...
    ************************/
    ...
    /* Minimum cut */
    cutLinks = (edge *)can_fail_malloc(sizeof(edge) * nLinks);
    assert(cutLinks);
...
eNode createENode(edge item, eNode next) {
    ...
    eNode new = (eNode)can_fail_malloc(sizeof(struct eNode));
    assert(new);
...
vNode createVNode(vertex item, vNode next) {
    ...
    vNode new = (vNode)can_fail_malloc(sizeof(struct vNode));
    assert(new);
...
void vListInit(int nVerts) {
    ...
    int i;
    vList = (vertex)can_fail_malloc(sizeof(struct vertex) * nVerts);
    assert(vList);
...
void stackInit(int nHeights) {
    ...
    stack.list = (vNode *)can_fail_malloc(sizeof(vNode) * (nHeights + 1));
    assert(stack.list);
