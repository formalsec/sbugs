...
int main(){
    ...
    graph grid = (graph) can_fail_calloc(1,sizeof(struct grid));
    ...
    grid->vertex = (vert *) can_fail_calloc(grid->nVertices, sizeof(vert));         /* Memory reservation for all vertex's.*/
    grid->heights = (link_v *) can_fail_calloc(grid->nVertices+1,sizeof(link_v));   /* Memory reservation for all verheights.*/
...
void minimumCut(link_v gapVertex) {
    coord minCutConect = (coord) can_fail_malloc(10*sizeof(struct cord));
    int * minCutGas = (int*) can_fail_malloc(10*sizeof(int));
        ...
        while(gapEdges != NULL) {
            ...
            if(gap->v->height > (gapEdges->e->out->height + 1) && gapEdges->e->capacity != 0 && gapEdges->e->out->id != 0) {
                ...
                if(nConections == mallConections-1) {
                    mallConections *= 2; 
                    minCutConect = (coord) can_fail_realloc(minCutConect,sizeof(struct cord)*mallConections);
        
                } 
                if (nGas == mallGas-1) {
                    mallGas *= 2; 
                    minCutGas = (int*) can_fail_realloc(minCutGas,sizeof(int)*mallGas);
                }
...
edge create_edge(vert v, int capacity, int flow) {
    edge e = (edge)can_fail_calloc(1,sizeof(struct e));
    e->capacity = capacity;
    e->out = v;
    e->flow = flow;
    return e;
...
vert create_vertex(int id, int height) {
    vert v = (vert)can_fail_calloc(1,sizeof(struct v));
    v->excess = 0;
    v->height = height;
    v->id = id;
    v->adj = NULL;
...
list_v create_list_v() {
    list_v i = (list_v) can_fail_calloc(1, sizeof(struct slist_v));
    i->head = i->tail = NULL;
    return i;
}
...
void insertEndV(list_v lis, vert v){
    link_v newEl = (link_v)can_fail_calloc(1,sizeof(struct node_vert));
    newEl->v = v;
        newEl->next = NULL;
...
link_v insertBeginV(link_v head, vert v) {
	link_v newEl = can_fail_calloc(1,sizeof(struct node_vert));
...
list_e create_list_e() {
    list_e i = (list_e) can_fail_calloc(1,sizeof(struct slist_e));
    i->head = i->tail = NULL;
    return i;
}
...
void insertBeginE(list_e lis, edge e) {
	link_e newEl = (link_e)can_fail_calloc(1,sizeof(struct node_adj));
	newEl->e = e;
