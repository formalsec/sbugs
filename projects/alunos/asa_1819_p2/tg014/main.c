...
void push(Queue *q, int element){
	Item *help;
	Item *newItem = can_fail_malloc(sizeof(struct item));
...
void addEdge(int origin, int dest, int capacity){
	...
	Edge *newEdge = can_fail_malloc(sizeof(struct edge));
	Edge *residualEdge = can_fail_malloc(sizeof(struct edge));
...
void parseGraph(){
	...
	G = (Node*)can_fail_malloc(sizeof(Node)*(size + refillStations + 2)); /* with source & target & ignoring 0 (2) */
...
void addCutEdge(int from, int to){
	...
	if (indexCutEdge % SIZE > 18)
		cutEdges = can_fail_realloc(cutEdges, sizeof(struct cutEdge)*sizeCutEdgesArray);
}
...
void findMinCut(){
	...
	visited = can_fail_malloc(sizeof(int)*(size));
	criticalStation = can_fail_malloc(sizeof(int)*(size+2));
	cutEdges = can_fail_malloc(sizeof(struct cutEdge)*(SIZE));
