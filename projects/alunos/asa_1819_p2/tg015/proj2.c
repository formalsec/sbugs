...
NetAudit initNetAudit(Graph G)
{
	NetAudit new = (NetAudit) can_fail_malloc(sizeof(struct audit));

	new->maxFlow = 0;
	new->minCutS = (char*) can_fail_calloc(G->V, sizeof(char));
	new->minCutE = (Edge*) can_fail_calloc(G->E, sizeof(Edge));
	new->idx = 0;
...
Graph initG(int V, int P, int S)
{
	...
	Graph new = (Graph) can_fail_malloc(sizeof(struct graph));

	new->V = V;
	new->P = P;
	new->S = S;
	new->E = 0;
	new->adj = (Link*) can_fail_malloc(sizeof(Link) * V);
...
void pushRelabelFIFO(Graph G, NetAudit output)
{
	...
	int* h = (int*) can_fail_malloc(sizeof(int) * G->V); 		/* Heights of vertices */
	int* e = (int*) can_fail_malloc(sizeof(int) * G->V); 		/* Excesses of vertices */
	char* active = (char*) can_fail_malloc(sizeof(char) * G->V); /* Active vertices (that have excess) */
	int* gap = (int*) can_fail_calloc(2 * G->V, sizeof(int));     /* Counter of heights */
...
Queue initQ()
{
	...
	new = (Queue) can_fail_malloc(sizeof(struct queue));
...
void putQ(Queue Q, int u)
{
	QueueLink new = (QueueLink) can_fail_malloc(sizeof(struct queueNode));
