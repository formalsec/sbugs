...
void queue_new(Queue *q, size_t size, bool refcount) {
	q->data     = can_fail_malloc((size+1) * sizeof(*q->data));
	q->in_queue = refcount ? can_fail_calloc((size+1), sizeof(*q->in_queue)) : NULL;
	q->front = q->rear = 0;
}
...
void graph_new(Graph *g, int num_v, int num_e)
{
	...
	g->vertex = can_fail_malloc((num_e+1)* sizeof(*g->vertex));
	memset(g->vertex, vertex_nil, (num_e+1)*sizeof(*g->vertex));
	g->next   = can_fail_calloc((num_e+1), sizeof(*g->next));
	g->prev   = can_fail_calloc((num_e+1), sizeof(*g->prev));

	g->flow      = can_fail_calloc((num_e+1), sizeof(*g->flow));
	g->capacity  = can_fail_calloc((num_e+1), sizeof(*g->capacity));
	g->v_minimum = can_fail_calloc((num_v+1), sizeof(*g->v_minimum));
}
...
{
	mf->value = 0;
	mf->level = can_fail_malloc((g->nr_vertices+1) * sizeof(*mf->level));
...
void maxflow_output(MaxFlow *mf, Graph *g)
{
	...
	edges[0] = can_fail_calloc(size, sizeof(*edges[0]));
	edges[1] = can_fail_calloc(size, sizeof(*edges[1]));
