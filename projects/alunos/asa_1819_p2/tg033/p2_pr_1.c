...
int main() {
  ...
  /*Creates and saves all the verts*/
	verts = (vert *) can_fail_malloc((n_vert + 1)*sizeof(vert));
  arcs = (transp *) can_fail_malloc((n_transp + n_sup + 1)*sizeof(transp));
  ...
  positions = (int *) can_fail_malloc(n_vert*sizeof(int));
  positions[0] = 1;
  cut = (transp **) can_fail_malloc((n_transp+1)*sizeof(transp*));
  cut_v = (int *) can_fail_malloc(n_vert*sizeof(int));
  ret = find_arcs(verts, positions, 1, cut, 0, cut_v, 0, n_vert);

  all_v = (int *) can_fail_malloc(ret.n_verts*sizeof(int));
  j=0;
...
void parse_input(vert* verts, int n_sup, int n_vert) {
    ...
    verts[i].height = n_vert;
		verts[i].n_in = 0;
    verts[i].n_out = 0;
    verts[i].entry = (transp **) can_fail_malloc(sizeof(transp*));
    verts[i].out = (transp **) can_fail_malloc(sizeof(transp*));
  ...
  verts[1].n_in = 0;
  verts[1].n_out = 0;
  verts[1].entry = (transp **) can_fail_malloc(sizeof(transp*));
}
...
void parse_matrix(vert* verts, transp* arcs, int n_transp, int n_vert, int n_sup) {
    ...
    verts[path.from].out = (transp**) can_fail_realloc(verts[path.from].out, (verts[path.from].n_out + 1)*sizeof(transp*));
    verts[path.to].entry[verts[path.to].n_in] = &arcs[i];
    verts[path.to].n_in++;
    verts[path.to].entry = (transp**) can_fail_realloc(verts[path.to].entry, (verts[path.to].n_in + 1)*sizeof(transp*));
  }
  ...
  verts[0].n_out = 0;
  verts[0].type = 0;
  verts[0].out = (transp**) can_fail_malloc(sizeof(transp*));
  ...
  for(i=0;i<n_sup;i++) {
    ...
    verts[0].n_out++;
    verts[0].out = (transp**) can_fail_realloc(verts[0].out, (verts[0].n_out + 1)*sizeof(transp*));
    verts[i+2].entry[verts[i+2].n_in] = &arcs[n_transp+i];
    verts[i+2].n_in++;
    verts[i+2].entry = (transp**) can_fail_realloc(verts[i+2].entry, (verts[i+2].n_in + 1)*sizeof(transp*));
  }
}
...
ret find_arcs(vert* verts, int* positions, int n_parse, transp** cut, int n_cut, int* cut_v, int nv, int n_vert) {
  ...
  int* next_batch;
  ret ret;
  next_batch = (int*) can_fail_malloc(n_vert*sizeof(int));
  n = 0;
