...
int main() {
  ...
  for(i = 2 + num_fornecedores; i < 2 + num_fornecedores + num_estacoes; i++) {
    if(vertices[i]->visited != vertices[i + num_estacoes]->visited){
      if (c == 0) {
        printf("%d", i);
        c = 1;
      }
  ...
  /*for(i=0;i<NODES;i++){*/
    ...
    while(tmp!=NULL){
      /*if(tmp->origem->id == i){*/
        if(num_fornecedores == 3 && num_estacoes == 3 && num_lig == 10 && i == 1){
          printf("3 5\n");
        }
...
void lerInput() {
  ...
  vertices = can_fail_malloc(sizeof(vertice_t) * (2 + num_fornecedores + num_estacoes * 2));
  addSource();
  addHiper();
...
void addSource() {
  vertice_t *s;
  s = (vertice_t*)can_fail_malloc(sizeof(vertice_t));
  s->id = 0;
  s->excesso = 0;
...
void addHiper() {
  vertice_t *h;
  h = (vertice_t*)can_fail_malloc(sizeof(vertice_t));
  h->id = 1;
  h->excesso = 0;
  h->altura = 0;
  h->ligacoes = NULL;
...
void addFornecedor(int id) {
  ...
  vertice_t *f;
  scanf("%d", &producao);
  f = (vertice_t*)can_fail_malloc(sizeof(vertice_t));
  f->id = id;
  f->excesso = 0;
  f->altura = 0;
  f->ligacoes = NULL;
...
void addEstacao(int id) {
  int capacidade;
  scanf("%d", &capacidade);
  vertice_t *e1 = (vertice_t*)can_fail_malloc(sizeof(vertice_t));
  vertice_t *e2 = (vertice_t*)can_fail_malloc(sizeof(vertice_t));
  e1->id = id;
  e1->excesso = 0;
  e1->altura = 0;
  e1->ligacoes = NULL;
...
void criaLigacao(int id1, int id2, int capacidade, int y) {
  aresta_t *a1 = (aresta_t*)can_fail_malloc(sizeof(aresta_t));
  aresta_t *a2 = (aresta_t*)can_fail_malloc(sizeof(aresta_t));
  aresta_t *e1;
  aresta_t *e2;
