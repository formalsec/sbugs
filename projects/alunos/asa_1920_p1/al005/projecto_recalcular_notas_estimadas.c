...
Graph graphInit(int V){
    ...
    G->grades=can_fail_malloc(V*sizeof(int));
    G->compSCC=can_fail_malloc(V*sizeof(int));
...
void SCC_Tarjan(Graph G){
    int u;
    int visited=0;
    int *d=can_fail_malloc(sizeof(int)*G->V);
    int *low=can_fail_malloc(sizeof(int)*G->V);
...
link* consListSCC(Graph G){
    int u;
    link*SCC=can_fail_malloc(G->numSCCs*sizeof(struct node)); 
...
void SCCToOneVertex(Graph G, Graph newG, link* SCC){
    ...
    link lScc;
    link lAdj;
    int *edge=can_fail_malloc(G->numSCCs*sizeof(int));
...
void serachBestSolution(Graph G){
    int u;
    int *color=can_fail_malloc(sizeof(int)*G->V);
