...
adj_node_ptr* adj_init(int number_of_nodes)
{
    if (number_of_nodes > 0) {
        int i = 0;
        adj_node_ptr* adj_list = can_fail_malloc(sizeof(adj_node_ptr) * number_of_nodes);
...
{
    adj_node_ptr new_entry = can_fail_malloc(sizeof(struct adj_node));
...
{
	int *ids = (int*) can_fail_malloc(sizeof(int)* n_students);	/*initializes ids array;*/
    adj_node_ptr adj_list = NULL;
...
int main(){
	...
	grades = (int*) can_fail_malloc(sizeof(int)* n_students);		/*initializes grade array*/
