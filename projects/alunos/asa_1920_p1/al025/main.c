...
Student *new_student(int grade, int total_students){
    Student *student = can_fail_malloc(sizeof(Student));
    student->grade = grade;
    student->state = CLEAR;
...
int main(){
    char *line = can_fail_malloc(sizeof(char)*100);
    ...
    Student **students = can_fail_malloc(n * sizeof(Student));
    ...
    for(i = 0; i < m; i++){
        ...
        students[b-1]->is_friends = can_fail_realloc(students[b-1]->is_friends, students[b-1]->friend_amount*sizeof(Student));
