...
struct AdjStudentNode* newAdjStudentNode(int id) { 
    struct AdjStudentNode* newNode = 
     (struct AdjStudentNode*) can_fail_malloc(sizeof(struct AdjStudentNode)); 
    newNode->id = id;
    newNode->next = NULL; 
    return newNode; 
}
...
struct Grafo* newGrafo(int N) {
    int i;  
    struct Grafo* grafo =  
        (struct Grafo*) can_fail_malloc(sizeof(struct Grafo)); 
    grafo->numAlunos = N; 
  
    grafo->array =  
      (struct AdjStudent*) can_fail_malloc(N * sizeof(struct AdjStudent)); 
