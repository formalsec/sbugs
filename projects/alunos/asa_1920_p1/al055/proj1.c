...
Graph initializeGraph(int V) {
    ...
    G->V = V;
    G->E = 0;
    G->adj = can_fail_malloc((V+2) * sizeof(link));
    G->visited = can_fail_malloc((V+2) * sizeof(int));
    G->closed = can_fail_malloc((V+2) * sizeof(int));
    G->grades = can_fail_malloc((V+2) * sizeof(int));
    for (v = 0; v <= V+1; v++) {
        G->adj[v] = NULL;
        G->visited[v] = 0;
		G->closed[v] = 0;
...
int DFS(Graph G, int v) {
    ...
    while(temp!=NULL) {
		...
		if (G->visited[connectedVertex] == 0) {   
			...
			int* visitedAux;
			visitedAux = can_fail_malloc((N+2) * sizeof(int));
