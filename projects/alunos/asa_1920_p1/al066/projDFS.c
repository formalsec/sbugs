...
graph* CreateGraph(int verticesNumber){
    ...
    G->adjlist = (vertex**)can_fail_malloc(sizeof(vertex*)*(verticesNumber+1));
    G->visited = (int*) can_fail_malloc(sizeof(int)*(verticesNumber+1));
...
void  createStudentVertex(int StudentGrade, graph* G, int index){
 ...
 void InsertFriendshipEdge(int student, int friend, graph* G){
    ...
    vertex* aux = (vertex*)can_fail_malloc(sizeof(vertex)); 
    vertex* temp_next=(vertex*)can_fail_malloc(sizeof(vertex));
...
int main(int argc, char *argv[]){
    ...
    PreviousGrades = (int*) can_fail_malloc((sizeof(int))*(studentsNumber+1));  
