...
char* readInput(int length){
    int c;
    int i = 0;
    char *v = (char*) can_fail_malloc(sizeof(char) * length);
    c = getchar();
    while (c != EOF && c != '\0'){
        if (i == (length - 1)){
            length = length * 2;
            v = (char*) can_fail_realloc(v, sizeof(char) * length);
        }
        v[i++] = c;
        c = getchar();
    }
...
char* processFirstLine(char *input, int *position, char mark){
    ...
    char *s = (char*) can_fail_malloc(sizeof(char) * length);
    while (input[pos] != mark && input[pos] != '\0'){
        if (j == (length - 1)){
            length = length * 2;
            s = (char*) can_fail_realloc(s, sizeof(char) * length);
        }
        s[j++] = input[pos++];
    }
    s[j] = '\0';
...
void processGrades(char *input, int *vec, int *position, int n){
    ...
    char *c = (char*) can_fail_malloc(sizeof(char) * lenght);
    for (j = 0; j < n - 1; j++){
        i = 0;
        while (input[pos] != '\n' && input[pos] != EOF) {
            if (i == (lenght - 1)) {
                lenght = lenght * 2;
                c = (char*) can_fail_realloc(c, sizeof(char) * lenght);
            }
            c[i++] = input[pos++];
        }
        pos++;
...
char* nextChar(char *input, char *c, int *position){
    ...
    while (input[pos] != '\n' && input[pos] != ' ' && input[pos] != '\0') {
        if (i == (length - 1)) { 
            length = length * 2;
            c = (char*) can_fail_realloc(c, sizeof(char) * length);
        }
        c[i++] = input[pos++];
    }
    c[i] = '\0';
...
void processFriendships(char *input, Friendship *vec, int *position, int n){
    int j;
    char *c = (char*) can_fail_malloc(sizeof(char) * 3);
    for (j = 0; j < n; j++){
...
void initializeStudents(pStudent *s, int *g, int n){
    int i;
    for (i = 0; i < n; i++){
        pStudent student = (pStudent) can_fail_malloc(sizeof(Student));
        student->id = i + 1;
        student->grade = g[i];
        student->nFriends = 0;
...
void addFriends(pStudent *s, Friendship *f, int n){
    ...
    for (i = 0; i < n; i++){
        ...
        if (s[id - 1]->nFriends == 0)
            s[id - 1]->friends = (pStudent*) can_fail_malloc(sizeof(pStudent));
        else
            s[id - 1]->friends = (pStudent*) can_fail_realloc(s[id - 1]->friends, sizeof(pStudent) * (s[id - 1]->nFriends + 1));
...
int main(){
    ...
    grades = (int*) can_fail_malloc(sizeof(int) * (atoi(numStudents) + 1));
    processGrades(input, grades, pos, atoi(numStudents) + 1);
    friendships = (Friendship*) can_fail_malloc(sizeof(Friendship) * atoi(numFriendships));
    processFriendships(input, friendships, pos, atoi(numFriendships));

    free(input);
    students = (pStudent*) can_fail_malloc(sizeof(pStudent) * atoi(numStudents));
