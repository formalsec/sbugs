...
void processInput(){
    ...
    stack = (int*)can_fail_malloc(sizeof(int)*num_students);

    nodes = (graphNode*)can_fail_malloc(sizeof(graphNode) * num_students);
    ...
    for(v = 0;v < num_friendships;v++){
        scanf("%d %d\n", &i, &j);
        nodes[i-1].num_adj++;   
        nodes[i-1].adj = (int*)can_fail_realloc(nodes[i-1].adj, sizeof(int) * nodes[i-1].num_adj);
...
void SCCS_VISIT(int v){
    ...
    if(nodes[v].low == nodes[v].d_SCC){
        gradesSCCs = (int*)can_fail_realloc(gradesSCCs, sizeof(int) * (num_SCCS+1));
