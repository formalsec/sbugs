...
void inicializarTabela(){
    int i;
    tabela = can_fail_malloc(sizeof(vertice)*(nAlunos+1));
    for(i = 1;i <= nAlunos;i++){
        tabela[i].nota = 0;
        tabela[i].cor = White;
        tabela[i].nAdjacencias = 0;
        tabela[i].adjacencias = can_fail_malloc(sizeof(int));
...
void adicionaArco(int u, int v){
    if(tabela[u].nAdjacencias < 1){
        ...
        tabela[u].adjacencias = can_fail_realloc(tabela[u].adjacencias,sizeof(int)*(tabela[u].nAdjacencias+1));
        tabela[u].nAdjacencias++;
