...
void DFS(Aluno* g,int* ordem, int total){
	int i;
	int* arvores;
	arvores = can_fail_malloc(sizeof(int));

	
	int* fechados;
    	fechados = can_fail_malloc(sizeof(int));
...
int main(){
	...
	for(i=0;i<n_alunos;i++){
		Aluno *aluno = can_fail_malloc(sizeof(struct Aluno));
		alunos[i] = *aluno;
		alunos[i].nota = 0;
		...
		alunos[i].fecho=0;
		alunos[i].arvore=0;
		alunos[i].relacoes = can_fail_malloc(sizeof(int)*n_alunos-1);
	}
	...
	for(i=0;i<n_alunos;i++){
		...
		alunos_T[i].fecho=0;
		alunos_T[i].arvore=0;
		alunos_T[i].relacoes = can_fail_malloc(sizeof(int)*n_alunos-1);
	}
