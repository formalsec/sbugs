...
void readUserInput() {
    ...
    scanf("%d,%d", &n, &m);
    class = (student**)can_fail_malloc(n*sizeof(student*));

    /* The *class Array index differs from student idex by 1 integer*/
    for (i = 0; i < n; i++) {
        class[i] = (student*)can_fail_malloc(sizeof(student));
        scanf("%d", &grade_input);
...
void putFriend(int u, int v) {
    ...
    u_buddy = (student*)can_fail_malloc(sizeof(student));
...
void doDfs() {
    ...
    visited = (int*)can_fail_calloc(n, sizeof(int));
