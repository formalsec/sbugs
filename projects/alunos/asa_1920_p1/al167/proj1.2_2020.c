...
graph newGraph(int n_students){
	...
	new->students = (student*) can_fail_malloc(sizeof(student) * n_students);
...
student newStudent(int id, int grade){
	student new = (student) can_fail_malloc(sizeof(struct Student));
	new->color = 0;
	new->in_list = 0;
	new->d = INT_MAX;
	new->low = INT_MAX;
...
void newFriend(student stud, student friend){
	friendLink new_rel = (friendLink) can_fail_malloc(sizeof(struct FriendLink));
...
void tarjanVisit(graph sGraph, student s){
	...
	if (s->low == s->d){
		shared_estim_ptr = (int*) can_fail_malloc(sizeof(int));
