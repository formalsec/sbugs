...
Graph* initializeGraph(int vertices) {
	/* can_fail_malloc for new node */
	Graph* graph = (Graph*)can_fail_malloc(sizeof(Graph));
	graph->vertices = vertices;
	graph->visited = (int*)can_fail_malloc(vertices * sizeof(int));
	/* can_fail_malloc for new adjacency list node */
	graph->head = (AdjNode**)can_fail_malloc(vertices * sizeof(AdjNode*));
	int i;
...
Queue* createQueue(int capacity) {
	...
	queue->array = (AdjNode**)can_fail_malloc(queue->capacity * sizeof(AdjNode*));
	return queue;
}
...
Stack* createStack(int capacity)
{
	...
	stack->array = (AdjNode**) can_fail_malloc(sizeof(AdjNode*) * capacity);
	return stack;
}
...
void tarjanVisit(Graph* graph, AdjNode* u, int* visited, Stack* stack, AdjNode*** scc_list, int* n_scc) {
	...
	if(u->node->discovered == u->node->low) {
		int counter_scc = 0;
		new_scc = (AdjNode**)can_fail_malloc((stack->size + 1) * sizeof(AdjNode*));
		int i;
...
AdjNode*** tarjanAlgorithm(Graph* graph) {
	int vertices = graph->vertices;
	AdjNode*** scc_list = (AdjNode***)can_fail_malloc((vertices + 1) * sizeof(AdjNode**));		/* list with SCCs */
	int i;
