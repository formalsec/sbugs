...
scc_t* create_scc_array(){
    int i;
    scc_t *scc = can_fail_malloc(N * sizeof(scc_t));
    for (i = 0; i < N; i++){
        scc[i].grade = 0;
        scc[i].group = NULL;
    }
...
stack_t* create_stack(){
    stack_t *stack = can_fail_malloc(sizeof(stack_t));
    stack -> lifo = can_fail_malloc(N * sizeof(int));
    stack -> values = can_fail_malloc(N * sizeof(int));
    stack -> top = 0;
    return stack;
}
...
node_t* create_node(int id){
    node_t *node = can_fail_malloc(sizeof(node_t));
    node -> id = id;
    node -> next = NULL;
    return node;
}
...
int main() {
    ...
    scanf("%d, %d", &N, &M);
    students = can_fail_malloc(N * sizeof(student_t));
