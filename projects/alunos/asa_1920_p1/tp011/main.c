...
int main(int argc, char *argv[]) {
    ...
    array = can_fail_malloc(sizeof(person)*(n_nodes+1));
    dummyArray = can_fail_malloc(sizeof(dummy)*(n_nodes+1));
    stack = can_fail_malloc(sizeof(person));
    ...
    for (i=1;i<=n_nodes;i++) {
        ...
        array[i].color = WHITE;
        array[i].friends = can_fail_malloc(sizeof(person*));
    ...
    for(i=0;i<n_c;i++) {
        scanf("%d %d", &p, &c);
        array[p].friends = can_fail_realloc(array[p].friends, sizeof(person**)*(array[p].n_friends + 1));
    ...
    while (1==1) {
        ...
        if(next == -1) {
            ...
            stack = can_fail_realloc(stack, sizeof(person)*(stack_size-1));
            stack_size--;
        } else {
