...
{
	graph = can_fail_calloc(*N,sizeof(node));
}

node* createNode(int *val, int *N)
{
	node *dummy = can_fail_malloc(sizeof(struct node));
	dummy -> value = *val;
	dummy -> done = 1; /* Equivalent to false */
	dummy -> num_conections = 0;
	if(*N < 50){  /*Otimizacao da alocacao da memoria */
		dummy -> conections = can_fail_malloc(sizeof(int)*(*N));
		dummy -> size = *N;
	}
	else{
		dummy -> conections = can_fail_malloc(sizeof(int)*50);
		dummy -> size = 50;
	}
	return dummy;
}
...
int parseArgs(int *N, int *M){
	...
	val = can_fail_malloc(sizeof(int));
	father_node = can_fail_malloc(sizeof(int));
	son_node = can_fail_malloc(sizeof(int));
	
	scanf("%d,%d",N, M);
	stack_size = *N;
	initinalizeGraph(N);
	graph_val = can_fail_calloc(21,sizeof(int));
	...
	for(i = 0; i < *M; i++){
		...
		{
			graph[*father_node - 1] -> conections = can_fail_realloc(graph[*father_node - 1] -> conections,sizeof(int)*(conects + 50));
...
{	
	...
	int *N, *M;
	N = can_fail_malloc(sizeof(int));
	M = can_fail_malloc(sizeof(int));
