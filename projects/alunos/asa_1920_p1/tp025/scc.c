...
void initialize(int *num_students, int *num_relations)
{
    ...
    students = can_fail_malloc(*num_students * sizeof(Student));
    group = can_fail_malloc(*num_students * sizeof(int));
    stack = can_fail_malloc(sizeof(Stack));
    stack->top = -1;
...
Student create_student(int id, int grade)
{
    Student new_student = can_fail_malloc(sizeof(struct stru_student));
    new_student->d = -1;
...
void add_relation(int src, int dest)
{
    ...
    student->frients = can_fail_realloc(student->frients, (student->num_friends + 1) * sizeof(Student));
...
Stack *create_stack(int size)
{
    Stack *stack = can_fail_malloc(sizeof(Stack));
    stack->top = -1;
    stack->array = can_fail_malloc(size * sizeof(Student));
    return stack;
}
...
{
    stack->top++;
    stack->array = can_fail_realloc(stack->array, (stack->top + 1) * sizeof(Student));
