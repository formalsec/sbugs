...
void createGraph(){
	int i;
	grafo = (struct graph*)can_fail_malloc(sizeof(struct graph));
	grafo->tamanho = nr_alunos;
	grafo->turma = (struct aluno*)can_fail_malloc(sizeof(struct aluno)*nr_alunos);
	d = (int*)can_fail_malloc(sizeof(int)*nr_alunos);
	low = (int*)can_fail_malloc(sizeof(int)*nr_alunos);
	visDfs = (int*)can_fail_malloc(sizeof(int)*nr_alunos);
	visBfs = (int*)can_fail_malloc(sizeof(int)*nr_alunos);
...
void connect(int src, int amigos){
	...
	aux->next = (struct aluno*)can_fail_malloc(sizeof(struct aluno));
	aux->next->i = amigos;
	aux->next->next = NULL;
}
...
void createStack(){
	stack = (struct stack*)can_fail_malloc(sizeof(struct stack));
	stack->topo = -1;
	stack->tamanho = nr_alunos;
	stack->array = (int*)can_fail_malloc(sizeof(int)*nr_alunos);
}
...
void createFila(){
	fila = (struct fila*)can_fail_malloc(sizeof(struct fila));
	fila->array = (int*)can_fail_malloc(sizeof(int)*nr_alunos);
	fila->topo = -1;
	fila->fim = -1;
}
...
void processInput(){
	...
	createStack();
	createFila();
	notas = (int*)can_fail_malloc(sizeof(int)*nr_alunos);
	for(i = 0; i < nr_alunos; i++){
		if(scanf("%d", &nota) != 1){
