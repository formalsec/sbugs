...
SCC newSCC(int id, int grade){
	SCC s = (SCC)can_fail_malloc(sizeof(struct scc));
	s->id = id;
    s->d = -1;
    s->f = -1;
    s->color=1;
...
void Tarjan_Visit(Vertice *amigos, int casa, int *vStack){
    ...
    if(v->d == v->low){
    	if (sccAtual == 0){
    		SCCs = (SCC*)can_fail_malloc((sccAtual+1)*sizeof(struct scc));
    	}
    	else {
    		SCCs = (SCC*)can_fail_realloc(SCCs,(sccAtual+1)*sizeof(struct scc));
    	}
        max = 0;
        do{
