...
/*Visited list functions*/
void init_visited(){
    visited=(int*)can_fail_calloc(numberOfVertices,sizeof(int));
}
...
void Graph_init(){
    ...
    ad_list=(edgePtr*)can_fail_malloc(sizeof(edgePtr)*numberOfVertices);
...
void add_new_Edge(int from, int to){
    edgePtr new_front=(edgePtr)can_fail_malloc(sizeof(struct edge));
    edgePtr new_backward=(edgePtr)can_fail_malloc(sizeof(struct edge));
    new_front->to=to;
    new_front->from=from;
