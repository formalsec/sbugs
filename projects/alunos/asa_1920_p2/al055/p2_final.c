...
void enQueue(queue *q, int value) { /*adds elements to the queue the end*/
    element *tmp;
    tmp = can_fail_malloc(sizeof(element));
...
link createLink(int n, int flow) { /*creates link to graph vertex with flow from another vertex to this one*/
    link l;
    l = can_fail_malloc(sizeof(struct node));
    l->n = n;
    l->flow = flow;
    l->next = NULL;
    return l;
...
link* initializeGraph() {   /*prepares the graph, initializes flows and connections and reads the remaining input*/
    ...
    G = can_fail_malloc(V * sizeof(link));
    for (i = 0; i < V; i++) {
        G[i] = can_fail_malloc(sizeof(struct node));
        G[i]->n = i;
        G[i]->next = NULL;
    }

    for (i = 1; i < V-2; i += 2) {
        n = can_fail_malloc(sizeof(struct node));
        n->n = i+1;
        n->flow = 1;
        n->next = NULL;
        G[i]->next = n;
...
int bfs(link *G, int s, int t, int parent[]) { /*visiting algorithm*/
    ...
    link l;
    queue *q;
    q = can_fail_malloc(sizeof(queue));
    visited = can_fail_malloc(V*sizeof(int));
...
int edmondsKarp(link *G, int s, int t) { /*runs the bfs algorithm and updates the graphs and flows*/
    ...
    link l, m;
    int* parent;
    parent = can_fail_malloc(V*sizeof(int));
