...
{
    vertice *falso = (vertice*)can_fail_malloc(sizeof(vertice));
...
{
    int i, j, k;
    vertice **grafo = (vertice **)can_fail_malloc(numDeAvenidas * sizeof(vertice *));
    for(i = 0; i < numDeAvenidas; i++)
    {
        grafo[i] = (vertice *)can_fail_malloc(numDeRuas*2 * sizeof(vertice));
        for(j=0; j<numDeRuas*2; j++)
        {
            ...
            grafo[i][j].oY = i;
            grafo[i][j].oX = j;
            grafo[i][j].arestas = (aresta**)can_fail_malloc(5*sizeof(aresta));
            for(k = 0; k<5; k++)
            {
                grafo[i][j].arestas[k] = (aresta*)can_fail_malloc(sizeof(aresta));
...
vertice* criarSink()
{
    vertice *sink = (vertice*)can_fail_malloc(sizeof(vertice));

    sink->arestas = (aresta**)can_fail_malloc(sizeof(aresta));
    sink->tipo = ralo;
    sink->oX = -2;
    sink->oY = -2;
    return sink;
...
vertice* criarSource(int numDePessoas, vertice *falso)
{
    ...
    vertice *source = (vertice*)can_fail_malloc(sizeof(vertice));
    source->arestas = (aresta**)can_fail_malloc(numDePessoas*sizeof(aresta));
    for(i = 0; i<numDePessoas*2; i++)
    {
        source->arestas[i] = (aresta*)can_fail_malloc(sizeof(aresta));
    ...
    for(i = 0; i < numDeMercados; i++)
    {
        if(scanf("%d %d", &avenida, &rua) != EOF)
        {
            if(manhatan[avenida-1][2*rua-1].tipo != superMercado)
            {
                ...
                manhatan[avenida-1][2*rua - 1].ligacaoEspecial = (aresta*)can_fail_malloc(sizeof(aresta));
...
int edmondsKarp(vertice *source, vertice *sink, int numDePessoas, int totalDeVertices, int numDeRuas, int numDeAvenidas, vertice **manhatan, int numDeMercados, vertice *falso)
{
    ...
    vertice *aux;
    vertice *aux2;
    queue *fila = (queue*)can_fail_malloc(sizeof(queue));
    fila->filaDeVertices = (vertice**)can_fail_malloc(totalDeVertices*sizeof(vertice));
