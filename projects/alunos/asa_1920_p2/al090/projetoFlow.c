...
void newEdge(Graph graph, int v, int w, int weight, bool hasReverse) {
    ...
    Edge* new = can_fail_malloc (sizeof (struct edge));
    new->from = v;
    new->to = w;
    new->capacity = weight;
...
Graph initGraph() { 
    ...
    G->V = avenues*streets;
    G->E = 0;
    G->adj = can_fail_malloc ((avenues*streets*2+2) * sizeof(AdjList));
...
int main() {
    ...
    prev = (int*)can_fail_malloc(vertices * sizeof(int));
    currentC = (int*)can_fail_malloc(vertices * sizeof(int));
