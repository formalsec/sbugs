...
List* newList() {
    List *l= (List*) can_fail_malloc(sizeof(List));
    l->head = l->tail = NULL;
    return l;
}

EdgeList* newEdgeList() {
    EdgeList *l= (EdgeList*) can_fail_malloc(sizeof(EdgeList));
    l->head = l->tail = NULL;
    return l;
}
...
List** initGraph(int graphSize) {
    int i, c = 0, x = 0;
    List **list = (List**) can_fail_malloc(sizeof(List*) * graphSize);
    for (i = 0; i < graphSize; i++) {
        list[i] = newList();
    }

    Vertex *source = (Vertex*) can_fail_malloc(sizeof(Vertex));
    Vertex *sink = (Vertex*) can_fail_malloc(sizeof(Vertex));
    source->id = c++; sink->id = c++;
    source->type = sink->type = NONE;
    addVertex(list[x++], source); addVertex(list[x++], sink);

    for (i = 0; i < (graphSize - 2)/2; i++) {
        Vertex *v_in = (Vertex*) can_fail_malloc(sizeof(Vertex));
        Vertex *v_out = (Vertex*) can_fail_malloc(sizeof(Vertex));
...
EdgeList** createEdges(List** graph, int graphSize) {
    int i;
    Node *aux;
    EdgeList **list = (EdgeList**) can_fail_malloc(sizeof(EdgeList*) * graphSize);
    for (i = 0; i < graphSize; i++) {
        list[i] = newEdgeList();
    }
    for (i = 0; i < graphSize; i++) {
...
void push(Queue *queue, int i) {
    Int *number = (Int*) can_fail_malloc(sizeof(Int));
    number->i = i;
    number->next = NULL;
    if (queue->head == NULL) {
...
int edmondsKarp(int startNode, int endNode, EdgeList **edges, int graphSize) {
    int maxFlow = 0;
    int *parentsList = (int*) can_fail_malloc(sizeof(int) * graphSize);
    int *currentPathCapacity = (int*) can_fail_malloc(sizeof(int) * graphSize);;
    while (TRUE) {
