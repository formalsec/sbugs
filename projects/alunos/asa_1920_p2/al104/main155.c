...
void initializeMarkets(){
	...
	for(;markets>0;markets--){
        ...
        if(city[i].present==2) nMarkets--;
            ...
            city[i+nIntersections].edges=can_fail_malloc(sizeof(edge)*2);
            city[i].edges=can_fail_malloc(sizeof(edge)*5);
...
void initializeHouses(){
	...
	for(;houses>0;houses--){
        ...
        if(city[i].present==1) nHouses--;
			...
			city[i].edges=can_fail_malloc(sizeof(edge) * 2);
            city[i+nIntersections].edges=can_fail_malloc(sizeof(edge)*5);
            ...
            temp = can_fail_malloc(sizeof(fluxer));
            temp->index = i;
...
void initializecity(){
    int i,edgeIndex = 0;
    city=can_fail_malloc(sizeof(Intersection)*(2*nIntersections+1));
    initializeMarkets();
    initializeHouses();
    initializeSuper();
