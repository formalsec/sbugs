...
struct edge* createEdge(int destx, int desty, int origx, int origy){
  struct edge* newEdge = can_fail_malloc(sizeof(struct edge));
...
struct vertice* createVertice(int x, int y){
  struct vertice* newVertice = can_fail_malloc(sizeof(struct vertice));
...
void initGraph(struct graph* G, int m, int n, int s, int c){
  ...
  G->headqueue = NULL;
  G->tailqueue = NULL;
  G->matrix = can_fail_malloc((m*2 + 1) * sizeof(struct vertice**));
  G->matrix[0] = can_fail_malloc(2 * sizeof(struct vertice*));
  for(i = 1; i <= m * 2; i++){
    G->matrix[i] = can_fail_malloc((n + 1) * sizeof(struct vertice*));
  }
}
