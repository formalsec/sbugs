...
struct node* createNode(int id, int x , int y) {
    ...
    newNode->vertex = can_fail_calloc(1, sizeof(struct vertex));
    newNode->vertex->x = x;
    newNode->vertex->y = y;
    newNode->next = NULL;
    return newNode;
...
void addEdge(struct node* node, int id){
    if (node->index_adj == NULL) {
        struct index *index = can_fail_calloc(1, sizeof(struct index));
        ...
        paux->pnext = can_fail_calloc(1, sizeof(struct index));
        paux->pnext->id = id;
...
struct Graph* createGraph(int n_avenues, int n_streets) {
    ...
    graph->adjLists = can_fail_calloc(graph->numVertices,sizeof(struct node*));
