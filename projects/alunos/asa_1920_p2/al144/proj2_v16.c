...
void allocAdjLists(elink** e){
    *e = (elink*) can_fail_malloc(sizeof(elink)*n_Vertex);
    return;
}
...
void allocBasic(int** M, int** N, int** S, int** C){
    *M = (int*)can_fail_malloc(sizeof(int));  /*number of avenues*/
    *N = (int*)can_fail_malloc(sizeof(int));  /*number of streets*/
    *S = (int*)can_fail_malloc(sizeof(int));  /*number of supermarkets*/
    *C = (int*)can_fail_malloc(sizeof(int));  /*number of citizens*/
    return;
}  
...
void parseBasic(int* M, int* N, int* S, int* C){
    ...
    target_id = ((*N)*(*M)*2)+1;
    n_Vertex = ((*N)*(*M)*2)+2;
    parents = (short*)can_fail_malloc(sizeof(short)*n_Vertex);
    queue = (int*)can_fail_malloc(sizeof(int)*n_Vertex);         /* allocates the maximum number of elements possible for the priority queue*/
    return;
}
...
void connectLeft(elink* edges, int i){
    elink e1, e2;
    e1 = (elink)can_fail_malloc(sizeof(edge));   /*"current-out" to "left-in"*/
    e1->to = i-1;
    e1->capacity = MAX_CAPACITY;
    e1->next = NULL;
    addEdge(edges, e1, i);
    
    e2 = (elink)can_fail_malloc(sizeof(edge));   /*"current-in" to "left-out"*/
    e2->to = i-2;
...
void connectRight(elink* edges, int i){
    elink e1, e2;
    e1 = (elink)can_fail_malloc(sizeof(edge));   /*"current-out" to "right-in"*/
    e1->to = i+3;
    e1->capacity = MAX_CAPACITY;
    e1->next = NULL;
    addEdge(edges, e1, i);
    
    e2 = (elink)can_fail_malloc(sizeof(edge));   /*"current-in" to "right-out"*/
    e2->to = i+2;
    e2->capacity = NO_CAPACITY;
    e2->next = NULL;
    addEdge(edges, e2, i+1);
...
void connectAbove(elink* edges, int i, int A){
    elink e1, e2;
    e1 = (elink)can_fail_malloc(sizeof(edge));   /*"current-out" to "above-in"*/
    e1->to = i-(2*A)+1;
    e1->capacity = MAX_CAPACITY;
    e1->next = NULL;
    addEdge(edges, e1, i);
    
    e2 = (elink)can_fail_malloc(sizeof(edge));   /*"current-in" to "above-out"*/
    e2->to = i-(2*A);
    e2->capacity = NO_CAPACITY;
    e2->next = NULL;
    addEdge(edges, e2, i+1);
...
void connectBelow(elink* edges, int i, int A){
    elink e1, e2;
    e1 = (elink)can_fail_malloc(sizeof(edge));   /*"current-out" to "below-in"*/
    e1->to = i+(2*A)+1;
    e1->capacity = MAX_CAPACITY;
    e1->next = NULL;
    addEdge(edges, e1, i);
    
    e2 = (elink)can_fail_malloc(sizeof(edge));   /*"current-in" to "below-out"*/
    e2->to = i+(2*A);
    e2->capacity = NO_CAPACITY;
    e2->next = NULL;
    addEdge(edges, e2, i+1);
...
void connectWithin(elink* edges, int i){
    elink e1, e2;
    e1 = (elink)can_fail_malloc(sizeof(edge));   /*"current-out" to "current-in"*/
    e1->to = i+1;
    e1->capacity = NO_CAPACITY;
    e1->next = NULL;
    addEdge(edges, e1, i);
    
    e2 = (elink)can_fail_malloc(sizeof(edge));   /*"current-in" to "current-out"*/
    e2->to = i;
...
void connectTarget(elink* edges, int SMerc, int Av){
    ...
    for(i = 0; i < SMerc; i++){
        ...
        e1 = (elink)can_fail_malloc(sizeof(edge));
        e1->to = target_id;
        e1->capacity = MAX_CAPACITY;
        e1->next = NULL;
        addEdge(edges, e1, orig);       /*"current-out" to target*/
        
        e2 = (elink)can_fail_malloc(sizeof(edge));
        e2->to = orig;
        e2->capacity = NO_CAPACITY;
        e2->next = NULL;
