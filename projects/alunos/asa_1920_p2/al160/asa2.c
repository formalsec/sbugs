...
void edmondsKarp(int** matrix, int* cits, int* sups, int num_sup, int num_cit, int n, int num_st, int num_av) {
	...
	int* parent = can_fail_malloc(sizeof(int) * n);
	queue* q = can_fail_malloc(sizeof(queue));
	q->node = can_fail_malloc(sizeof(int) * n);
...
int** initializeGraph(int num_av, int num_st) {
	...
	int** m = can_fail_malloc(sizeof(int*) * n);

	for (i = 0; i < n; i++) {
		m[i] = can_fail_malloc(sizeof(int) * 5);
	}
...
int* initializeCitLocations(int n, int num_st) {
	int i, av, st;
	int* list = can_fail_malloc(sizeof(int) * n);
	for (i = 0; i < n; i++) {
		scanf("%d %d", &av, &st);
