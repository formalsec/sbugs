...
void join(list *s,node * v) {
	...
	listNode *tmp = can_fail_malloc(sizeof(listNode));
	tmp->value = v;
	tmp->next = NULL;
...
node * createNode(int x, int y, char t) {
	node *tmp = can_fail_malloc(sizeof(node));
...
void pair(node *n1, node *n2) {
	...
	tmp = can_fail_malloc(sizeof(listNode));
...
void scanMap() {
	...
	map = can_fail_malloc(sizeof(corner*) * M);

	for(x = 0; x < M; x++) {
		map[x] = can_fail_malloc(sizeof(corner) * N);
		for(y = 0; y < N; y++) {
