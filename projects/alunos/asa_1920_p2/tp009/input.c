...
void insert(int src, int dest) {
    graph.adjList[src] = can_fail_realloc(graph.adjList[src], ((++graph.numArcs[src]))*sizeof(int));
...
void processInput() {
    ...
    graph.numArcs = can_fail_malloc(sizeof(int)*(graph.numVertices*2 + 2));
    graph.adjList = (int **)can_fail_malloc((graph.numVertices*2 + 2) * sizeof(int *));
    /* initialize empty capacity matrix */
    for (i = 0; i < graph.numVertices; i++) {
        graph.adjList[i] = (int *)can_fail_malloc(sizeof(int));
        graph.numArcs[i] = 0;
        graph.adjList[i + graph.numVertices] = (int *)can_fail_malloc(sizeof(int));
    ...
    scanf("%d %d", &c, &e);
    graph.adjList[graph.numVertices] = (int *)can_fail_malloc(sizeof(int));
...
int fordFulkerson() {
    ...
    queue.list = can_fail_malloc(sizeof(int)*graph.numVertices);
