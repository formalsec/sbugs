...
int main(){
  ...
  graphIn = (node_t *) can_fail_calloc(graphSize, sizeof(node_t));
  graphOut = (node_t *) can_fail_calloc(graphSize, sizeof(node_t));
  buildGraph();
...
void allocateNeighbor(node_t *graph, int index){
    if(graph[index].vizinhos == NULL)
        graph[index].vizinhos = (vizinho_t *) can_fail_calloc(32, sizeof(vizinho_t));
}

void checkIfReallocNeeded(node_t *in, node_t *out, int index) {
    if(in->nVizinhos % 32 == 0){
      in->vizinhos = (vizinho_t *) can_fail_realloc(in->vizinhos, sizeof(vizinho_t)*(in->nVizinhos + 32));
    }

    if(out->nVizinhos % 32 == 0){
        out->vizinhos = (vizinho_t *) can_fail_realloc(out->vizinhos, sizeof(vizinho_t)*(out->nVizinhos + 32));

    }
    if(graphIn[index].nVizinhos % 32 == 0){
        graphIn[index].vizinhos = (vizinho_t *) can_fail_realloc(graphIn[index].vizinhos, sizeof(vizinho_t)*(graphIn[index].nVizinhos + 32));

    }

    if(graphOut[index].nVizinhos % 32 == 0){
        graphOut[index].vizinhos = (vizinho_t *) can_fail_realloc(graphOut[index].vizinhos, sizeof(vizinho_t)*(graphOut[index].nVizinhos + 32));
    }
}
...
void buildGraph(){
  ...
  if(uSourceIn.vizinhos == NULL)
    uSourceIn.vizinhos = (vizinho_t *) can_fail_calloc(32, sizeof(vizinho_t));
  if(uSinkIn.vizinhos == NULL)
    uSinkIn.vizinhos = (vizinho_t *) can_fail_calloc(32, sizeof(vizinho_t));
  if(uSourceOut.vizinhos == NULL)
    uSourceOut.vizinhos = (vizinho_t *) can_fail_calloc(32, sizeof(vizinho_t));
  if(uSinkOut.vizinhos == NULL)
    uSinkOut.vizinhos = (vizinho_t *) can_fail_calloc(32, sizeof(vizinho_t));
...
void bfs_call(node_t *node){
  ...
  int queuePtr = 0;
  int queueEndPtr = 0;
  queue = (node_t **) can_fail_calloc(graphSize*2+4, sizeof(node_t *));
