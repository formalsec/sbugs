...
{
	node *brotha;
	node * some = can_fail_malloc(sizeof(node));
	some -> id = id;
...
{
	adjGraph[i] -> connections = can_fail_malloc(7*sizeof(int));
	if(i == 0)
	{
		addEdge(0,1);
		return;
...
{
	node *some, *brotha;
	some = can_fail_malloc(sizeof(node));
	some -> num_connections = 0;
	if(type == 0)
	{
		some-> connections = can_fail_malloc(pessoas *sizeof(int));
		brotha = some;
		return some;
	}
	some -> connections = can_fail_malloc(supermercados *sizeof(int));
	brotha = some;
	return brotha;
}
...
{
	int i;
	adjGraph = can_fail_malloc((avenidas * ruas * 2 + 2)*sizeof(node*));
	parent = can_fail_malloc((avenidas * ruas * 2 + 2)*sizeof(int));
...
void parseArgs()
{
	...
	array = can_fail_calloc(supermercados,sizeof(int));
	initializeAdjGraph();
	...
	free(array);
	array = can_fail_calloc(pessoas,sizeof(int));
	size = 0;
	for(i = 0; i < pessoas; i++)
	{
		scanf("%d %d", &x,&y);
...
{	
	int flo = 0;
	parseArgs();
	queue = can_fail_malloc((BigBoss+ 2)*sizeof(int));
	visited = can_fail_calloc(ruas * avenidas * 2 + 2,sizeof(int));
	flo = edmondsKARPA();
	free(queue);
	free(parent);
	printf("%d\n", flo);
