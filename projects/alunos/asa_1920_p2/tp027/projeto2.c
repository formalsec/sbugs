...
int main(){
    ...
    manhattan = can_fail_malloc(sizeof(node) * numVertices);
    supermarkets = can_fail_malloc(sizeof(int) * numSuper);
    citizens = can_fail_malloc(sizeof(int) * numCitizens);
    visited = can_fail_malloc(sizeof(int)*numVertices+2);
    head = NULL;
    last = NULL;
...
int fordFulkerson(int s, int t){ 
    ...
    parent = can_fail_malloc(sizeof(int)* numVertices+2);
