...
void connect(int i1, int i2, int c){
	if(grafo->matriz[i1] == NULL){
		grafo->matriz[i1] = can_fail_malloc(sizeof(edge));
		...
		aux->next = can_fail_malloc(sizeof(edge));
		aux->next->v = i2;
		aux->next->cap = c;
		aux->next->next = NULL;
	}
...
void createGraph(){
	...
	grafo = can_fail_malloc(sizeof(graph));
	grafo->tamanho = nr_cruzamentos*2 + 2;
	grafo->matriz = can_fail_malloc(sizeof(edge*) * grafo->tamanho);
	vis = can_fail_malloc(sizeof(int)*grafo->tamanho);
	pi = can_fail_malloc(sizeof(int)*grafo->tamanho);
...
void createFila(){
	fila = can_fail_malloc(sizeof(queue));
	fila->array = can_fail_malloc(sizeof(int)*grafo->tamanho);
	fila->topo = -1;
	fila->fim = -1;
}
