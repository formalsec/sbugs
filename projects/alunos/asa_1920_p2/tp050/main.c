...
int main(){
	...
	list = can_fail_malloc(avenidas * sizeof(node**));
	for(i = 0; i < avenidas; i++){
		list[i] = can_fail_malloc(ruas * sizeof(node*));
	}

	queue = can_fail_malloc(ruas*avenidas*sizeof(node*));
	for(i = 0; i < avenidas; i++){
		for(j = 0; j < ruas; j++){
				list[i][j] = can_fail_malloc(sizeof(node));
				list[i][j]->direcoes = can_fail_malloc(4* sizeof(node*));
		}
	}
	...
	supermercadosList = can_fail_malloc(supermercados * sizeof(node*));
	for(i = 0; i < supermercados; i++){
	...
	casasList = can_fail_malloc(casas * sizeof(node*));
	for(i = 0; i < casas; i++){
		scanf("%d %d", &auxAvenida, &auxRua);
		auxRua--;
		auxAvenida--;
