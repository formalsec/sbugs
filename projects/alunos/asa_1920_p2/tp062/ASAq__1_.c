...
void enqueue(int data){
    Queue newNode = NULL;
    newNode = (Queue)can_fail_malloc(sizeof(struct queue));
    newNode->data = data;
    newNode->next = NULL;
    if (rear)
    {
...
int *to2D(int index, int width){
	int *coor=(int*)can_fail_malloc(2*sizeof(int));
	coor[0] = index % width;
	coor[1] = index / width;
	return coor;
}
...
Vertex *initUrb(int N, int width, int height){
	...
	int *coor;
	int newcoor;
	Vertex *sect=(Vertex*)can_fail_malloc((N+2)*sizeof(struct vertex));
	res=(int*)can_fail_malloc((N+2)*(N+2)*sizeof(int));
	for(i=0; i<N; i++){
		sect[i]=newVertex(i);
		coor = to2D(i, width);
		if (coor[1]-1>=0){
