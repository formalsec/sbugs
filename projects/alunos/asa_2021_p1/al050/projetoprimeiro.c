...
void putPiece(int first, int second)
{
    if (table[first - 1].n_filhos == 0)
    {
        ...
        table[first - 1].filhos = can_fail_malloc(sizeof(int));
    ...
    else
    {
        ...
        table[first - 1].n_filhos++;
        table[first - 1].filhos = can_fail_realloc(table[first - 1].filhos, sizeof(int) * table[first - 1].n_filhos);
...
void inicializeTable()
{
    ...
    w = can_fail_malloc(sizeof(int) * n_pieces);
...
int topologicalSort()
{
    ...
    int node;
    int ans = 0;
    int * order = (int *) can_fail_malloc(n_pieces * sizeof(int));
    int * lista = (int *) can_fail_malloc(n_pieces * sizeof(int));
    int * array = (int *) can_fail_malloc(n_pieces * sizeof(int));
...
{   
    ...
    table = can_fail_malloc(sizeof(Piece) * n_pieces);
