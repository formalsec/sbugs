...
void createGraph(int vertices){
    ...
    ArrayAdjLists = (List *)can_fail_malloc(sizeof(List)*vertices);

    for(i=0; i<vertices; i++){
        ArrayAdjLists[i].head = NULL;
    }

    order_topol = (int *)can_fail_malloc(sizeof(int)*vertices);
    visited = (bool *)can_fail_malloc(sizeof(bool)*vertices);
    indegree = (int *)can_fail_malloc(sizeof(int)*vertices);
}
