...
int main(){
    ...
    table = (Node*)can_fail_malloc(sizeof(Node)*number_total_dominos);
    initialize();
...
void putnode(int first, int second){
    if(table[first].nfilhos == 0){
        table[first].filhos =  (int*)can_fail_malloc(sizeof(int));
        table[first].filhos[0] = second;
        table[first].nfilhos++;
    }else{
        table[first].nfilhos++;
        table[first].filhos = (int*)can_fail_realloc(table[first].filhos,sizeof(int)*table[first].nfilhos);
...
void push(int value){
    struct stack *new;
    new = (struct stack*)can_fail_malloc(sizeof(struct stack));
    new->value = value;
    new->next = top;
    top = new;
}
