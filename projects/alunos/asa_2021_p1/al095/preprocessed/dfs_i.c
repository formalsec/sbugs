...
int max = 0;
int min = 0;
int number_total_dominos;
int number_total_relations;
void initialize();
...
int main()
{
  number_total_dominos = new_sym_var(sizeof(int) * 8);
  number_total_relations = new_sym_var(sizeof(int) * 8);
  ...
  for (i = 0; i < number_total_dominos; i++)
  {
    if (table[i].color == White)
    {
      ...
      while (!is_empty())
      {
        ...
        {
          if (!filho_color_white_or_gray(value))
          {
            pop();
            table[value].color = Black;
