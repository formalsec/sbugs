...
void readInput(){
    ...
    if(scanf("%d %d", &n_vertex, &n_edges) == 2){
        vertexArray = can_fail_malloc(sizeof(link) * n_vertex);
        sourcesToFind = can_fail_malloc(sizeof(int) * n_vertex);
    ...
    for(i = 1; i <= n_edges; i++){
        ...
        if(scanf("%d %d", &vertex1, &vertex2) == 2){
            link *vertex = can_fail_malloc(sizeof(link));
...
void findSources(){
    ...
    for(i = 0; i < n_vertex; i++){
        if(sourcesToFind[i] == 0){
            dominoes_to_move++;
            sourcesToUse = can_fail_realloc(sourcesToUse, sizeof(int) * dominoes_to_move);
...
void findLongestPath(){
    ...
    link *visited = can_fail_malloc(sizeof(link));
    ...
    for(path = 0;;){
        ...
        if(path <= 0 && vertex.next == NULL){
            ...
            visited = can_fail_realloc(visited, sizeof(link) * path);
            path--;
        }
