...
struct Queue* createQueue(int max_size) {
    Queuept queue = (struct Queue*)can_fail_malloc(sizeof(struct Queue));
    queue->max_size = max_size;
    queue->front = queue->size = 0;
    queue->back = max_size - 1;
    queue->array = (int*)can_fail_malloc(queue->max_size * sizeof(int));
    return queue;
}
...
struct vertice* newVertice() {
    ...
    verticept v = (struct vertice*)can_fail_malloc(sizeof(struct vertice));
    v->edges = 0;
    v->tmp = 0;
    v->vis_state = 0;
    return v;
...
struct Graph* newGraph (int size) {
    ...
    Graphpt graph = (struct Graph*)can_fail_malloc(sizeof(struct Graph));
    graph->size = size;
    graph->adj_lists = (struct node**)can_fail_malloc(size * sizeof(struct node*));
    graph->v_list = (struct vertice**)can_fail_malloc(size * sizeof(struct vertice*));
...
int* topologicalSort (Graphpt graph) {
    ...
    int* sorted = (int*)can_fail_malloc(graph->size * sizeof(int));
    int i,j=0;
