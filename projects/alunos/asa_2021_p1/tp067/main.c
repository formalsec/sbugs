...
void dominoCracker(int vertexs, int arcs) {
    ...
    int currDomi = 0;
    int* stack = (int*)can_fail_malloc((arcs > 0 ? arcs+1 : 1)*sizeof(int));
    int* topoOrder = (int*)can_fail_malloc((arcs > 0 ? arcs+1 : 1)*sizeof(int));
...
int main(int argc, char** argv) {
    ...
    if( M > 0 && N > 0 ) {
        graph = (Domino**)can_fail_malloc(N*sizeof(Domino*));

        while( i < N ) {
            graph[i] = (Domino*)can_fail_malloc(sizeof(Domino));
            graph[i]->degree = 0;
