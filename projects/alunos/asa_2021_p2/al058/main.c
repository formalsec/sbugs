...
void initGraph(Graph *graph, int vertices) {
    ...
    graph->graphF = (int **) can_fail_malloc(sizeof(int *) * vertices);

    for (i = 0; i < vertices; i++) {
        graph->graphF[i] = (int *) can_fail_malloc(sizeof(int) * vertices);
    }
...
int getShortestPath(Graph *graph, int *path, int *pathLength, char *color, int *parent) {
    ...
    *pathLength = 0;
    ...
    /* Initialize stack */
    int *stack = (int *) can_fail_malloc(sizeof(int) * graph->vertices);
    int stackHead = 0;
...
int computeMinimumCost(Graph *graph) {
    ...
    /* Allocate memory */
    int *path = (int *) can_fail_malloc(sizeof(int) * graph->vertices);
    int pathLength;

    char *color = (char*) can_fail_malloc(sizeof(char) * graph->vertices);
    int *parent = (int*) can_fail_malloc(sizeof(int) * graph->vertices);
