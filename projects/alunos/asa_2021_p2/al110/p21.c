...
GRAPH *createGraph(int num_processes) {
    ...
    graph->nodes = (NODE*) can_fail_malloc(graph->size * sizeof(NODE));
    for (i = 0; i < graph->size; i++) {
        graph->nodes[i].color = 0;
        graph->nodes[i].n_edges = 0;
        graph->nodes[i].edges = (EDGE*) can_fail_malloc(sizeof(EDGE));
    }
    return graph;
}
...
void addEdge(GRAPH *graph, int source, int dest, int weight) {
    ...
    graph->nodes[source].edges = (EDGE*) can_fail_realloc(graph->nodes[source].edges, sizeof(EDGE) * (n_edges_f + 2));
    graph->nodes[dest].edges[n_edges_b].dest = source;
    graph->nodes[dest].edges[n_edges_b].weight = weight;
    graph->nodes[dest].n_edges += 1;
    graph->nodes[dest].edges = (EDGE*) can_fail_realloc(graph->nodes[dest].edges, sizeof(EDGE) * (n_edges_b + 2));
}
...
int fordfulkerson(GRAPH *graph, int s, int t) {
    ...
    int *parent = (int*) can_fail_malloc(graph->size * sizeof(int));
    int *queue = (int *) can_fail_malloc(graph->size * sizeof(int));
