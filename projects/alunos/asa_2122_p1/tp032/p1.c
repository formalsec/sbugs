...
int main()
{
	int allocd = 10000000;
	int problem;
	int *v1 = can_fail_malloc(sizeof(int)*allocd);
	int *v2 = can_fail_malloc(sizeof(int)*allocd);
    int lis;
	int lis_amount;
	int i = -1, j = 0, curr = 0;
	/*
	...
	do{
		...
		if(i > (int)allocd){
			v1 = (int*)can_fail_realloc(v1, 2*allocd);
			allocd *= 2;
		}
		if(c == ' '){
			v1[i++] = curr;
	...
	v1 = can_fail_realloc(v1, sizeof(int)*i); /* cuts of tail of trailing 0s in v1 */
	allocd=100;
	if(problem == 2){
		while(scanf("%d[^\n]", &v2[j]) != EOF){
			if(i > (int)allocd){
				v2 = (int*)can_fail_realloc(v2, 2*allocd);
				allocd *= 2;
			} 
			j++;
		}
	}
	v2 = can_fail_realloc(v2, sizeof(int)*j);
