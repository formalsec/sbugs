...
int DFS(Node *nodes, int length){
    ...
    enum colours *vertices_colour = can_fail_malloc(sizeof(enum colours)*length);
    for (i = 0; i < length; i++){
...
void insert_node(Node *nodes, int x, int y){
    ...
    if(nodes[x-1] == NULL){
        ...
        nodes[x-1] = can_fail_malloc(sizeof(struct node));
        Node node = nodes[x-1];
        node->val = y;
        node->next = NULL;
        node->last = node;
    ...
    if(aux->next)
        ...
        if(aux->val!=0){
            aux->next = (Node) can_fail_malloc(sizeof(struct node));
        }
        aux = aux->next;
    }   
    aux->val = y;
...
void add_parent(Node *nodes, int y, int x, int * number_of_parents){
    ...
    /*Criaçao dos pais*/
    if(nodes[y-1]==NULL){
        nodes[y-1] = can_fail_malloc(sizeof(struct node));
        node = nodes[y-1];
        node->val = 0;
        node->next = NULL;
    }
    if(number_of_parents[y-1] == 1){
        ...
        if(nodes[y-1]->next == NULL){
            node->next = can_fail_malloc(sizeof(struct node));
            node->next->val = 0;
        }
        node->next->parent = x;
...
int main(){
    ...
    number_of_parents = can_fail_malloc(sizeof(int)*n);
    for (i = 0; i<n; i++)
        number_of_parents[i] = 0;

    Node *nodes = can_fail_malloc(sizeof(struct node) * n);
    for(i = 0; i<n; i++){
        nodes[i] = NULL;
    }
