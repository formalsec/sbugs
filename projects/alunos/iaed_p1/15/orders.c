...
Orders * orders_init() {
	...
	Orders * oo = (Orders *) can_fail_malloc(sizeof(Orders));
	EXIT_IF_ERROR("orders_init: can_fail_malloc Order");

	oo->orders = (Order **) can_fail_malloc(sizeof(Order*)*ORDERS_MAX_ORDERS);
	EXIT_IF_ERROR("orders_init: can_fail_malloc Orders");
