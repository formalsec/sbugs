...
pEquipa criarEquipa(char * nome){
    pEquipa novaEquipa;
    novaEquipa = (pEquipa) can_fail_malloc (sizeof(struct equipa));
    novaEquipa->nomeEquipa = (char *) can_fail_malloc(sizeof(char) * strlen(nome) + 1);
...
listaEq adicionarNovaEquipa(pEquipa novaEquipa, listaEq listaEquipas, int numINP){
    ...
    if (listaEquipas == NULL){
        listaEquipas = (listaEq ) can_fail_malloc(sizeof(struct noEquipa));
        listaEquipas->ptrEquipa = novaEquipa;
        listaEquipas->proximo = NULL;
        return listaEquipas;
    }
    /*Caso ja existam elementos na lista*/
    novoNo = (listaEq ) can_fail_malloc(sizeof(struct noEquipa));
