...
pTabela novaTabJogos(int m){
    int i;
    pTabela novaTabela = (pTabela) can_fail_malloc (sizeof(struct entradasJogo));
    novaTabela->numJogos = 0;
    novaTabela->tamanho = m;
    novaTabela->entradaLista = (listaJogo *) can_fail_malloc(sizeof (listaJogo) * m);
    for(i = 0; i < m; i++){
...
void listarJogos(pTabela tabelaJogos, int ordem, int numINP){
    int i, k;
    listaJogo * entradas;
    entradas = (listaJogo * ) can_fail_malloc (sizeof(listaJogo) * tabelaJogos->tamanho);
    k = 0;
