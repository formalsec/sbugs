...
void Stinsert(char* nome_jogo,char* equipa1, char* equipa2, int score1, int score2,int* M,pjogo *st,link* head)
{
  ...
  st[i]=(pjogo)can_fail_malloc(sizeof(struct jogo));
  st[i]->score1 = score1;
  st[i]->score2 =score2;
  st[i]->nome_jogo =(char*)can_fail_malloc(sizeof(char)*(strlen(nome_jogo)+1));
  strcpy(st[i]->nome_jogo,nome_jogo);
  st[i]->equipa1 =(char*)can_fail_malloc(sizeof(char)*(strlen(equipa1)+1));
  strcpy(st[i]->equipa1,equipa1);
  st[i]->equipa2 =(char*)can_fail_malloc(sizeof(char)*(strlen(equipa2)+1));
...
void Htinsert(char* equipa,int vitorias,int* L,pequipa *ht,link* head_2)
{
  ...
  ht[i]=(pequipa)can_fail_malloc(sizeof(struct equipas));
  ht[i]->vitorias = vitorias;
  ht[i]->equipa =(char*)can_fail_malloc(sizeof(char)*(strlen(equipa)+1));
