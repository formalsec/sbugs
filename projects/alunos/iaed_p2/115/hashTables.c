...
TeamsHash* initTeamsHash(int M) {
    ...
    Hash = (TeamsHash*) can_fail_malloc (sizeof(TeamsHash));

    /* Inicializa um vetor de teamNodeP's -> ponteiros para nos de equipas */
    heads = (teamNodeP*)can_fail_malloc(sizeof(teamNodeP)*M);
...
GamesHash* initGamesHash(int M) {
    ...
    Hash = (GamesHash*) can_fail_malloc (sizeof(GamesHash));

    /* Inicializa um vetor de gameNodeP's -> ponteiros para nos de jogos */
    heads = (gameNodeP*)can_fail_malloc(sizeof(gameNodeP)*M);
...
Team** addWinnerTable(Team* x, int* lenP, Team **winnersTable, int* maxC) {
    ...
    if ((*lenP) == 1) {
        ...
        if (*lenP == *maxC){
            ...
            winnersTable = (Team**) can_fail_realloc (winnersTable, sizeof(Team*)*(*maxC)*2);
            (*maxC) = (*maxC)*2;
