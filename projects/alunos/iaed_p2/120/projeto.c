...
int main()
{
    ...
    NodeTeam **headsT = (NodeTeam**) can_fail_malloc(Mod*sizeof(NodeTeam));
    
    /*Allocation of memory for the hash table that will contain all the games
      that will be added to the program*/
    NodeGame **headsG = (NodeGame**) can_fail_malloc(Mod*sizeof(NodeGame));
...
void more_victories(NodeTeam **headsT, int mod, unsigned long int NL)
{
    ...
    teamsMoreWins = (char**) can_fail_malloc(sizeof(char*) * numTeams);
