...
{
    int i;
    Hash_Jogo *HashTable = (Hash_Jogo *)can_fail_malloc(sizeof(Hash_Jogo));
    linkjogo *heads;

    heads = can_fail_malloc(m * sizeof(linkjogo));
    for (i = 0; i < m; i++)
        heads[i] = NULL;
              ...
              int m) /*cria uma estrutura do novo elemento, auxiliar da insertBegin*/
{
    link s, t;
    jogo x;
    x = can_fail_malloc(sizeof(struct jogo));
...
{      /*auxiliar da STinsert permite adicionar novo jogo na lista de uma "fila" da hash, auxiliar da STinsert*/
    linkjogo y;
    jogo x;
    y = (struct nodejogo *)can_fail_malloc(sizeof(struct nodejogo));
...
{
    list *lst;
    lst = (list *)can_fail_malloc(sizeof(list));
    lst->head = NULL;
    lst->tail = NULL;
    return lst;
}
...
void add_last(list *lista, char name[], Hash_Jogo *HashTable, int m)  /*adiciona no final da lista ordenada um jogo recebe nome ponteiro para lista e hash do jogo*/
{
    ...
    lst = (linkordenado *)can_fail_malloc(sizeof(linkordenado));
