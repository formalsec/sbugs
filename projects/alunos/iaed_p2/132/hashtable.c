...
//     p = can_fail_malloc(strlen(s) + 1);  /* allocate memory */
//     if (p != NULL)
...
tnode **init_teams(tnode **teams_tab)
{
    ...
    teams_tab = can_fail_malloc(HASH_KEY * sizeof(tnode));   
...
{
    tnode *new = can_fail_malloc(sizeof(tnode));
...
gnode **init_games(gnode **games_tab)
{
    ...
    games_tab = can_fail_malloc(HASH_KEY * sizeof(gnode));
    for (i = 0; i < HASH_KEY; i++)
        games_tab[i] = NULL;
    return games_tab;
}
...
{
    game *g = can_fail_malloc(sizeof(game));
...
{
    gnode *new = can_fail_malloc(sizeof(gnode));
