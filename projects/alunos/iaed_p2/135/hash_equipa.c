...
link_equipa insereLista_equipa(link_equipa head, pEquipa e){
    ...
    head = can_fail_malloc(sizeof(node_equipa));
    head->next = old_head;
    head->e = e;
    return head;
}
...
hashtable_equipa* cria_hash_equipa(int m){
    int i;
    hashtable_equipa* ht = (hashtable_equipa*) can_fail_malloc(sizeof(hashtable_equipa));
    ht->M = m;
    ht->heads = (link_equipa*) can_fail_malloc(ht->M*sizeof(link_equipa));
...
void nome_equipas_maior_ganhos(hashtable_equipa* ht, 
    ...
    equipas = (char**) can_fail_malloc(sizeof(char*)*n_equipas);
