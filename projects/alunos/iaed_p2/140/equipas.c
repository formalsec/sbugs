...
Equipa newEquipa(char* buffer) {
  Equipa final = can_fail_malloc(sizeof(struct equipa));

  final->nome = can_fail_malloc(sizeof(char*)*(strlen(buffer)+1));
...
link* EhashInit(){
  int i;
  link* heads = can_fail_malloc(HASH_SIZE*sizeof(link));
    ...
    for (t = hashEquipas[i]; t != NULL; t=t->next) {
      ...
      if (t->equipa->ganhos == *maior) {
        *count += 1; /* incrementa o nr de jogos */
        lista = can_fail_realloc(lista, sizeof(Equipa)*(*count)); /* aumenta a lista */
      ...
      /* se o nr de jogos ganhos for superior cria uma lista nova e adiciona a equipa */
        ...
        *count = 1;
        free(lista);
        lista = can_fail_malloc(sizeof(Equipa)); /* cria uma lista nova */
        lista[*count-1] = t->equipa;
      }
    }
  }
