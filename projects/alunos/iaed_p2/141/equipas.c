...
Vetor inicializaVetor(int M) {
    Vetor a = can_fail_malloc(sizeof(struct vetor));
    a->vetor = can_fail_malloc(M * sizeof(struct equipa));
...
void insereVetor(Vetor a, Equipa eq) {
    /* caso o vetor esteja cheio, expandir vetor */
        ...
        a->vetor = can_fail_realloc(a->vetor, novo_tam * sizeof(struct equipa));
    }
    /* adiciona equipa */
    a->vetor[a->N++] = eq;
}
...
Hashtable_eq inicializaHT_eq(int max){
    Hashtable_eq ht = can_fail_malloc(sizeof(struct hashtable_eq));
    Equipa * tab = can_fail_malloc(max*sizeof(Equipa));
    int i;
...
void expandeHT_eq(Hashtable_eq ht) {
    ...
    Equipa * tab = can_fail_malloc(novo_tam*sizeof(Equipa)); /* nova tabela */
