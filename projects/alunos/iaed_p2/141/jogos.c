...
Lista criaLista(){
    Lista new = can_fail_malloc(sizeof(struct lista));
    new -> primeiro = NULL;
    new -> ult = NULL;
    return new;
}
...
void insereLista(Lista l, char * c){
    Node new = can_fail_malloc(sizeof(struct node));
    new -> c = c;
    new -> prox = NULL;
...
Hashtable_j inicializaHT_j(int max){
    Hashtable_j ht = can_fail_malloc(sizeof(struct hashtable_j));
    Jogo * tab = can_fail_malloc(max*sizeof(Jogo));
    int i;
...
void expandeHT_j(Hashtable_j ht) {
    ...
    Jogo * tab = can_fail_malloc(novo_tam*sizeof(Jogo)); /* nova tabela */
