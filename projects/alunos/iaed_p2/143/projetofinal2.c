...
int main(){
    ...
    Equipas = (Hash_Equipa*) can_fail_malloc (sizeof(Hash_Equipa));
    ...
    Jogos = (Hash_Jogos*) can_fail_malloc (sizeof(Hash_Jogos));
...
Hash_Equipa *add_nova_equipa(int NL, Hash_Equipa *hash){
    ...
    if (Equipa_search(hash->tam_hash, nome_equipa, hash->equipas) == NULL){
        p = (equipa) can_fail_malloc (sizeof(struct Equipa)); /* alocar memoria para uma equipa */
        p->nome = (char*) can_fail_malloc(sizeof(char)*(strlen(nome_equipa) + 1)); /* alocar memoria para o nome da equipa */
...
Hash_Jogos *add_novo_jogo(int NL, Hash_Jogos *hash_j, Hash_Equipa *hash_e, equipa *e){
    ...
    if (Jogo_search(hash_j->tam_hash, nome_jogo, hash_j->jogos) == NULL){
        ...
        if (e1 != NULL && e2 != NULL){
            ...
            p = (jogo) can_fail_malloc (sizeof(struct Jogo)); /* alocar memoria para um jogo */
            p->nome = (char*) can_fail_malloc(sizeof(char)*(strlen(nome_jogo) + 1)); 
...
void todos_jogos(int NL, Hash_Jogos *J, jogo *j){
    ...
    v_jogos = (jogo*) can_fail_malloc(sizeof(jogo) * J->numero_jogos); /* alocar memoria para o vetor v_jogos */
...
void equipas_ganharam_mais_jogos(int NL, Hash_Equipa *E, equipa *e){
    ...
    vetor_strings = (char**) can_fail_malloc (sizeof(char*) * s);  
    ...
    for(i = 0; i < E->tam_hash; i++){
        if(e[i]){
            if(e[i]->n_vitorias == max_vitorias){
                ...
                vetor_strings[t] = (char*) can_fail_malloc(sizeof(char) * strlen(e[i]->nome) + 1);
