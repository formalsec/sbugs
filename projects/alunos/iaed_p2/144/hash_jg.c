...
{
  lJogo novo = (lJogo) can_fail_malloc(sizeof(struct nodulos));
  novo->jogo= jogo;
  novo->prox = inicio;
...
{ 
  ...
  lJogo* jogos = (lJogo*) can_fail_malloc(m*sizeof(lJogo));
...
{
	LDupla lista = can_fail_malloc(sizeof(struct nodulo_L));
...
{
	ligacao novo = can_fail_malloc(sizeof(struct noduloJ));
