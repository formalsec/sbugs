...
game_n *a_adiciona_jogo(game_n **hash_table, team_n **hash_teams, game_n *head, int nl){
  ...
  int score1 = 0;
  int score2 = 0;
  char *nome = can_fail_malloc(sizeof(char) * (MAX_NAME + 1));
  char *equipa1 = can_fail_malloc(sizeof(char) * (MAX_NAME + 1));
  char *equipa2 = can_fail_malloc(sizeof(char) * (MAX_NAME + 1));

  scanf(" %[^:\n]:%[^:\n]:%[^:\n]:%d:%d", nome, equipa1, equipa2, &score1, &score2);
  t = (game_n *)can_fail_malloc(sizeof(game_n));

  /* alocacoes de memoria para strings */
  t->nome = (char *)can_fail_malloc(sizeof(char) * strlen(nome) + 1);
  strcpy(t->nome, nome);
  t->equipa1 = (char *)can_fail_malloc(sizeof(char) * strlen(equipa1) + 1);
  strcpy(t->equipa1, equipa1);
  t->equipa2 = (char *)can_fail_malloc(sizeof(char) * strlen(equipa2) + 1);
...
team_n *A_adiciona_equipa(team_n **hash_teams, team_n *head_team, int nl){
  ...
  int id;
  char *name = can_fail_malloc(sizeof(char) * (MAX_NAME + 1));

  scanf(" %[^:\n]", name);
  t = (team_n *)can_fail_malloc(sizeof(team_n));
  /* alocacao de memoria pelo facto de ser uma string */
  t->name = (char *)can_fail_malloc(sizeof(char) * strlen(name) + 1);
  strcpy(t->name, name);
  t->wins =0;
  t->next = NULL;
  t->lista = NULL;
...
void g_equipas_vencedoras(team_n *head_team, int nl, int maximo, int n_maximos){
  ...
  int indice = 0;
  char **lista;
  lista = can_fail_malloc(sizeof(char *) * n_maximos);
