    ...
    for (i = 0; i < (TAB_SIZE/2); i++) {
        ...
        {            
            if (aux->equipa->vitorias == max_score) {
                ...
                nomes[size] = (pt_nome) can_fail_malloc(sizeof(struct nome));
                nomes[size]->nome = (char*) can_fail_malloc(comp * sizeof(char));
...
int main()
{
    ...
    pt_nome nomes_jogos = (pt_nome) can_fail_malloc(sizeof(struct nome));

    nomes_jogos->next = NULL;

    jogos = (link_jogo*) can_fail_malloc(TAB_SIZE * sizeof(struct node_jogo));   
    jogos = init_jogos(jogos);

    equipas = (link_equipa*) can_fail_malloc((TAB_SIZE / 2) * sizeof(struct node_equipa)); 
