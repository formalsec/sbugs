...
pJogo cria_jogo(char *nome, char *equipa1, char *equipa2,unsigned int score[2])
{
    ...
    pJogo jogo = (pJogo)can_fail_malloc(sizeof(struct jogo));
    

    /* aloca memoria para os nomes das equipas e do jogo, e copia-os */
    jogo -> nome = (char*)can_fail_malloc(sizeof(char)*(1 + strlen(nome)));
    strcpy(jogo -> nome, nome);

    jogo -> equipa1 = (char*)can_fail_malloc(sizeof(char)*(1 + strlen(equipa1)));
    strcpy(jogo -> equipa1, equipa1);

    jogo -> equipa2 = (char*)can_fail_malloc(sizeof(char)*(1 + strlen(equipa2)));
