...
team_ptr NewTeamLink(char * team_name_var){
    ...
    team_ptr x = can_fail_malloc(sizeof(struct team));

    x->team_name = can_fail_malloc(sizeof(char) *(strlen(team_name_var) +1));
...
link NewLink(char* game_name_var, char* team1_var, char* team2_var, 
    ...
    x->this_game = (game_ptr) can_fail_malloc(sizeof(struct game));
    
    x->this_game->game_name = (char*) can_fail_malloc(sizeof(char) * (strlen(game_name_var) + 1));
    strcpy(x->this_game->game_name, game_name_var);

    x->this_game->team1 = (char*) can_fail_malloc(sizeof(char) * (strlen(team1_var) + 1));
    strcpy(x->this_game->team1, team1_var);
    
    x->this_game->team2 = (char*) can_fail_malloc(sizeof(char) * (strlen(team2_var) + 1));
...
link* Init(link * heads, int size_table){
    ...
    int i;
    if(heads == NULL){
        heads = (link*) can_fail_malloc(sizeof(link) * size_table);
