...
Jogo * mk_jogo(char nome[MAXNOME], char eq1[MAXEQUIPA],
    ...
    Jogo * novoJogo = can_fail_malloc(sizeof(Jogo));
    novoJogo->nome = can_fail_malloc(sizeof(char)*(strlen(nome)+1));
    memset(novoJogo->nome,'\0', sizeof(char)*(strlen(nome)+1));
    strcpy(novoJogo->nome, nome);
    novoJogo->equipa1 = can_fail_malloc(sizeof(char)*(strlen(eq1)+1));
    memset(novoJogo->equipa1,'\0', sizeof(char)*(strlen(eq1)+1));
    strcpy(novoJogo->equipa1, eq1);
    novoJogo->equipa2 = can_fail_malloc(sizeof(char)*(strlen(eq2)+1));
...
list_jogo * mk_list_jogo(){
    list_jogo * newL = can_fail_malloc(sizeof(list_jogo));
    newL->head = NULL;
    newL->last = NULL;
    return newL;
}
...
void add_last_jogo(list_jogo * l, Jogo * j){
    node_jogo * newNode = can_fail_malloc(sizeof(node_jogo));
    newNode->next = NULL;
    newNode->jogo = j;
