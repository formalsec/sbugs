...
link2* STinitE(int m){
    ...
    int i;
    link2 *hashtable = (link2*)can_fail_malloc(m*sizeof(link2));
    for (i = 0; i < m; i++){
        hashtable[i] = NULL;
    } 
    return hashtable;
...
link1* STinitJ(int m){
    ...
    int i;
    link1 *hashtable = (link1*)can_fail_malloc(m*sizeof(link1));
    for (i = 0; i < m; i++){
        hashtable[i] = NULL;
    } 
    return hashtable;
...
link2 NEWE(char* buffer){
    ...
    link2 x = (link2)can_fail_malloc(sizeof(struct node2));
    x->equipa = (char*)can_fail_malloc(sizeof(char)*(strlen(buffer)+1));
...
link1 NEWJ(char* buffer1, char* buffer2, char* buffer3, int n, int m){
    ...
    link1 x = (link1)can_fail_malloc(sizeof(struct node1)); 
    x->Jogo = (pJogo)can_fail_malloc(sizeof(jogo));
    x->Jogo->nome = (char*)can_fail_malloc(sizeof(char)*(strlen(buffer1)+1));
    strcpy(x->Jogo->nome, buffer1);
    x->Jogo->equipa1 = (char*)can_fail_malloc(sizeof(char)*(strlen(buffer2)+1));
    strcpy(x->Jogo->equipa1, buffer2);
    x->Jogo->equipa2 = (char*)can_fail_malloc(sizeof(char)*(strlen(buffer3)+1));
