...
lista_jogos inicia_lista_jogos(){
    ...
    lista_jogos lj = (lista_jogos)can_fail_malloc(sizeof(struct jlist));
...
pjogo novo_jogo(char *buffer, pequipa pe1, pequipa pe2, int score1, int score2){
    ...
    pjogo x = (pjogo) can_fail_malloc(sizeof(struct jogo));
...
void inserir_jogo(lista_jogos lj, char* texto, pequipa pe1, pequipa pe2,
    ...
    jlink x = (jlink)can_fail_malloc(sizeof(struct jnode));
...
ht_jogos iniciar_tabela_jogos(){
    ...
    ht_jogos ht_j = (ht_jogos)can_fail_malloc(sizeof(struct hashtable_jogos));
...
jlink* iniciar_hash_jogos(int M) {
    int i;
    /* reservo espaco*/
    jlink * hash_jogos = (jlink*)can_fail_malloc(M*sizeof(jlink));
    for (i = 0; i < M; i++)
