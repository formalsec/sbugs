...
lista *inicializa_lista() {
	lista* tmp = (lista*) can_fail_malloc(sizeof(lista));
	tmp->head = NULL;
	tmp->last = NULL;
	return tmp;
}
...
jogo inicializa_jogo(jogo st1, int m) {
    int i; 
    st1 = (jogo)can_fail_malloc(m*sizeof(struct perso));
    st1->tamanho=m;
    st1->elementos=0;
    st1->v = (pPerson*)can_fail_malloc(m*sizeof(pPerson));
...
jogo insere_jogo(jogo st, lista *l, char*nome, char *equipa1, char *equipa2, int score1, int score2) {
    ...
    st->v[i] = (pPerson)can_fail_malloc(sizeof(struct person));
    st->v[i]->score1 = score1;
    st->v[i]->score2 = score2;
    st->v[i]->eliminado = 0;
    st->v[i]->nome =(char*)can_fail_malloc(sizeof(char)*(strlen(nome)+1));
    strcpy(st->v[i]->nome, nome);
    st->v[i]->equipa1 =(char*)can_fail_malloc(sizeof(char)*(strlen(equipa1)+1));
    strcpy(st->v[i]->equipa1, equipa1);
    st->v[i]->equipa2 =(char*)can_fail_malloc(sizeof(char)*(strlen(equipa2)+1));
