...
Team createTeam(char *name){
  ...
  /*name = format(name);*/
  len = strlen(name);
  newTeam.name = can_fail_malloc(len + 1 * sizeof(char));
  if(!newTeam.name){
    perror("createTeam Failed can_fail_malloc");
  }
...
Teams *createTeams(char *name){
  Teams *new;
  new = can_fail_malloc(sizeof(Teams));
  if(!new){
    perror("createTeams Failed can_fail_malloc");
  }
...
void printBest(Teams *teamsList, int commandNumber){
  ...
  if(teamsList != NULL){
    ...
    list = can_fail_malloc(num * sizeof(char*));
    while(teamsList->nextTeam != NULL){
      if(teamsList->team.victories == max){
        list[i] = can_fail_malloc(MAX_STRING_SIZE * sizeof(char));
