...
char **separate(char *string, int separator){
  int i, j, x, len;
  char **temp;
  temp = can_fail_malloc(separator*sizeof(char *));
  if(!temp){
    perror("separate Failed can_fail_malloc");
  }
  for(j = 0; j < separator; j++){
    temp[j] = can_fail_malloc(MAX_STRING_SIZE * sizeof(char));
    if(!temp[j]){
      perror("separate Failed can_fail_malloc");
    }
  }
...
Game *createGame(char* gameName, Team team1, Team team2, int scoreTeam1, int scoreTeam2){
  int len;
  Game *newGame = can_fail_malloc(sizeof(Game));
  len = strlen(gameName);
  newGame->name = can_fail_malloc((len + 1) *sizeof(char));
  newGame->teams = can_fail_malloc(2*sizeof(Team));
  newGame->teamsScores = can_fail_malloc(2*sizeof(int));
  if(!newGame || !newGame->name || !newGame->teams || !newGame->teamsScores){
    perror("createGame Failed can_fail_malloc");
  }
...
void searchGame(Game *game, char *gameName, int commandNumber){
  ...
  char *name;
  len = strlen(gameName);
  name = can_fail_malloc(len + 1 * sizeof(char));
