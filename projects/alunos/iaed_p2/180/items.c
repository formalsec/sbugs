...
linkEquipa nova_equipa (char *nome) {
    ...
    linkEquipa nv_eq = (linkEquipa)can_fail_malloc(sizeof(struct nodeEquipa));
    nv_eq->nome = (char*)can_fail_malloc(sizeof(char)*(strlen(nome)+1));
...
linkJogo novo_jogo (char *nome, char *eq1, char *eq2, int s1, int s2) {
    ...
    linkJogo nv_jg = (linkJogo)can_fail_malloc(sizeof(struct nodeJogo));
    nv_jg->nome = (char*)can_fail_malloc(sizeof(char)*(strlen(nome)+1));
    strcpy(nv_jg->nome, nome);
    nv_jg->eq1 = (char*)can_fail_malloc(sizeof(char)*(strlen(eq1)+1));
    strcpy(nv_jg->eq1, eq1);
    nv_jg->eq2 = (char*)can_fail_malloc(sizeof(char)*(strlen(eq2)+1));
