...
{
    NodeE *t = cabeca;
    NodeE *x = can_fail_malloc(sizeof(NodeE));
...
{
    NodeE **hash_table;
    hash_table = can_fail_calloc(TAM_HASH, sizeof(NodeE*));
...
{
    NodeJ *x = can_fail_malloc(sizeof(NodeJ));
    x->nome = can_fail_malloc(sizeof(char)*strlen(nome)+1);
    strcpy(x->nome, nome);
    x->casa = can_fail_malloc(sizeof(char)*strlen(casa)+1);
    strcpy(x->casa, casa);
    x->fora = can_fail_malloc(sizeof(char)*strlen(fora)+1);
    strcpy(x->fora, fora);
    x->resCasa = resCasa;
    x->resFora = resFora;
    x->next_lista = NULL;
...
{
    NodeJ **hash_table;
    hash_table = can_fail_calloc(TAM_HASH, sizeof(NodeJ*));
    /*inicializa a tabela vazia*/
    return hash_table;
}
