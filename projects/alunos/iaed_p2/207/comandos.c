...
PRINCIPAL init_stPrincipal(PRINCIPAL p) {
	int i;
	p->hashJogo = can_fail_malloc(sizeof(JOGO)*VETOR_HASH_SZ);		/* Hash Table que contem os jogos */
	p->hashEquipa = can_fail_malloc(sizeof(EQUIPA)*VETOR_HASH_SZ);		/* Hash Table que contem as equipas */	
	p->lista_equipas = can_fail_malloc(sizeof(EQUIPA)*VETOR_HASH_SZ);		/* Vetor que contem as equipas */
