...
void matchInit(){
  int i;
  matchHeads = (matchNode*)can_fail_malloc(MAX*sizeof(matchNode));
  for (i = 0; i < MAX; i++){
    matchHeads[i] = NULL;
  }
}
...
void matchInsert(Match match){
  ...
  if (matchHeads[i] == NULL){
    matchHeads[i] = (matchNode)can_fail_malloc(sizeof(struct matchnode));
    matchHeads[i]->match = match;
    matchHeads[i]->next = NULL;
  }
  else{
    matchNode t, prev;
    matchNode newNode = (matchNode)can_fail_malloc(sizeof(struct matchnode));
    newNode->match = match;
    newNode->next = NULL;
...
matchNode newMatchNode(Match match, matchNode next){
	matchNode x = (matchNode) can_fail_malloc(sizeof(struct matchnode));
	x->match = match;
  x->next = next;
  return x;
}
