...
void teamInit(){
  int i;
  teamHeads = (teamNode*)can_fail_malloc(MAXTEAM*sizeof(teamNode));
  for (i = 0; i < MAXTEAM; i++){
    teamHeads[i] = NULL;
  }
}
...
void teamInsert(Team team){
  ...
  if(teamHeads[i] == NULL){
    teamHeads[i] = (teamNode)can_fail_malloc(sizeof(struct teamnode));
    teamHeads[i]->team = team;
    teamHeads[i]->next = NULL;
  }
  else{
    teamNode newNode = (teamNode)can_fail_malloc(sizeof(struct teamnode));
    teamNode t, prev;
    newNode->team = team;
...
teamNode newTeamNode(Team t, teamNode next){
	teamNode x = (teamNode) can_fail_malloc(sizeof(struct teamnode));
  x->team = t;
  x->next = next;
  return x;
}
