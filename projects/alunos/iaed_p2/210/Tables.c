...
void tableEInit(int max) {
	...
	tableEquipas = can_fail_malloc(Mequipas*sizeof(equipa));
...
void tableJInit(int max) {
	...
	tableJogos = can_fail_malloc(Mjogos*sizeof(element));
	for (i = ZERO; i< Mjogos; i++) {
		tableJogos[i] = can_fail_malloc(sizeof(struct element));
...
void initArrayE(int max) {
	int i;
	sizeArrayE = DOIS*max;
	listaEquipas = can_fail_malloc(sizeArrayE * sizeof(equipa));
...
void initArrayJ(int max) {
	int i;
	sizeArrayJ = DOIS*max;
	listaJogos = can_fail_malloc(sizeArrayJ * sizeof(jogo));
