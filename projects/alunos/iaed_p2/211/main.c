...
void cmd_g(int *NL, linked_list_teams *linked_teams){
    if (linked_teams->head){
        ...
        node_team **list_vic = can_fail_malloc(sizeof(node_team) * cap);
        node_team **aux_l_v;
        node_team *n = linked_teams->head;
        while (n){
            if (n->victories == v){
                if (i == cap) {
                    list_vic = can_fail_realloc(list_vic, sizeof(node_team) * cap * 2);
                    cap*=2;
                }
                list_vic[i++] = n;
            }
        ...
        last_index = i - 1;
        aux_l_v = can_fail_malloc(sizeof(node_team) * (last_index + 1));
...
int main(){
    char c;
    int *NL = can_fail_malloc(sizeof(int));
    hash_table_matches *hash_matches = can_fail_malloc(sizeof(hash_table_teams));
    hash_table_teams *l_teams = can_fail_malloc(sizeof(hash_table_matches));
    linked_list_matches * linked_matches = can_fail_malloc(sizeof(linked_list_matches));
    linked_list_teams * list_t = can_fail_malloc(sizeof(linked_list_teams));
