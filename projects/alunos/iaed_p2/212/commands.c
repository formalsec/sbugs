...
void cmd_a(link *head, link *last, Tree **g, Tree2 **t, int nl){
	...
	game.nome = can_fail_malloc(sizeof(char)*(strlen(line[0])+1));
	strcpy(game.nome, line[0]);

	game.equipa1 = can_fail_malloc(sizeof(char)*(strlen(line[1])+1));
	strcpy(game.equipa1, line[1]);

	game.equipa2 = can_fail_malloc(sizeof(char)*(strlen(line[2])+1));
...
void cmd_A(Tree2 **g, int nl){
	...
	name = can_fail_malloc(sizeof(char)*(strlen(line)+1));
	strcpy(name, line);
