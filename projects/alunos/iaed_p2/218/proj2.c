...
void addGame(char command[])
{
	...
	int tIndex1;
	int tIndex2;
	struct gNode* newptr = can_fail_malloc(sizeof(gNode));
	struct Game* temp = can_fail_malloc(sizeof(Game));
	/* game name */
	token = strtok(command, delim);
	temp->name = can_fail_malloc(sizeof(char)*(strlen(token)+1));
	strcpy(temp->name, token);
	/* team 1 name */ 
	token = strtok(NULL, delim);
	temp->team1 = can_fail_malloc(sizeof(char)*(strlen(token)+1));
	strcpy(temp->team1, token);
	/* team 2 name */ 
	token = strtok(NULL, delim);
	temp->team2 = can_fail_malloc(sizeof(char)*(strlen(token)+1));
...
void addTeam(char command[])
{
	...
	int teamExists = 0;
	struct tNode* newptr = can_fail_malloc(sizeof(tNode));
	struct Team* temp = can_fail_malloc(sizeof(Team));
	temp->name = can_fail_malloc(sizeof(char)*(strlen(command)+1));
...
void findBigWinners()
{
	...
	if(teamsExist == 1)
	{
		...
		bigWinners = can_fail_malloc(sizeof(struct tNode*)*empates);
