...
void comando_a(struct game** hash_games,struct team** hash_teams,struct game** lastGame,struct game** firstGame){
  char* name = can_fail_malloc(sizeof(char) * MAXINPUT);
  char* team1 = can_fail_malloc(sizeof(char) * MAXINPUT);
  char* team2 = can_fail_malloc(sizeof(char) * MAXINPUT);
  int s1,s2;
...
void comando_g(struct team** hash_teams){
  struct team* t;
  char** teams = can_fail_malloc(sizeof(char*)*500);
  int i;
  int n;
  int max = findMaxWins(hash_teams);
  n = 0;
  for(i=0;i<500;i++){
    teams[i] = can_fail_malloc(sizeof(char)*300);
  }
  if(max==-1){
    for(i=0;i<500;i++){
      free(teams[i]);
...
int main(){
  char* control = can_fail_malloc(sizeof(char)*6);
  char* input = can_fail_malloc(sizeof(char)* MAXINPUT * 3);
  int end;
  struct game** hash_games = can_fail_malloc(sizeof(struct game*)*HASHSIZE);
  struct team** hash_teams = can_fail_malloc(sizeof(struct team*)*HASHSIZE);
