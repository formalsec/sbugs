...
listJogo* mkListJogo(){
	listJogo* l = can_fail_malloc(sizeof(listJogo));
	l->head = l->last = NULL;
	return l;
}
...
listEquipa* mkListEquipa(){
	listEquipa* l = can_fail_malloc(sizeof(listEquipa));
	l->head = l->last = NULL;
	return l;
}
...
void addNodeJogo(listJogo* l, const char* _nomeJogo, const char* _nomeEquipa1, const char* _nomeEquipa2, int _resultado1, int _resultado2){
	nodeJogo* n = can_fail_malloc(sizeof(nodeJogo));
	n->previous = l->last;
	n->next = NULL;
	n->nomeJogo = can_fail_malloc(sizeof(char)*(strlen(_nomeJogo)+1));
	strcpy(n->nomeJogo, _nomeJogo);	/* preenche o campo nomeJogo */
	n->nomeEquipa1 = can_fail_malloc(sizeof(char)*(strlen(_nomeEquipa1)+1));
	strcpy(n->nomeEquipa1, _nomeEquipa1);	/* preenche o campo nomeEquipa1 */
	n->nomeEquipa2 = can_fail_malloc(sizeof(char)*(strlen(_nomeEquipa2)+1));	
...
void addNodeEquipa(listEquipa* l, const char* _nomeEquipa){
	nodeEquipa *n = can_fail_malloc(sizeof(nodeEquipa));
	n->previous = l->last;
	n->next = NULL;
	n->nomeEquipa = can_fail_malloc(sizeof(char)*(strlen(_nomeEquipa)+1));
