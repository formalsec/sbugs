...
Jogo *CriaJogo(char input_separado[MAX_PARAM][MAX_STR]){
    ...
    jogo -> equipaX = (char *) can_fail_malloc((strlen(equipaX)+1)*sizeof(char));
    strcpy(jogo -> equipaX, equipaX);

    jogo -> equipaY = (char *) can_fail_malloc((strlen(equipaY)+1)*sizeof(char));
...
Equipa *CriaEquipa(char input_separado[MAX_PARAM][MAX_STR]){
    ...
    equipa -> nome = (char *) can_fail_malloc((strlen(nome)+1)*sizeof(char));
...
void OrdenaEquipas(int NL, Lista *lista){
    ...
    while(equipa_aux){
        if(equipa_aux -> jogos_ganhos == mais_jogos_ganhos){
            copia = can_fail_malloc(sizeof(Equipa));

            copia -> nome = (char *) can_fail_malloc((strlen(equipa_aux -> nome)+1)*sizeof(char));
