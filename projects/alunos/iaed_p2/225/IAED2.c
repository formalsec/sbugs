...
{
	linkG x = can_fail_malloc(sizeof(nodeG));
	x->game = g;
	x->next = NULL;
	return x; 
}
...
{
	linkT x = can_fail_malloc(sizeof(nodeT));
	x->team = t;
	x->next = NULL;
	return x;
}
...
void a(char *gName, char *team1, char *team2, int score1, int score2)
{
	...
	newG = (Game*)can_fail_malloc(sizeof(Game));
	...
	else
	{
		...
		newG->gName = (char*)can_fail_malloc(sizeof(char) * (p+1));
...
void A(char *name)
{
	...
	newT = (Team*)can_fail_malloc(sizeof(Team));
	...
	else
	{
		...
		newT->tName = (char*)can_fail_malloc(sizeof(char) * (p+1));
...
void g()
{
	...
	int howMany =0;
	int most_wins=0;
	t = (Team*)can_fail_calloc(1,sizeof(Team));

	while(i != NULL){
		if(i->team->wins > most_wins || (i->team->wins==0 && howMany==0))
		{

			t = (Team*)can_fail_realloc(t, sizeof(Team));
			t[0] = *i->team;
		...
		{
			howMany++;
			aux_x = (Team*)can_fail_realloc(t, sizeof(Team)*howMany);
			if(aux_x)
			{
				t = aux_x;
			}
	...
	t_aux = (char**)can_fail_malloc(sizeof(char*)*howMany);
	for(k=0; k < howMany; k++)
	{
		t_aux[k] = (char*)can_fail_malloc(sizeof(char)*strlen((t+k)->tName)+1);
	...
	for(k=0; k < howMany; k++)
	{
		for(j=k+1; j < howMany; j++)
		{
			...
			aux_aux = (char*)can_fail_malloc(sizeof(char)*strlen(t_aux[j])+1);
			if(strcmp(t_aux[k], t_aux[j])>0)
			{
				strcpy(aux_aux,t_aux[j]);
				t_aux[j] = (char*)can_fail_realloc(t_aux[j], sizeof(char)*strlen(t_aux[k])+1);
				strcpy(t_aux[j],t_aux[k]);
				t_aux[k] = (char*)can_fail_realloc(t_aux[k], sizeof(char)*strlen(aux_aux)+1);
...
int main()
{
	...
	name = (char*)can_fail_malloc(sizeof(char)*MAX_NAME);
	team2 = (char*)can_fail_malloc(sizeof(char)*MAX_NAME);
	team1 = (char*)can_fail_malloc(sizeof(char)*MAX_NAME);
