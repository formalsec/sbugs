...
Data data_init(){
	Data data = can_fail_malloc(sizeof(struct data));
...
/* Changes a games score returning the updated data */ 
...
void printBest(Data data){
	...
	if(n > 0){
		v = can_fail_malloc(n * sizeof(char*));
		for(i = 0; i < n;i++)
			v[i] = can_fail_malloc(TEAMSIZE*sizeof(char));
