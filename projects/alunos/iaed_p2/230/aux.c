...
void createNewGame(List *node){
    node->data = can_fail_malloc(sizeof(Game));
...
void insertInfoGame(Game *gameData){
    gameData->name = can_fail_malloc(sizeof(char) * strlen(game) + 1);
    strcpy(gameData->name, game);

    gameData->team1 = can_fail_malloc(sizeof(char) * strlen(team1) + 1);
    strcpy(gameData->team1, team1);

    gameData->team2 = can_fail_malloc(sizeof(char) * strlen(team2) + 1);
...
void insertListGames(Game *gameData){
    ...
    if(!(numGames % MEM_SPACE) && (sizeListGames-numGames) < MEM_SPACE){
        listGames = can_fail_realloc(listGames, (sizeListGames + MEM_SPACE) * sizeof(char*));
...
void createNewTeam(List *node){
    node->data = can_fail_malloc(sizeof(Team));

    ((Team*)node->data)->name = can_fail_malloc(sizeof(char) * strlen(team1) + 1);
