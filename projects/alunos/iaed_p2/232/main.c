...
void add_game(char* name, char* team_1, char* team_2, int score_1, int score_2,
    ...
    if ((G_Node *) locate_node(name, (void *) g_hashtab))
        ...
        G_Node *hash_node = can_fail_malloc(sizeof(G_Node));
        G_Node *list_node = can_fail_malloc(sizeof(G_Node));

        new_game->name = can_fail_malloc(sizeof(char) * strlen(name) + 1);
...
void add_team(char* team_name, T_Node **t_hashtab, T_Node **t_head, int cmd_cnt) {
    if (!locate_node(team_name, (void *) t_hashtab)) {
        T_Node* team_hash_node = can_fail_malloc(sizeof(T_Node));
        T_Node* team_list_node = can_fail_malloc(sizeof(T_Node));
...
int main() {
    ...
    g_hashtab = can_fail_calloc(HASHTABLE_SIZE, sizeof(G_Node *));
    t_hashtab = can_fail_calloc(HASHTABLE_SIZE, sizeof(T_Node *));
    g_head = g_tail = NULL;
