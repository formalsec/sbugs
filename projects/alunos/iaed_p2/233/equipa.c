...
void createHash(){
	int i;
	nr_equipas = 0;
	hash_e = can_fail_malloc(sizeof(equipa*)*tamanho_hash);
...
int A(){
	...
	hash_e[i] = can_fail_malloc(sizeof(equipa));
	hash_e[i]->nome = can_fail_malloc(sizeof(char)*(strlen(nome)+1));
...
void g(){
	...
	if(nr_equipas > 0){
		...
		equipa **aux = can_fail_malloc(sizeof(equipa*)*nr_equipas);
