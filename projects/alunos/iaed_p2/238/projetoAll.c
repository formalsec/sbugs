...
which are nodes containing information about games and teams, by order of insertion */
    ...
    games = can_fail_malloc(HASH_GAMES_SIZE * sizeof(g_node*));
    for(i = 0; i < HASH_GAMES_SIZE; i++)
        games[i] = NULL;

    teams = can_fail_malloc(HASH_TEAMS_SIZE * sizeof(t_node*));
...
game* newGame(char *name, char *team1, char *team2, int score1, int score2){
    ...
    game *pointer_game = (game*) can_fail_malloc(sizeof(struct game));

    pointer_game->name = (char*) can_fail_malloc((strlen(name)+1) * sizeof(char));
    strcpy(pointer_game->name, name);
    pointer_game->team1 = (char*) can_fail_malloc((strlen(team1)+1) * sizeof(char));
    strcpy(pointer_game->team1, team1);
    pointer_game->team2 = (char*) can_fail_malloc((strlen(team2)+1) * sizeof(char));
...
team* newTeam(char *name){
    ...
    team *pointer_team = (team*) can_fail_malloc(sizeof(struct team));

    pointer_team->name = (char*) can_fail_malloc((strlen(name)+1) * sizeof(char));
...
g_node* insertGameNode(g_node *head, char *name, char *team1, char *team2, int score1, int score2){
    ...
    pointer_g_node = can_fail_malloc(sizeof(struct g_node));
...
t_node* insertTeamNode(t_node *head, char *name){
    ...
    pointer_t_node = can_fail_malloc(sizeof(struct t_node));
    /* the new head starts off as the previous head, and will only be changed if the linked list is empty */
    new_head = head;
...
void addGameStorage(game_slot **game_storage_head, char *name){
    ...
    pointer_slot = can_fail_malloc(sizeof(game_slot));
    pointer_slot->name = (char*) can_fail_malloc((strlen(name)+1) * sizeof(char));
...
void addTeamStorage(team_slot **team_storage_head, team *pointer_team){
    ...
    pointer_slot = can_fail_malloc(sizeof(team_slot));
...
void listWinners(team_slot *team_storage_head, int NL){
    ...
    {   
        ...
        array = can_fail_malloc(number_winners * sizeof(char*));
        printf("%d Melhores %d\n", NL, max_wins);
