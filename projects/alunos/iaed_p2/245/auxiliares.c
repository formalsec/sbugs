...
hashtable_equipas *criar_equipas() {
    int i;
    hashtable_equipas *hashtable = can_fail_malloc(sizeof(hashtable_equipas) * 1);
    hashtable->entradas = can_fail_malloc(sizeof(Equipa) * TAMANHO);
...
Equipa *ht_equipas_aloca(char *key) {
    Equipa *p_equipa = can_fail_malloc(sizeof(Equipa));

    p_equipa->nome = can_fail_malloc((strlen(key) + 1));
    p_equipa->next = NULL;
    p_equipa->prev = NULL;
...
Lista_jogos *criar_lista_j() {
    Lista_jogos *lista = can_fail_malloc(sizeof(Lista_jogos) * 1);
    lista->head = NULL;
    lista->last = NULL;
    return lista;
}
...
hashtable_jogos *criar_jogos() {
    int i;
    hashtable_jogos *hashtable_j = can_fail_malloc(sizeof(hashtable_jogos));
    hashtable_j->entradas = can_fail_malloc(sizeof(Jogo) * TAMANHO);
