...
Jogoslist* inicializa_l_jogos(){	
	Jogoslist* lista = (Jogoslist*)can_fail_malloc(sizeof(Jogoslist)); /*aloca memoria*/
...
Jogos* alocar_jogos(){
	...
	int arg4, arg5;
	Jogos* novo_jogo; 
	novo_jogo = can_fail_malloc(sizeof(Jogos)); 
	scanf(" %[^:\n]:%[^:\n]:%[^:\n]:%d:%d", arg1, arg2, arg3, &arg4, &arg5); /*informacao recebida na funcao*/
	novo_jogo->next = NULL; 
	novo_jogo->prev = NULL; 
	novo_jogo->nome = can_fail_malloc(sizeof(char*)*(strlen(arg1)+1)); /*aloca espaco na memoria*/
	novo_jogo->equipa1 = can_fail_malloc(sizeof(char*)*(strlen(arg2)+1));
	novo_jogo->equipa2 = can_fail_malloc(sizeof(char*)*(strlen(arg2)+1));
...
Equipas* alocar_equipas_nome(){
	char arg1[MAXNAME]; 
	Equipas* nova_equipa; 
	nova_equipa = can_fail_malloc(sizeof(Equipas)); 
	scanf(" %[^:\n]", arg1);
	nova_equipa->next_e = NULL; 
	nova_equipa->prev_e = NULL; 
	nova_equipa->nome = can_fail_malloc(sizeof(char*)*(strlen(arg1)+1)); /*aloca espaco na memoria*/
...
void can_fail_malloc_array(char **array, int k){/*funcao para alocar espaco na memoria para o array com as equipas (usado na funcao g)*/
	int i;
	for(i = 0; i<k;i++){
		array[i] = can_fail_malloc(MAXNAME*sizeof(char));
	}
}
...
int insert(Nodo_jogo **table, Jogos* novojogo){
	...
	Nodo_jogo* new_node = (Nodo_jogo*)can_fail_malloc(sizeof(Nodo_jogo));
...
int insert_e(Nodo_equipa **table, Equipas* novaequipa){
	...
	Nodo_equipa* new_node_e = (Nodo_equipa*)can_fail_malloc(sizeof(Nodo_equipa));
...
void ordena(int i, int *teams, Nodo_equipa** table){
	...
	char **array = can_fail_malloc(num_equipas*sizeof(char*)); /*double pointer porque e uma array de arrays de strings*/
	char temp[MAXNAME];
	int k = 0;
	int j = 0;
	int max_score=0; /*guarda o valor maximo de pontuacao*/
	can_fail_malloc_array(array, num_equipas);
...
int main(){
	...
	hashtable_j = (Nodo_jogo**)can_fail_malloc(SIZE*sizeof(Nodo_jogo));/*aloca espaco na memoria para a Hash Table dos jogos*/
	inicializa_jogo(hashtable_j);/*incializa a Hash Table dos jogos*/
	hashtable_e = (Nodo_equipa**)can_fail_malloc(SIZE*sizeof(Nodo_equipa));/*aloca espaco na memoria para a Hash Table das equipas*/
