...
jogo *criar_hash_jogo(){
	...
	jogo *heads = can_fail_malloc(SIZE * sizeof(struct jogo));
	for (i = 0; i < SIZE; i++)
...
linked_list_jogo *criar_hash_lista_jogos(){
	linked_list_jogo *heads = can_fail_malloc(sizeof(struct linked_list_jogo));
	heads[0] = NULL;
	return heads;
}
...
equipa *criar_hash_equipa(){
	int i;
	equipa *heads = can_fail_malloc(SIZE* sizeof(struct equipa));
	for (i = 0; i < SIZE; i++)
		heads[i] = NULL;
	return heads;
}
...
equipa *criar_hash_equipa1(){
	equipa *heads = can_fail_malloc(sizeof(struct equipa));
	heads[0] = NULL;
	return heads;
}
...
equipa NEW_equipa(char *text){
	...
	equipa x = can_fail_malloc(sizeof(struct equipa));
	x->nome = can_fail_malloc(sizeof(char)*(strlen(text)+1));
...
jogo NEW_jogo(char *text, char *equipa1_input, char *equipa2_input, int golos1_input, 
	...
	x->nome = can_fail_malloc(sizeof(char)*(strlen(text)+1));
	x->equipa1 = can_fail_malloc(sizeof(char)*(strlen(equipa1_input)+1));
	x->equipa2 = can_fail_malloc(sizeof(char)*(strlen(equipa2_input)+1));
...
jogo insertbegin_jogo_lista_jogos(jogo head, linked_list_jogo *lista_jogos,char *text, 
    ...
    linked_list_jogo y = can_fail_malloc(sizeof(struct linked_list_jogo));
...
equipa insert_ordem_alfabetica(equipa head, char *text){
	...
	t = can_fail_malloc(sizeof(struct equipa));
	t->nome = can_fail_malloc(sizeof(char)*(strlen(text)+1));
	strcpy(t->nome, text);
	t->wins = 0;
