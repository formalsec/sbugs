...
EQUIPA ** ht_equipas_init(){
	int i;
	EQUIPA ** ht = can_fail_malloc(sizeof(EQUIPA *)*TAMANHO_HASHTABLE_MAX);
...
EQUIPA * cria_equipa(char * nome){
	...
	nova_equipa->nome = can_fail_malloc(sizeof(char *)*(strlen(nome)+1));
...
LISTA_EQ ** adiciona_eq_lista(LISTA_EQ ** lista, EQUIPA * equipa){
	...
	LISTA_EQ * nova_lista = can_fail_malloc(sizeof(LISTA_EQ));
