...
void adicionaequipa(struct TodasEquipas *equipas, int num_comandos)
{
	...
	/* Criar equipa */
	equipa = can_fail_malloc(1 * sizeof(struct Equipa));
	nome_len = strlen(nome);
	equipa->nome = can_fail_malloc( nome_len + 1 );
	...
	equipas->vetor = can_fail_realloc(equipas->vetor, (equipas->num_equipas + 1) * sizeof(struct Equipa *));
