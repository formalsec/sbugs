...
listajogos* NovaListaJogos() {
	...
	listajogos *novalista = can_fail_malloc(sizeof(struct listajogos)); /* Permite alocar espaco para a lista que sera criada */
	novalista->fimj = NULL; /* Inicia o root(fim) da lista */
	novalista->inicioj = NULL; /* Inicia a head(inicio) da lista */

	while(i < HASHSIZE) {
		novalista->hashjogos[i] = can_fail_malloc(sizeof(struct jogo) * HASHSIZE);
...
listaequipas* NovaListaEquipas() {
	...
	listaequipas *novalista = can_fail_malloc(sizeof(struct listaequipas)); /* Permite alocar espaco para a lista que sera criada */
	novalista->fimeq = NULL; /* Inicia o root(fim) da lista */
	novalista->inicioeq = NULL; /* Inicia a head(inicio) da lista */

	while(i < HASHSIZE) {
		novalista->hashequipas[i] = can_fail_malloc(sizeof(struct equipa) * HASHSIZE); 
...
equipa* NovaEquipa(char* equipanome, listaequipas *list1) {
	...
	equipa *novaequipa = can_fail_malloc(sizeof(struct equipa));

	novaequipa->proximoeq = NULL;
	novaequipa->anterioreq = list1->fimeq;
	novaequipa->nomeeq = can_fail_malloc(sizeof(char) * EQUIPAS);
...
jogo* NovoJogo(char* nomejogo, char* equipa1nome, char* equipa2nome, int score1recebido, int score2recebido, listajogos *list) {
	...
	jogo *novojogo = can_fail_malloc(sizeof(struct jogo));

	novojogo->proximoj = NULL;
	novojogo->anteriorj = list->fimj;
	novojogo->nomej = can_fail_malloc(sizeof(char) * NOME);
	novojogo->equipa1 = can_fail_malloc(sizeof(char) * EQUIPAS);
	novojogo->equipa2 = can_fail_malloc(sizeof(char) * EQUIPAS);

	novojogo->nomej = strcpy(novojogo->nomej,nomejogo);
	novojogo->equipa1->nomeeq = (char *) memcpy(can_fail_malloc(sizeof(char)*(strlen(equipa1nome)+1)),equipa1nome, sizeof(char)*(strlen(equipa1nome)+1));
	novojogo->equipa2->nomeeq = (char *) memcpy(can_fail_malloc(sizeof(char)*(strlen(equipa2nome)+1)),equipa2nome, sizeof(char)*(strlen(equipa2nome)+1));
