...
void StInitEquipa(){
	int i;
	hashequipa = can_fail_malloc(M*sizeof(struct equipa));
	for (i=0; i < M; i++){
...
void StInitJogo(){
	int i;
	hashjogos = can_fail_malloc(M*sizeof(struct jogo));
	for (i=0; i < M; i++){
...
struct equipa *cria_equipa(char *nomeequipa){
	struct equipa *equipaAuxiliar = can_fail_malloc(sizeof(struct equipa)); /* Cria espaco para equipa nova */
	equipaAuxiliar->nomeequipa = can_fail_malloc(sizeof(char) * strlen(nomeequipa) + 1); /* Preenche com o seu nome */
...
struct jogo *cria_jogo(char *nomeJogo, char *nomeEquipa1, char *nomeEquipa2,int r1, int r2){
	struct jogo *jogoAuxiliar = can_fail_malloc(sizeof(struct jogo)); /* Cria espaco */
	jogoAuxiliar->nomeJogo = can_fail_malloc(sizeof(char) * strlen(nomeJogo) + 1);
	strcpy(jogoAuxiliar->nomeJogo, nomeJogo);
	jogoAuxiliar->nomeEquipa1 = can_fail_malloc(sizeof(char) * strlen(nomeEquipa1) + 1);
	strcpy(jogoAuxiliar->nomeEquipa1, nomeEquipa1);
	jogoAuxiliar->nomeEquipa2 = can_fail_malloc(sizeof(char) * strlen(nomeEquipa2) + 1);
...
void funcao_a(){
	...
	struct equipa *equipaAuxiliar;
	char *nomeJogo= can_fail_malloc(sizeof(char)*1023);
	char *nomeequipa1= can_fail_malloc(sizeof(char)*1023);
	char *nomeequipa2= can_fail_malloc(sizeof(char)*1023);
	int r1,r2,j;
	linha++;
...
void funcao_A(){
	struct equipa *equipaAuxiliar;
	char *nomeequipa = can_fail_malloc(sizeof(char)*1023);
	linha++;
	scanf(" %[^:\n]s", nomeequipa);
...
void funcao_g(){
	...
	struct equipa* equipaAuxiliar;
	char* nomeAuxiliar;
	char** nomes= can_fail_malloc(sizeof(char*)*200);
	for(j=0; j<200; j++){
		nomes[j]=can_fail_malloc(sizeof(char)*1023);
	}
	linha++;
	maximo = -1;
	j = 0;
...
int main() {
	...
	StInitJogo();
	StInitEquipa();
	tabela = can_fail_malloc(sizeof(struct jogo*)*300);
	for(i = 0; i < 300; i++){
		tabela[i] = NULL;
	}
	while(1){
