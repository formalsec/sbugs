...
void inicializa_tabla_dispersao() {
	int i;
	inicio_jogos = (jogo*) can_fail_malloc(M*sizeof(jogo));
	for (i = 0; i < M; i++) 
		inicio_jogos[i] = NULL;
}
...
jogo criar_jogo(char *nomejogo, char *nome_equipa1, char *nome_equipa2, int pontos1, int pontos2) {
	jogo novo_jogo = (jogo) can_fail_malloc(sizeof(struct fila));
	novo_jogo -> nome_jogo = (char *) can_fail_malloc(sizeof(char)*(MAX_CAR_EQUIPA));
	strcpy(novo_jogo -> nome_jogo, nomejogo);
	
	novo_jogo -> equipa1 = (char *) can_fail_malloc(sizeof(char)*(MAX_CAR_EQUIPA));
	strcpy(novo_jogo -> equipa1, nome_equipa1);
	novo_jogo -> equipa2 = (char *) can_fail_malloc(sizeof(char)*(MAX_CAR_EQUIPA));
...
equipa criar_equipa(char *nome) {
	equipa nova_equipa = (equipa) can_fail_malloc(sizeof(struct lista));
	ordem_equipa++;
	nova_equipa -> ordem_entrada = ordem_equipa;
	nova_equipa -> nome_equipa = (char *) can_fail_malloc(sizeof(char)*(MAX_CAR_EQUIPA));
...
jogo remove_jogo_lista(int i, char *nome) {
	jogo j, anterior;
	equipa p;
	for (j = inicio_jogos[i], anterior = NULL; j != NULL; anterior = j, j = j -> proximo) {
...
podio adiciona_podio(char *nome) {
	podio novo = (podio) can_fail_malloc(sizeof(struct pilha));
	novo -> nome_equipa_podio = (char*) can_fail_malloc(sizeof(char)*MAX_CAR_EQUIPA);
