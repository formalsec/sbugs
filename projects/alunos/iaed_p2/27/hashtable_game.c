...
ht_game *ht_create_hgame(void){
    ...
    int i = 0;
    /* allocate table */
    ht_game *hashtable = can_fail_malloc(sizeof(ht_game) * 1);

    /* allocate table entries */
    hashtable->entries = can_fail_malloc(sizeof(entry_game) * TABLE_SIZE);
...
entry_game *ht_pair_game(GAME *game){
    ...
    /* allocate the entry */
    entry_game *entry = can_fail_malloc(sizeof(entry_game));
...
GAME* addGame(int line, ht_game *hashtable, ht_team *hashtable_team, GAME* linkedlist) {
    ...
    TEAM* team = NULL;
    TEAM* teamaux = NULL;
    team1 = can_fail_malloc(1024);
    team2 = can_fail_malloc(1024);
    scanf(" %[^:\n]:%[^:\n]:%[^:\n]:%d:%d", name, team1, team2, &score1, &score2);
    game = (GAME*)can_fail_calloc(1, sizeof(GAME));
    ...
    game->name = can_fail_malloc(strlen(name)+1);
