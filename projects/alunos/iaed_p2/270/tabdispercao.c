...
pTabEquipas inicia_te(int max){
  int i, m = 2 * max;
  pTabEquipas tab_dispercao = can_fail_malloc(sizeof(struct tab_equipas));
  equipas(tab_dispercao) = can_fail_malloc(m * sizeof(pEquipa));
...
pTabJogos inicia_tj(int max){
  int i, m = 2 * max;
  pTabJogos tab_dispercao = can_fail_malloc(sizeof(struct tab_jogos));
  jogos(tab_dispercao) = can_fail_malloc(m * sizeof(pJogo));
  M(tab_dispercao) = m; N(tab_dispercao) = 0;
  for (i = 0; i < m; i++)
