...
void adiciona_jogo(struct Jogos*jogos, struct Equipas*equipas, int comandos_total) {
    ...
    novo_jogo = can_fail_malloc(sizeof(struct Jogo));
    nome_length = strlen(nome);
    /* Alocamos memoria para o nome do jogo */
    novo_jogo->nome = can_fail_malloc(sizeof(char) * (nome_length + 1));
   /* Copiamos o nome do novo jogo */
    ...
    jogos->ordem = can_fail_realloc(jogos->ordem, (jogos->njogos + 1) * sizeof(struct Jogo));
