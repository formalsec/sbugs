...
tabela *cria_tabela() {
    int i;
    tabela *tab = can_fail_malloc(sizeof(tabela));
    for (i = 0; i < MAXHASH; i++)
        tab->vet[i] = NULL;
    return tab;
}
...
lista *cria_lista() {
    lista *list = can_fail_malloc(sizeof(lista));
    list->inicio = NULL;
    list->fim = NULL;
    return list;
}
...
tabela_e *cria_tabela_e() {
	int i;
    tabela_e *tab_e = can_fail_malloc(sizeof(tabela_e));
    for (i = 0; i < MAXHASH; i++)
        tab_e->vet[i] = NULL;
    return tab_e;
}
...
lista_das_equipas *cria_lista_e() {
	lista_das_equipas *list_e = can_fail_malloc(sizeof(lista_das_equipas));
    list_e->inicio = NULL;
    list_e->fim = NULL;
    return list_e;
}
...
lista *adiciona_j(tabela *tab, lista *list, char *nome, char *equipa_1, char *equipa_2,
 int resultado_1, int resultado_2, tabela_e *tab_e) {
    ...
    strcpy(j->nome, nome);
    j->equipa_1 = can_fail_malloc(sizeof(char) * (strlen(equipa_1) + 1));
    strcpy(j->equipa_1, equipa_1);
    j->equipa_2 = can_fail_malloc(sizeof(char) * (strlen(equipa_2) + 1));
    ...
    entrada = can_fail_malloc(sizeof(hashtable)); /* aloca memoria para a entrada na hashtable */
    entrada->item = j;
...
lista_das_equipas *adiciona_e(tabela_e *tab_e, lista_das_equipas *list_e, char *nome) {
    ...
    e->nome = can_fail_malloc(sizeof(char) * (strlen(nome) + 1));
    strcpy(e->nome, nome);
    e->vitorias = 0;
    e->seguinte = NULL;
    ...
    list_e->fim = e;
    entrada = can_fail_malloc(sizeof(hash_equipas)); /* aloca memoria para a entrada na hashtable */
    entrada->item = e;
...
void encontra_equipas(int contador, lista_das_equipas *list_e) {
    int i, e = 0, j = 0;
    equipa *temp;
    char **aux = can_fail_malloc(sizeof(char*) * tamanho_lista(list_e));
    temp = list_e->inicio;
