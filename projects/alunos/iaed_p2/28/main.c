...
jogo* createGame(char* nome, char* equipa1, char* equipa2) {
  jogo* game = can_fail_malloc(sizeof(struct jogo));
  if(game == NULL) {
    return NULL;
  }
  game->nome = can_fail_malloc((strlen(nome)+1) * sizeof(char));
  if(game->nome == NULL) {
    free(game);
    return NULL;
  }
  game->equipa1 = can_fail_malloc((strlen(equipa1)+1) * sizeof(char));
  if(game->equipa1 == NULL) {
    free(game->nome);
    free(game);
    return NULL;
  }
  game->equipa2 = can_fail_malloc((strlen(equipa2)+1) * sizeof(char));
...
equipa* criaEquipa(char* nome) {
  equipa * team = can_fail_malloc(sizeof(struct equipa));
  if(team == NULL){
    return NULL;
  }
  team->nome = can_fail_malloc((strlen(nome)+1) * sizeof(char));
  if(team->nome == NULL) {
    free(team);
    return NULL;
  }
