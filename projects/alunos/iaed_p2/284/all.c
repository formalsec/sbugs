...
node* create_game(char* name, team* t1, team* t2, int s1, int s2) {
	node* n = can_fail_malloc(sizeof(node));			/* no do jogo */
	game* g = can_fail_malloc(sizeof(game));			/* jogo em si */

	g->name = can_fail_malloc((strlen(name) + 1) * sizeof(char));	/* aloca o espaco para o nome */
...
node* create_team(char* name) {
	node* n = can_fail_malloc(sizeof(node));			/* no da equipa */
	team* t = can_fail_malloc(sizeof(team));			/* equipa em si */

	t->name = can_fail_malloc((strlen(name) + 1) * sizeof(char));	/* aloca o espaco para o nome */
...
void get_winners(long int NL, list* team_list) {
	...
	char** lst = (char**)can_fail_malloc(size * sizeof(char*));	/* lista com nome das equipas vencedoras */
	...
	if (head) {
		while (head) {
			...
			if (head->d.t->wins > wins) {
				...
				if (i >= size) {
					size *= 2;
					lst = (char**)can_fail_realloc(lst, size);
				}
