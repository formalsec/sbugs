...
Jogo novo_jogo(char * nome,char * e1,char * e2, int s1, int s2){
    Jogo new = (Jogo)can_fail_malloc(sizeof(struct jogo));
    new->score_e1 = s1;
    new->score_e2 = s2;
    new->nome = can_fail_malloc(strlen(nome) +1);
    strcpy(new->nome, nome);
    new->equipa1= can_fail_malloc(strlen(e1) +1 );
    strcpy(new->equipa1, e1);
    new->equipa2= can_fail_malloc(strlen(e2) +1);
...
link_jogo * init_tab_jogos(){
    ...
    link_jogo *jogos;
    jogos = can_fail_malloc(tamanho*sizeof(link_jogo));
    for(i = 0; i < tamanho; i++)
        jogos[i] = NULL;
    return jogos;
}
...
link_jogo insere_jogo_cadeia(link_jogo equi_slot,Jogo jogo){
    link_jogo new = NULL;
    new = can_fail_malloc(sizeof(struct node_jogo));
    new->next = equi_slot;
    new->jogo = jogo;
    return new;
}
...
Lista_jogos inicia_ls(){
    Lista_jogos l = can_fail_malloc(sizeof(struct lista_jogos));
...
Lista_jogos adiciona_jogo_ls(Jogo jogo,Lista_jogos l){
    link_jogo_ls novo = can_fail_malloc(sizeof(struct node_jogo_ls));
    novo->prev = l->ultimo;
    novo->next = NULL;
    novo->jogo = jogo;
