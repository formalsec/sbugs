...
/*false_t-struct que devolvo em vez de FALSE quando procuro uma equipa*/
	struct Equipas *false_t=(struct Equipas*)can_fail_malloc(sizeof(struct Equipas));
/*false_g-struct que devolvo em vez de FALSE quando procuro um jogo*/
	struct Jogos *false_g = (struct Jogos*)can_fail_malloc(sizeof(struct Jogos));
 ...
 * jogo/equipa no caso destas nao existirem*/
	false_t->ID = can_fail_malloc((strlen("NONE")+1)*sizeof(char));
	strcpy(false_t->ID, "NONE");
	false_g->ID = can_fail_malloc((strlen("none")+1)*sizeof(char));
	strcpy(false_g->ID, "none");
	
/*Inicilizacao das tabelas dinamicas dos jogos e equipas*/	
	lista_equipas = can_fail_malloc(N*sizeof(teams));
	games_list = can_fail_malloc(M*sizeof(games));
