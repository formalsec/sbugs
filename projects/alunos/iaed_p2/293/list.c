...
Game *SetGameNode(Game *new, Game *headG, char *fname, char *fteam1, char *fteam2, int fscore1, int fscore2, int nl) {
	...
	ln=strlen(fname);
	new->name=(char *)can_fail_malloc(sizeof(char)*(ln)+1);
	strcpy(new->name, fname);
	
	l1=strlen(fteam1);
	new->team1=(char *)can_fail_malloc(sizeof(char)*(l1)+1);
	strcpy(new->team1, fteam1);
	
	l2=strlen(fteam2);
	new->team2=(char *)can_fail_malloc(sizeof(char)*(l2)+1);
...
Team *InitTeamNode(){
	...
	/*memory allocation for new array of structures*/
	Team *new=(Team *)can_fail_malloc(sizeof(Team));
	/*inicialization of variables*/
	new->name='\0';
	new->wins=0;
	new->next=NULL;
