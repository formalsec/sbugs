...
Hash_table *init_table(int size_table){
	Hash_table  *new_t = (Hash_table *) can_fail_malloc(sizeof(Hash_table));
	new_t->table = (Hash_node **) can_fail_calloc(size_table, sizeof(Hash_node *));
...
Hash_node *insertJogo_N(Hash_node *head, Node *jogoIns){
	Hash_node *new = (Hash_node *) can_fail_malloc(sizeof(Hash_node));
	new->jogo = jogoIns;
	new->next = NULL;
	if(head == NULL){
		head = new;
