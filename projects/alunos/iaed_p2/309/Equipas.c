...
hashtable * CreateHash(){
	...
	hashtable * tb = can_fail_malloc(sizeof(hashtable));
	tb->heads = can_fail_calloc(HASH_SIZE, sizeof(hash_node*));
	return tb; 
}
...
Equipa * AddTeam(int NL, Equipa * headE, hashtable * hash){
	...
	if(CheckTeam(hash, buffer, key)){
		...
		new = can_fail_malloc(sizeof(Equipa));
		new->nome = can_fail_malloc(sizeof(char) * (strlen(buffer)+1));
