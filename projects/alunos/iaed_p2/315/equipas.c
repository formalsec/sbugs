...
node_equipa criar_node_equipa(char *nome){
	...
	node_equipa novo = can_fail_malloc(sizeof(struct stru_node_equipa));
	novo->next = NULL;
	novo->vitorias = 0;
	novo->nome_equipa = can_fail_malloc((strlen(nome)+1)*sizeof(char));
