...
void f_A(int NL, EHnode *Ehash, Equipas_sistema *equipas_sis){
    ...
    equipa -> nome = can_fail_malloc(sizeof(char) * (strlen(nome) + 1));
...
void f_g(int NL, Equipas_sistema *equipas_sis){
	...
	equipas_ord = can_fail_malloc(sizeof(Equipa *) * EQUI_SIS_M);
	...
	for(node = *(equipas_sis -> equipas); node != NULL; node = node -> next){
		...
		if(node -> equipa -> jogos_ganhos == equipas_sis -> n_max_vitorias){
			...
			if (equipas_sis -> n_equipas_max >= m){
				m += EQUI_SIS_M;
				equipas_ord = can_fail_realloc(equipas_ord, sizeof(Equipa *) * m); /*aumentar a memoria alocada para a lista caso necessario*/
			}
...
int main(){
    int NL = 1;
    char comando;
    EHnode *Ehash = can_fail_malloc(HASH_M * sizeof(EHnode));
    JHnode *Jhash = can_fail_malloc(HASH_M * sizeof(JHnode));
    Jogos_sistema *jogos_sis = can_fail_malloc(sizeof(struct jogos_sistema));
    Equipas_sistema *equipas_sis = can_fail_malloc(sizeof(struct equipas_sistema));
