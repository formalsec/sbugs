...
{
    jogo *novo = can_fail_malloc(sizeof(jogo) + 1);
    equipa *equipa;
    int score1, score2;
    char tmp[1023]; /*usa-se apenas um char tmp para todos os inputs para poupar memoria*/

    scanf(" %[^:]", tmp);
    novo->nome = can_fail_malloc(sizeof(char) * (strlen(tmp) + 1)); /*Adiciona-se o nome*/
    strcpy(novo->nome, tmp);

    scanf(":%[^:]", tmp);
    novo->e1 = can_fail_malloc(sizeof(char) * (strlen(tmp) + 1)); /*Adiciona-se a equipa 1*/
    strcpy(novo->e1, tmp);

    scanf(":%[^:]", tmp);
    novo->e2 = can_fail_malloc(sizeof(char) * (strlen(tmp) + 1)); /*Adiciona-se a equipa 2*/
    strcpy(novo->e2, tmp);
