...
void NewGame(int line){
  ...
  char* name1 = can_fail_malloc(sizeof(char)*MAX_STR);
  char* name2 = can_fail_malloc(sizeof(char)*MAX_STR);
  int i,i1,i2;
  int score1, score2;
  gamelink t;
  link t1,t2;
...
void SearchGame(int line){
  char* name = can_fail_malloc(sizeof(char)*1024);
  gamelink l;
  int i;
...
void GameInit(){
  int i;
  g_heads = (gamelink*)can_fail_malloc(sizeof(gamelink)*M_G);
  for (i = 0; i < M_G; i++)
    g_heads[i] = NULL;
}
...
gamelink newGameLink(char* name, char* name1, char* name2, int score1, int score2){
  size_t size;
  gamelink l = (gamelink)can_fail_malloc(sizeof(struct GameNode));
  l->game = can_fail_malloc(sizeof(struct game));

  size = strlen(name) + 1;
  l->game->name = can_fail_malloc(sizeof(char)*size);
  strncpy(l->game->name, name, size);

  size = strlen(name1) + 1;
  l->game->team1 = can_fail_malloc(sizeof(char)*size);
  strncpy(l->game->team1, name1, size);

  size = strlen(name2) + 1;
  l->game->team2 = can_fail_malloc(sizeof(char)*size);
...
GamesList new_game_node(char* name){
  size_t size;
  GamesList game = can_fail_malloc(sizeof(struct names));

  size = strlen(name) + 1;
  game->name = can_fail_malloc(sizeof(char*)*size);
