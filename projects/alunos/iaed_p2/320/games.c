...
game * new_game(char * g_name, char * name_t1, char * name_t2, int score1, int score2){
	game * p_g = can_fail_malloc(sizeof(game));
...
dlist_games * mk_dlist_games(){
	dlist_games * dl = can_fail_malloc(sizeof(dlist_games));
	dl->head = NULL;
	dl->last = NULL;
	return dl;
}
...
node_dlist_games * addlast_dlist_games(dlist_games * dl, game * p_g){
	node_dlist_games * n_dl = can_fail_malloc(sizeof(node_dlist_games));
	n_dl->prev = dl->last;
	n_dl->next = NULL;
	n_dl->other = NULL;
	n_dl->p_g = p_g;
...
dlist_games ** init_hasht_games(){
	int i;
	dlist_games ** hasht_g = can_fail_malloc(sizeof(dlist_games *) * N_HASH_TABLE);
