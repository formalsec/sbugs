...
jogos * mk_list_jg()
{
    ...
    jogos * l = can_fail_malloc(sizeof(jogos));
    /*Inicializa a lista*/
    l->head =l->last=  NULL;
    return l;
}
...
jg_node *add_last_jg(jogos *l, const char *nome, const char *equipa1, const char *equipa2, int score1, int score2)
{
    ...
    jg_node *nn = can_fail_malloc(sizeof(jg_node));
    /*Vai adicionar o no ao fim da lista*/
    nn->previous =l->last;
    nn->next = NULL;
    /*Aloca espaco para os argumentos strings do novo no a adicionar*/
    nn->value = can_fail_malloc(sizeof(char)*(strlen(nome)+1));
    nn->equipa1 = can_fail_malloc(sizeof(char)*(strlen(equipa1)+1));
    nn->equipa2 = can_fail_malloc(sizeof(char)*(strlen(equipa2)+1));
