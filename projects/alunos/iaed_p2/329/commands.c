...
void Start_Sytem(System *s){
	s->inputline = 0;
	s->ts = can_fail_malloc(sizeof(TS));
	s->gs = can_fail_malloc(sizeof(GS));
	Init_Teams(s->ts);
	Init_Games(s->gs);
}
...
void command_add_game(System *s){
	...
	if (games){
		...
		name_game = can_fail_malloc(sizeof(char)*strlen(bufferg)+1);
...
void command_add_team(System *s){
	...
	scanf(" %[^:\n]", buffer);
	s->inputline += 1;	
	name = can_fail_malloc(sizeof(char)*strlen(buffer)+1);
	strcpy(name, buffer);
	if (STsearch(name, s->ts)){
		...
		s->ts->Nteams += 1;
		t = can_fail_malloc(sizeof(struct Team));
		t->name = name;
		t->wins = 0;
		STinsert(s->ts, t);
	}
...
void command_bggst_wnnrs(System *s){
	...
	if (s->ts->Nteams > 0){
		...
		biggest_winners = can_fail_malloc(sizeof(char*) * n_winners);
