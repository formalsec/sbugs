...
void ht_init(Hashtable* ht){
    int i;
    *ht = (Hashtable) can_fail_malloc(sizeof(struct hashtable_str));
    (*ht)->heads = (ht_link*) can_fail_malloc(sizeof(ht_link)*HTSIZE);
    for(i = 0; i < HTSIZE; i++)
        (*ht)->heads[i] = NULL;
}
...
ht_link ht_cria(lst_link lst){
    ht_link link = (ht_link) can_fail_malloc(sizeof(struct ht_node));
    link->lst = lst;
    return link;
}
