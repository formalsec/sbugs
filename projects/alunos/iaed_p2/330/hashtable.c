...
Hequipas *init_hashE(){
	...
	nova_hashequipas = can_fail_malloc(sizeof(Hequipas)); /*Aloca memoria para a hashtable*/
	nova_hashequipas->cabecas = can_fail_calloc(MAX_TABLE, sizeof(NodeE*)); /*Coloca tudo a NULL*/
...
Equipa *hash_insertE(Hequipas *h_eq, Equipa *e, char *buffer){
	Equipa *eq_add = can_fail_malloc(sizeof(Equipa)); /*Cria e aloca memoria para um ponteiro do tipo eq_add*/
	NodeE *aux = can_fail_malloc(sizeof(NodeE)); /*Cria e aloca memoria para um ponteiro do tipo NodeE*/
	size_t index = hash(buffer, h_eq->capacidade); /*Obtem o index que ira corresponder a uma posicao da hashtable*/
	eq_add->nome = can_fail_malloc(strlen(buffer) + 1);/*Aloca memoria para o nome da equipa*/
...
Hjogos *init_hashJ(){
	...
	nova_hashjogos = can_fail_malloc(sizeof(Hjogos));
	nova_hashjogos->cabecas = can_fail_calloc(MAX_TABLE, sizeof(NodeJ*));
...
void hash_insertJ(Hjogos *h_j, Cabeca_cauda *extremos_lista_jogos, char *buffer, char *equipa1_aux, char *equipa2_aux, int pontos1, int pontos2){
	Jogo *j_add = can_fail_malloc(sizeof(Jogo));
	NodeJ *aux = can_fail_malloc(sizeof(NodeJ));
	size_t index = hash(buffer, h_j->capacidade);
	j_add->nome = can_fail_malloc(strlen(buffer) + 1);
	strcpy(j_add->nome, buffer);
	j_add->equipa1 = can_fail_malloc(strlen(equipa1_aux) + 1);
	strcpy(j_add->equipa1, equipa1_aux);
	j_add->equipa2 = can_fail_malloc(strlen(equipa2_aux) + 1);
