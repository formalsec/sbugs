...
t_node* push_team(t_node *head, char *team){
    t_node *new_node = can_fail_malloc(sizeof(t_node));
    new_node->next = head;
    new_node->team_name = can_fail_malloc(sizeof(char) * (strlen(team) + 1));
...
link_t** init_hash_teams(){
    int i;
    link_t** hash_teams;
    hash_teams = can_fail_malloc(sizeof(link_t) * T_HASH);
    for(i = 0; i < T_HASH; i++){
        hash_teams[i] = NULL;
    }
    return hash_teams;
...
link_t* insert_beguin_hash_team(link_t *head, t_node *team){
    link_t *new_head = can_fail_malloc(sizeof(link_t));
