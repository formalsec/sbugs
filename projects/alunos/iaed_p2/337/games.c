...
Games * init_Glist(){
	Games * list = can_fail_malloc(sizeof(Games));
...
game * alloc_game(char game_name[MAXCHAR], char t1_name[MAXCHAR], char t2_name[MAXCHAR], int score_t1, int score_t2){
	...
	new_game->name = can_fail_malloc(sizeof(char) * (strlen(game_name) + 1));
	strcpy(new_game->name, game_name);

	new_game->t1_name = can_fail_malloc(sizeof(char) * (strlen(t1_name)+ 1));
	strcpy(new_game->t1_name, t1_name);

	new_game->t2_name = can_fail_malloc(sizeof(char) * (strlen(t2_name)+ 1));
	...
	new_game->winner = can_fail_malloc(sizeof(char) * (strlen(winner) + 1));
...
void change_winner(game * game_x, char * new_winner, HashTeams * hashtable_teams){
	...
	free(game_x->winner);
	game_x->winner = can_fail_malloc(sizeof(char) * (strlen(new_winner) + 1));
...
HashGames * init_HGtable(int size){
	int i;
	HashGames * hashtable = can_fail_malloc(sizeof(HashGames));
	hashtable->slots = can_fail_malloc(sizeof(Slot) * size);
	hashtable->size = 0;
	hashtable->cap = size;
...
void resize_hashtable_games(HashGames * hashtable){
	...
	while (eh_primo(new_cap) != 1)
 	...
 	new_slots = can_fail_malloc(sizeof(Slot) * new_cap);
