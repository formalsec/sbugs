...
void insert_equipa(char *key,equipa value,int tblsz, equipa *eq,int N){
	...
	while(eq[index] != NULL)
		index = index + 1 % tblsz;
	eq[index] = can_fail_malloc(sizeof(struct equipas));
	eq[index]->nome_equipa = can_fail_malloc(sizeof(char)*(strlen(value->nome_equipa)+1));
...
void expand_equipa(int tblsz_equipa, equipa *eq, int N){
	int i;
	equipa *t = eq;
	eq = can_fail_realloc(eq,2*tblsz_equipa*sizeof(equipa));
...
void insert_jogo(char *key, jogo value,int tblsz, jogo *st ,int M){
	...
	while(st[index] != NULL){
		...
		st[index] = can_fail_malloc(sizeof(struct jogos));
		st[index]->nome = can_fail_malloc(sizeof(char)*(strlen(value->nome)+1));
		strcpy(st[index]->nome,value->nome);
		st[index]->equipa1 = can_fail_malloc(sizeof(char)*(strlen(value->equipa1)+1));
		strcpy(st[index]->equipa1,value->equipa1);
		st[index]->equipa2 = can_fail_malloc(sizeof(char)*(strlen(value->equipa2)+1));
...
void expand_jogo(int tblsz_jogo, jogo *st, int M){
	int i;
	jogo *t = st;
	st = can_fail_realloc(st,2*tblsz_jogo*sizeof(jogo));
