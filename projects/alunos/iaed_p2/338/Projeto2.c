...
int jogo_inexistente(char nome[],int tblsz_jogo, int contalinhas, jogo printado, jogo *st){
	char *aponta;
	aponta = can_fail_malloc(sizeof(char)*(strlen(nome)+1));
	strcpy(aponta,nome);
...
int main()
{
	...
	struct jogos *game = can_fail_malloc(sizeof(struct jogos));
	struct jogos *procura_jogo = can_fail_malloc(sizeof(struct jogos));
	jogo *printado = can_fail_malloc(sizeof(jogo));
	struct equipas *add_equipa = can_fail_malloc(sizeof(struct equipas));
	...
	jogo *st;
	equipa *eq;
	tuplo = (struct tuple*)can_fail_malloc(sizeof(struct tuple));
	st = can_fail_malloc(tblsz_jogo*sizeof(jogo));
	eq = can_fail_malloc(tblsz_equipa*sizeof(equipa));
	...
	while((c=getchar()) != EOF){
		switch(c){
			case 'a':
				...
				game->nome = can_fail_malloc(sizeof(char)*(strlen(nome)+1));
				strcpy(game->nome,nome);
				game->equipa1 = can_fail_malloc(sizeof(char)*(strlen(equipa1)+1));
				strcpy(game->equipa1,equipa1);
				game->equipa2 = can_fail_malloc(sizeof(char)*(strlen(equipa2)+1));
			...
			case 'A':
				...
				scanf(" %[^\n]", team_name);
				getchar();
				aponta_nome_equipa = can_fail_malloc(sizeof(char)*(strlen(team_name)+1));
				...
				add_equipa->nome_equipa = can_fail_malloc(sizeof(char)*(strlen(team_name)+1));
			...
			case 's':
				...
				if(quem_ganhou(nome,tblsz_jogo,st) != NULL){
					aponta_nome_equipa = can_fail_malloc(sizeof(char)*(strlen(quem_ganhou(nome,tblsz_jogo,st))+1));
			...
			case 'g':
				...
				if(tuplo->conta_eq_c_max_vit != 0){
					...
					equipas_c_vitorias = can_fail_malloc(sizeof(char*)*(tuplo->conta_eq_c_max_vit+1));
