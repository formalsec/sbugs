...
entradaE_t *criarEntrada_htE(char *chaveE, Equipa *equipa) {
    /* Alocar a entrada. */
    entradaE_t *entrada = can_fail_malloc(sizeof(entradaE_t));
    entrada -> chaveE = can_fail_malloc(strlen(chaveE) + 1);
...
htE_t *criar_htE(void) {
    ...
    htE_t *hashtable = can_fail_malloc(sizeof(htE_t));
    /* Alocar espaco para ad entradas da tabela. */
    hashtable -> entradas = can_fail_malloc(sizeof(entradaE_t) * TAMANHO_TABELA);
