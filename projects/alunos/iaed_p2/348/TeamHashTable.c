...
Hash_Table_Team* mk_Team_Hash_Table(){
	...
	Hash_Table_Team* pHT = can_fail_malloc(sizeof(Hash_Table_Team));
	pHT->capacity = TEAM_HASH_SIZE;
	pHT->table = can_fail_malloc(sizeof(Hash_Node_Team) * pHT->capacity);
...
void addToTeamHashList(char *teamName, Hash_Table_Team *t, int index){
	...
	Hash_Node_Team *node = can_fail_malloc(sizeof(Hash_Node_Team));
	Team *team = can_fail_malloc(sizeof(Team));

	team->name = can_fail_malloc(sizeof(char) * (strlen(teamName) + 1));
