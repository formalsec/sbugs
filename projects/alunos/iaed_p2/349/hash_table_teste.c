...
entry_jogos *ht_pair_jogos(const char *nome, const char *equipa1, const char *equipa2, int score1, int score2) {
    /* allocate the entry */
    entry_jogos *entry = can_fail_malloc(sizeof(entry_jogos) * 1);
    entry->jogo = can_fail_malloc(sizeof(jogo) * 1);
    entry->jogo->nome = can_fail_malloc(strlen(nome) + 1);
    entry->jogo->equipa1 = can_fail_malloc(strlen(equipa1) + 1);
    entry->jogo->equipa2 = can_fail_malloc(strlen(equipa2) + 1);
...
ht_jogos *ht_create_jogos(void) {
    int i = 0;
    /* allocate table */
    ht_jogos *hashtable = can_fail_malloc(sizeof(ht_jogos) * 1);

    /* allocate table entries */
    hashtable->capacidade = TAMANHO_INICIAL;
    hashtable->entries = can_fail_malloc(sizeof(entry_jogos*) * hashtable->capacidade);
...
entry_equipas *ht_pair_equipas(const char *nome, int vitorias) {
    /* allocate the entry */
    entry_equipas *entry = can_fail_malloc(sizeof(entry_equipas) * 1);
    entry->equipa = can_fail_malloc(sizeof(equipa) * 1);
    entry->equipa->nome = can_fail_malloc(strlen(nome) + 1);
...
ht_equipas *ht_create_equipa(void) {
    int i = 0;
    /* allocate table */
    ht_equipas *hashtable = can_fail_malloc(sizeof(ht_equipas) * 1);

    /* allocate table entries */
    hashtable->capacidade = TAMANHO_INICIAL;
    hashtable->entries = can_fail_malloc(sizeof(entry_equipas*) * hashtable->capacidade);
