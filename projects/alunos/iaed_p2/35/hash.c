...
Hashtable_JOGOS HashtableInit_JOGOS(int M) {
    ...
    Hashtable_JOGOS HT = can_fail_malloc(sizeof(struct hashtable_jogos));
    HT->M = M;
    HT->adj = can_fail_malloc(M * sizeof(linkPJogo));
...
void HashtableInsert_JOGOS(Hashtable_JOGOS HT, linkJogo j) {
    ...
    linkPJogo  a = (linkPJogo)can_fail_malloc(sizeof(struct nodePJogo));
...
Hashtable_EQUIPAS HashtableInit_EQUIPAS(int M) {
    ...
    Hashtable_EQUIPAS HT = can_fail_malloc(sizeof(struct hashtable_jogos));
    HT->M = M;
    HT->adj = can_fail_malloc(M * sizeof(linkPEquipa));
    /*inicializacao de todas as heads a NULL*/
    for (i = 0; i < M; i++)
        HT->adj[i] = NULL;
    return HT;
...
void HashtableInsert_EQUIPAS(Hashtable_EQUIPAS HT, linkEquipa e) {
    /*aloca espaco para novo nodo*/
    linkPEquipa  a = (linkPEquipa)can_fail_malloc(sizeof(struct nodePEquipa));
