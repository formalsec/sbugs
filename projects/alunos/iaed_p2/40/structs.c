...
Team createTeam(char* name) {
    ...
    Team new_team = (Team)can_fail_malloc(sizeof(struct team));
...
unsigned int t1_score, unsigned int t2_score, int nl_count) {
    ...
    Game new_game =
    (Game)can_fail_malloc(sizeof(struct game));
    new_game->name =
    (char*)can_fail_malloc(sizeof(char)*(strlen(game_name) + 1));
    new_game->team1 =
    (char*)can_fail_malloc(sizeof(char)*(strlen(t1_name) + 1));
    new_game->team2 =
    (char*)can_fail_malloc(sizeof(char)*(strlen(t2_name) + 1));
...
void* hashStartup() {
    ...
    Team* temp = can_fail_calloc(HASH_TABLE_LIM, sizeof(void*));
    return temp;
