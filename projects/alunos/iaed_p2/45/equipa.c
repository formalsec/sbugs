...
ItemSecondHT * inserirInicioListaE(ItemSecondHT * head, char * nome){
    ItemSecondHT *temporario = can_fail_malloc(sizeof(ItemSecondHT));
    elementHT(temporario) = can_fail_malloc(sizeof(equipa));
    keyHT(temporario) = (char *)can_fail_malloc(sizeof(char) * (strlen(nome) + 1));
...
void HTencontraEquipasVencedoras(ItemSecondHT **headsEquipa, int capacidadeE, int linha){
    ...
    for(i = 0; i < capacidadeE; i++){
        ...
        while (headsEquipa[i] != NULL){
            ...
            if (descHT(headsEquipa[i]) == max){
                elementos++;
                arrayV = (equipa **)can_fail_realloc(arrayV, sizeof(equipa *) * elementos);
