...
void adicionarUltimoLista(lista* listaJ, jogo * nJogo){
    ItemList *no = can_fail_malloc(sizeof(ItemList));
...
ItemFirstHT * inicializarItem(char *nome, equipa * e1, equipa * e2, int s1, int s2){
    ItemFirstHT *temporario = (ItemFirstHT *)can_fail_malloc(sizeof(ItemFirstHT));
    elementHTS(temporario) = (jogo *)can_fail_malloc(sizeof(jogo));
    keyHTS(temporario) = (char *)can_fail_malloc((strlen(nome) + 1) * sizeof(char));
