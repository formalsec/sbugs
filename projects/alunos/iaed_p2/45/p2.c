...
int main(){
    ...
    ItemFirstHT ** headsJogo = (linkJogo **)can_fail_malloc(capacidadeJ * sizeof(linkJogo));

    /* listaJogo: variavel local que armazena o ponteiros para o primeiro e ultimo elemento da lista
                            de Jogos*/
    lista * listaJogo = can_fail_malloc(sizeof(lista));

    /* headsEquipa: variavel local que armazena as heads da HT de Encademento Externo das Equipas*/
    ItemSecondHT  ** headsEquipa = (linkEquipa **) can_fail_malloc(capacidadeE * sizeof(linkEquipa));
