...
GameLinkedList list_init() {
	GameLinkedList list = can_fail_malloc(sizeof(struct MasterNode)); /*aloca memoria para a estrutura MasterNode*/
	list->n_elementos = 0;
	list->head = NULL;
	list->tail = NULL;
	return list;
...
Link add_node(Link prev, Link next, GAME game) {
	Link new = can_fail_malloc(sizeof(struct Node)); /*aloca memoria para um node*/
	new->Game = game;
