...
void HT_Equipas_Init(int m, Hash_equipa *hash_ptr)
{
    int i;
    hash_ptr->size = m;
    hash_ptr->table_eq = (Node_equipa **)can_fail_malloc(m * sizeof(Node_equipa *)); /* Criacao de um array de m ponteiros para node*/
    for (i = 0; i < m; i++)
...
void HT_Equipas_Insert(char nome[], Hash_equipa *hash, int NL)
{
    ...
    else
    {
        eq = novaEquipa(nome);
        hash_value = calc_hash(nome, hash->size);
        node_tmp = (Node_equipa*) can_fail_malloc(sizeof(Node_equipa));
        node_tmp->eq = eq;
