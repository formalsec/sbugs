...
void insere(Jogo *jogo_ptr) 
{
        ...
        novono = (struct Node_jogo *)can_fail_malloc(sizeof(struct Node_jogo));
        novono->jg = jogo_ptr;
        novono->next = NULL;
        if (cabeca == NULL)
        {
...
void HT_Jogos_Init(int m, Hash_jogo *hash_ptr)
{
    int i;
    hash_ptr->size = m;
    hash_ptr->table_jg = (Node_jogo **)can_fail_malloc(m * sizeof(Node_jogo *)); /* criei um array de m ponteiros para node*/
    for (i = 0; i < m; i++)
...
void HT_Jogos_Insert(char nome_jogo[], char equipa_1[], char equipa_2[], int pontuacao_1, int pontuacao_2, Hash_jogo *hash_jg, Hash_equipa *hash_eq, int NL)
{
    ...
    else
    {
        ...
        node_tmp = (Node_jogo*) can_fail_malloc(sizeof(Node_jogo));
        node_tmp->jg = jg;
