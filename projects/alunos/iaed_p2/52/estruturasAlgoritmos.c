...
lista* novaLista(int m){
    lista* newQ;
    newQ = (lista*)can_fail_malloc(sizeof(lista));
    newQ ->vetor = (item*)can_fail_malloc(sizeof(item)*m);
...
lista* dupTamanhoLista(lista *q){
    ...
    q->m = 2 * q->m;
    q->vetor = can_fail_realloc(q->vetor,sizeof(item)*q->m);
    return q;
}
...
tabela* novaTabela(int m){
    tabela *new = (tabela*)can_fail_malloc(sizeof(tabela));
    new->m = m;
    new->n = 0;
    new->itens = (lista**)can_fail_malloc(sizeof(lista*)*m);
