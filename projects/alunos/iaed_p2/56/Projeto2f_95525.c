...
void Teamtableinit(){   /* initiates the teams' table*/
    int i;
    teamtable = (pteam*) can_fail_malloc (TEAMTABLESIZE * sizeof(pteam));
...
void add_team(char *new_name){  /* creates a new team in the database */
    pteam new_team;
    new_team = (pteam) can_fail_malloc (sizeof (Team));
    new_team -> name = (char*) can_fail_malloc (strlen (new_name) + 1);
...
void Matchtableinit(){  /* inititates the matches' table */
    int i;
    matchtable = (pmatch*) can_fail_malloc (MATCHTABLESIZE * sizeof(Match));
...
void add_match(char *name, char *team_1, char *team_2, int score_1, int score_2){   /* creates a new match in the database */
    pmatch new_match;
    new_match = (pmatch) can_fail_malloc (sizeof (Match));
    new_match -> id = (char*) can_fail_malloc (strlen(name) + 1);
    new_match -> team1 = (char*) can_fail_malloc (strlen(team_1) + 1);
    new_match -> team2 = (char*) can_fail_malloc (strlen(team_2) + 1);
...
char* str_treatment(char str[STRMAX]){  /* processes the input strings */
    char* new_str;
    str[strlen(str)+1] = '\0';
    new_str = (char*) can_fail_malloc(sizeof(char)*(strlen(str)+1));
