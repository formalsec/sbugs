...
pJogo *init_tab_dispersao(unsigned int M) {
    ...
    tab_dispersao = (pJogo*)can_fail_malloc(M*sizeof(pJogo));
    for (i = 0; i < M; i++) {
...
pJogo tab_dispersao_insere(pJogo *tab_dispersao, char *nome, char *equipa1, char *equipa2, 
    ...
    tab_dispersao[i] = (pJogo)can_fail_malloc(sizeof(struct Jogo));
    tab_dispersao[i]->nome =  (char*) can_fail_malloc(sizeof(char)*(strlen(nome)+1));
