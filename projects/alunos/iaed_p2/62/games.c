...
{
    GameLink game = (GameLink)can_fail_malloc(sizeof(struct GameNode));

    /* Copies the game name and both team names */
    game->game_name = (char *)can_fail_malloc((strlen(game_name) + 1) * sizeof(char));
    strcpy(game->game_name, game_name);

    game->team1 = (char *)can_fail_malloc((strlen(team1) + 1) * sizeof(char));
    strcpy(game->team1, team1);

    game->team2 = (char *)can_fail_malloc((strlen(team2) + 1) * sizeof(char));
...
GameHT game_ht_init(TeamHT *team_ht, int size)
{
    ...
    game_ht.size = size;
    game_ht.items_num = 0;
    game_ht.ht_header = (GameLink *)can_fail_malloc(size * sizeof(GameLink));
