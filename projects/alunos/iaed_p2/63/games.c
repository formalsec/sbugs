...
game create_game(node_teams **teams_table){
    ...
    new_game.name = can_fail_malloc((strlen(name)+1)*sizeof(char));
    strcpy(new_game.name, name);

    /*allocates the necessary memory for the name and saves it in the team*/
    node_aux_t1.t.name = can_fail_malloc((strlen(name_t1)+1)*sizeof(char));
    strcpy(node_aux_t1.t.name, name_t1);
    p_node_aux_t1 = &node_aux_t1;

    /*allocates the necessary memory for the name and saves it in the team*/
    node_aux_t2.t.name = can_fail_malloc((strlen(name_t2)+1)*sizeof(char));
...
node_games * push_game(node_games *head, game g){
    ...
    node_games *new = can_fail_malloc(sizeof(node_games));
...
node_games * find_game(node_games **games_table){
    ...
    scanf(" %[^:\n]", name);
    g.name = can_fail_malloc((strlen(name)+1)*sizeof(char));
    strcpy(g.name,name);
