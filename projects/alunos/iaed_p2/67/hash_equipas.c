...
link_equipa* init_equipa(int n){
  ...
  int i;
  tabela_equipas = (link_equipa*)can_fail_malloc(n* sizeof(link_equipa));
  for(i=0; i<n; i++){
...
link_equipa nova_equipa(char *nome){
  ...
  x->equipa = (pEquipa) can_fail_malloc(sizeof(struct equipa));
  x->equipa->nome = (char*) can_fail_malloc((strlen(nome) + 1) * sizeof(char));
