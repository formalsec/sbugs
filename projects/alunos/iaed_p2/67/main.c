...
int main(){
  ...
  while (x!= 'x'){
    ...
    if (x=='a'){
      ...
      nome = (char*) can_fail_malloc((strlen(buf) + 1) * sizeof(char));
      strcpy(nome, buf);
      equipa1 = (char*) can_fail_malloc((strlen(buf1) + 1) * sizeof(char));
      strcpy(equipa1, buf1);
      equipa2 = (char*) can_fail_malloc((strlen(buf2) + 1) * sizeof(char));
      strcpy(equipa2, buf2);
    ...
    /*adiciona uma nova equipa */
      ...
      linha++;
      scanf(" %[^:\n]", buffer);
      nome = (char*) can_fail_malloc((strlen(buffer) + 1) * sizeof(char));
      strcpy(nome, buffer);
    ...
    /*encontra as equipas que venceram mais jogos*/
      ...
      for(x = lista_equipas; x!= NULL; x = x->next){
        if (x->equipa->vitorias == max){
          lista_melhores[i] = (char*) can_fail_malloc(sizeof(char) * (strlen(x->equipa->nome)+1));
