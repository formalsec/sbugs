...
DB db_init(uint max) {
	DB db = can_fail_malloc(sizeof(struct db));
...
m_link* M_HT_init(uint max) {
	uint i;
	m_link* heads = (m_link*) can_fail_malloc(sizeof(m_link) * max);
...
M_DLL* M_DLL_init() {
	M_DLL* list = can_fail_malloc(sizeof(M_DLL));
	list->head = NULL;
	list->tail = NULL;
	return list;
}
...
t_link* T_HT_init(uint max) {
	uint i;
	t_link* heads = (t_link*) can_fail_malloc(sizeof(t_link) * max);
...
DBMS dbms_init(uint max) {
	DBMS dbms = can_fail_malloc(sizeof(struct dbms));
...
m_link new_match_node(Match match, m_link ht_next, m_link l_tail) {
	m_link new_m_link = can_fail_malloc(sizeof(struct m_node));
...
t_link new_team_node(Team team, t_link next) {
	t_link new_t_link = can_fail_malloc(sizeof(struct t_node));
...
void list_best_teams(DBMS dbms) {
	...
	team_list = can_fail_malloc(sizeof(Team)*number_max_teams);
