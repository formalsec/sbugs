...
equipa* insertBeginList_eq(equipa* head, char nome[Max]){
    int i;
    equipa* t; 
    t=(equipa*)can_fail_malloc(sizeof(equipa));
    i=strlen(nome);
    i++;
    t->nome=(char*) can_fail_malloc(sizeof(char)*i);
    strcpy(t->nome,nome);
    t->i=0;
    t->next=head;
    return t;
...
void proc(equipa** hash,equipa**haed,int sc1,int m,int sc2){
    int f,*i=can_fail_malloc(sizeof(int));
    (*i)=0;
    for (f=0;f<m;f++){
...
hash_jogo* insertBeginList(hash_jogo* hash_game, jogo* jog){
    hash_jogo* t= can_fail_malloc(sizeof(hash_jogo));
    t->nome=jog;
    t->next=hash_game;
    return t;
}
...
void add_jogo(l_jogo* l, char jog[Max],equipa* eq1,equipa* eq2,int sc1,int sc2){
    jogo *n = can_fail_malloc(sizeof(jogo));
    int e;
    e=strlen(jog);
    n->n_jogo= (char*) can_fail_malloc(sizeof(char)*(e+1));
    strcpy(n->n_jogo,jog);
    n->equipa1=eq1;
    n->equipa2=eq2;
    n->sc1=sc1;
