...
link_game* initGame_Hash(int size){
    link_game* p_hashgames= (link_game*)can_fail_malloc(sizeof(link_game)*size);  /* creates vector of pointers to link_game of size size*/
    int i = 0;
    while (i < size){
...
void insertGame_Hash(int size, link_game* h, Game* g){
    ...
    link_game ptaux = (link_game)can_fail_malloc(sizeof(struct node_game)); /* Allocate size for node_game */
    ptaux -> pgame = g;
...
lnk_dblist insrtGHash_auxList(lnk_dblist head, Game* game){
    lnk_dblist ptr_aux = (lnk_dblist)can_fail_malloc(sizeof(struct db_listnode));     /* Allocate space for new node */
