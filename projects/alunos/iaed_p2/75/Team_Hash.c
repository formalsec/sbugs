...
link_team* initTeam_Hash(int size){
    int i;
    link_team* head = (link_team*)can_fail_malloc(sizeof(link_team)*size);  /* head is pointer to HashTable of teams */
    for(i=0; i < size; i++){
...
void insertTeam_Hash(int size, link_team* h, Team* t){
    ...
    link_team ptr_node = (link_team)can_fail_malloc(sizeof(struct node_team));  /* Creation of node to insert in HT*/
