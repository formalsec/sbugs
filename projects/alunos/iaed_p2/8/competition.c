...
Competition * comp_new(char * name) {
	...
	Competition * competition = (Competition *) can_fail_malloc(sizeof(Competition));
...
Comp_node * comp_new_node(Game *g) {
    Comp_node * cn = (Comp_node*) can_fail_malloc(sizeof(Comp_node));
	EXIT_IF_ERROR("comp_new_node: can_fail_malloc");
    cn->game = g;
    return cn;
}
