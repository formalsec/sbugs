...
hash_teams create_teams_hash_table(int table_size) {
    ...
    hash_teams table = can_fail_malloc(sizeof(struct my_hash_teams));

    /* Allocates memory for each node of table and initializes them as NULL */
    table->table = can_fail_calloc(table_size, sizeof(struct sl_node));
...
void expands_team_hash(hash_teams table) {
    ...
    table->table = (sl_link*)
                    can_fail_calloc(table->max_size * 2, sizeof(sl_link));
...
hash_games create_games_hash_table(int table_size) {
    ...
    /* Allocates memory for table */
    hash_games table = can_fail_malloc(sizeof(struct my_hash_games));

    /* Initializes each node as NULL */
    table->table = can_fail_calloc(table_size, sizeof(struct dl_node));
...
void expands_games_hash(hash_games table) {
    ...
    /* Resets and allocates more space for table's pointers */
    table->table = (dl_link*)
                    can_fail_calloc(table->max_size * 2, sizeof(dl_link));
