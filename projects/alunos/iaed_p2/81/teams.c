...
fut_team creates_team(char* name) {
    ...
    fut_team new = can_fail_calloc(1, sizeof(struct my_team));
...
sl_list create_sl_list(){
    ...
    sl_list new_list = can_fail_calloc(1, sizeof(struct singly_linked_list));
...
sl_link create_team_node(sl_link next, fut_team team) {
    ...
    sl_link new = can_fail_calloc(1, sizeof(struct sl_node));
