...
char* str_duplicate(char str[]){
    ...
    char* copia;
    copia = (char*) can_fail_malloc(sizeof(char)*(len+1));
    for (i = 0; i < len; ++i){
        copia[i] = str[i];
    }
    copia[i] = '\0';
...
pEquipa cria_equipa(char str[1024]){
    pEquipa nova_equipa;
    nova_equipa = (pEquipa) can_fail_malloc(sizeof(equipa));
...
pEquipa* cria_tabela_equi(){
    long i;
    pEquipa* tabela;
    tabela = (pEquipa*) can_fail_malloc(sizeof(pEquipa)*tabEq_size);
...
void merge(pEquipa *vetor, int e, int d, int m){
    int i, j, k;
    pEquipa *aux;
    aux = (pEquipa*) can_fail_malloc(sizeof(pEquipa) * tabEq_ocup);
    for (i = m + 1; i > e; --i)
        aux[i-1] = vetor[i-1];
    for (j = m; j < d; ++j)
        aux[d + m - j] = vetor[j+1];
...
pJogo cria_jogo(char nome[1024], pEquipa e1, pEquipa e2, int sc1, int sc2){
    pJogo novo_jogo;
    novo_jogo = (pJogo) can_fail_malloc(sizeof(jogo));
...
pJogo* cria_tabela_jogo(){
    long i;
    pJogo* tabela;
    tabela = (pJogo*) can_fail_malloc(sizeof(pJogo)*tabJo_size);
