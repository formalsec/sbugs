...
void s(int cont,table_jogos tab_j,table_eq tab_eq) {
    ...
    if (match->winner != NULL) {
        old_winner = can_fail_malloc(sizeof(char)*strlen(match->winner)+1);
...
void g(int cont,table_eq tab_eq) 
{
    ...
    vetor = (char**)can_fail_malloc(sizeof(char*)*nr_eq);
    ...
    for (i = 0; i<tab_eq->M && j != nr_eq;i++) {
        ...
        while(temp != NULL) {
            if (temp->equipa->vitorias == max_wins) {
                vetor[j] = can_fail_malloc(sizeof(char)*strlen(temp->equipa->nome)+1);
