...
{
    listagem lst;
    lst = can_fail_malloc(sizeof(struct lst));
    lst->head = NULL;
    lst->tail = NULL;
    return lst;
}
...
table_eq Init_eq(int m)
{
    int i;
    table_eq table;
    table = (table_eq)can_fail_malloc(sizeof(struct hash_eq));
    table->M = m;
    table->heads = can_fail_malloc(table->M*sizeof(struct node_eq));
    for (i = 0; i < table->M; i++) 
    {
        table->heads[i] = NULL;
    }
...
table_jogos Init_jogo(int m)
{
    int i;
    table_jogos table;
    table = (table_jogos)can_fail_malloc(sizeof(struct hash_jogos));
    table->M = m;
    table->heads = can_fail_malloc(table->M*sizeof(struct node_jogo));
    for (i = 0; i < table->M; i++) 
    {
        table->heads[i] = NULL;
    }
