...
bloco_hash_equipa *inicializa_hash_e(){
    int i;
    bloco_hash_equipa *hash_table_e = can_fail_malloc(sizeof(bloco_hash_equipa)*Tamanho_hash);
...
void guarda_equipa_hash(bloco_hash_equipa *table_e,equipa *e){
    ...
    bloco_hash_equipa equipa1 = can_fail_malloc(sizeof(struct bloco_h_equipa));  /* guarda memoria para o bloco que vai guardar na hash table */
...
bloco_hash_jogo *inicializa_hash_j(){
    int i;
    bloco_hash_jogo *hash_table_j = can_fail_malloc(sizeof(bloco_hash_jogo)*Tamanho_hash);
    for(i = 0;i < Tamanho_hash; i++){
        hash_table_j[i] = NULL;
    }
    return hash_table_j;
...
void guarda_jogo_hash(bloco_hash_jogo *table_j,jogo *j){
    int pos = hash(j->nome);
    bloco_hash_jogo jogo1 = can_fail_malloc(sizeof(struct bloco_h_jogo));
