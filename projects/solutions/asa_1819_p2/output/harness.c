...
int main()
{
  ...
  for (i = 0; i < V; ++i)
  {
    adj[i].l = calloc(1000, sizeof(arc));
    adj[i].alloc_size = 1000;
    adj[i].size = 0;
  }
...
void pArcList_add(pArcList *list, arc *arc1)
{
  ...
  if (list->size > alloc_size)
  {
    ...
    list->l = realloc(list->l, ((size_t) list->alloc_size) * (sizeof(arc *)));
  }
...
arc *addArcSingle(int source, int destination, int capacity, arc *dual)
{
  ...
  if (adj[source].size > alloc_size)
  {
    ...
    adj[source].l = realloc(adj[source].l, ((size_t) adj[source].alloc_size) * (sizeof(arc)));
  }
...
void addArc(int source, int destination, int capacity)
{
  arc *dual = addArcSingle(source, destination, capacity, (void *) 0);
...
char dfs(pArcList *path, int *maxFlow)
{
  ...
  *maxFlow = 0x7fffffff;
  ...
  return dfs_visit(1, path, maxFlow);
}
...
void FordFulkersonAndCut()
{
  ...
  for (i = 1; i < e_end; ++i)
  {
    ...
    for (j = 0; j < adj[i].size; ++j)
    {
      ...
      if (adj1->capacity > 0)
        continue;
      if (adj1->capacity != adj1->flow)
        continue;
      if ((((dest >= (e_start + e)) && (dest < e_end)) && (i >= e_start)) && (i < (e_start + e)))
        continue;
    ...
    source = i;
    if ((source >= (e_start + e)) && (source < e_end))
      source -= e;
...
void PrintAdjGraph()
{
  ...
  for (i = 0; i < V; ++i)
  {
    ...
    {
      arc *arc1 = &adj[i].l[j];
      printf("\t(%d) -- [%d/%d] --> (%d)", i, arc1->flow, arc1->capacity, arc1->destination);
    }
...
int Quick_Partition(int *v, int l, int r)
{
  int i = ((int) l) - 1;
  int t;
  int j = r;
